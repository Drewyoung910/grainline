...
- `npx prisma validate`
- `npx prisma generate`
- For `SavedSearch` Release 0, deploy the pre-RLS owner-RPC migration while RLS
  is still off, verify it, and only then deploy the app release that calls the
  RPCs. Because `vercel.json` automatically runs every pending migration, the
  exact Release 0 artifact/cherry-pick must exclude the later RLS policy
  migration, and Release-0 CI must assert that migration is absent. On that
  clean artifact only, set `SAVED_SEARCH_RLS_DEPLOY_PHASE=release-0`; the
  production deploy guard requires the RPC migration present and the RLS
  migration absent before `prisma migrate deploy` can run.
- Do not set `SAVED_SEARCH_RLS_DEPLOY_PHASE=phase-a-reviewed` until the required
  ...
  not a repair for a rejected build, and requires both rollout migrations.
