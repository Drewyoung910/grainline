...
- `BUYER_DELETION_REPLAY_PROOF_CONFIRM=test-mode-replay BUYER_DELETION_REPLAY_PROOF_DB_CONFIRM=staging-or-local-read BUYER_DELETION_REPLAY_PROOF_SESSION_ID="<cs_test...>" BUYER_DELETION_REPLAY_PROOF_EVIDENCE_PATH="buyer-deletion-replay-evidence.json" npm run audit:buyer-deletion-replay` (after a real Stripe test-mode checkout completion/replay whose source buyer was deleted, suspended, or missing before webhook processing; verifies the local blocked-review order, buyer-PII purge, exact Stripe-bound event through the rollback-only fixed lease, refund ledger, and audit evidence without direct webhook-table SELECT)
