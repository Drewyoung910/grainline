...
- Stripe: platform snapshot webhook endpoint registered at `https://thegrainline.com/api/stripe/webhook` with exactly the ten handled platform events: `checkout.session.completed`, `checkout.session.async_payment_succeeded`, `checkout.session.expired`, `checkout.session.async_payment_failed`, `charge.refunded`, `charge.dispute.created`, `charge.dispute.updated`, `charge.dispute.closed`, `charge.dispute.funds_withdrawn`, and `charge.dispute.funds_reinstated`. Do not subscribe this endpoint to `payment_intent.*` events while Checkout Sessions are card-only and order creation is driven by Checkout events.
- Stripe: classic Connect webhook endpoint registered at `https://thegrainline.com/api/stripe/webhook/connect`, scoped in Stripe to events on connected accounts, subscribed only to `payout.failed`, and bound to the distinct `STRIPE_CONNECT_WEBHOOK_SECRET`.
...
- The 2026-07-19 slot 1 at commit `c188ea4306ed15b1160a8525ad8c38baf934dfa4` and deployment `dpl_2U8ccnSKFcgiPUrF9SyYtDYz2woQ` is failed evidence and consumed its run; slot 2 was not called. The one-statement RPC candidate passed target/burst p95 (19.9/35.5 ms versus 19.8/35.6 ms), but the legacy wrapper-versus-autocommit Prisma burst comparison failed (146.5 ms versus 71.0 ms). For SavedSearch Bucket A/Phase A only, that comparison was treated as diagnostic because the promoted list/delete candidate was the separately gated one-statement RPC and the remaining wrappers were already transactions. The Bucket B branch has removed that diagnostic escape hatch: generic raw and Prisma wrapper-versus-autocommit latency is blocking again. This does not convert the failed run into a pass; Bucket B requires a fresh run id, trigger, commit, deployment, and two new passing slots.
...
- After Phase B, follow
  ...
  newly reset environment-scoped migration secret, absence of every privileged
  database variable from Vercel, a single pinned Neon owner reset that
  ...
  The pre-encrypted SQL `ALTER ROLE` method is retired after PostgreSQL `XX000`;
...
- `STRIPE_WEBHOOK_SUBSCRIPTIONS_PROOF_CONFIRM=live-read STRIPE_WEBHOOK_SUBSCRIPTIONS_PROOF_EVIDENCE_PATH="stripe-webhook-subscriptions-evidence.json" npm run audit:stripe-webhooks` (live Stripe read-only; verifies the platform snapshot, classic Connect payout, and Connect v2 thin destination URLs/event sets; does not prove deployed signing-secret values or the classic Connect source flag)
...
- `BUYER_DELETION_REPLAY_PROOF_CONFIRM=test-mode-replay BUYER_DELETION_REPLAY_PROOF_DB_CONFIRM=staging-or-local-read BUYER_DELETION_REPLAY_PROOF_DATABASE_URL="<restricted-runtime-url>" BUYER_DELETION_REPLAY_PROOF_DATABASE_TARGET=neon-staging BUYER_DELETION_REPLAY_PROOF_DATABASE_NAME="<database>" BUYER_DELETION_REPLAY_PROOF_DATABASE_ENDPOINT_ID="<ep-staging>" BUYER_DELETION_REPLAY_PROOF_DATABASE_REGION="<region.provider>" BUYER_DELETION_REPLAY_PROOF_SESSION_ID="<cs_test...>" BUYER_DELETION_REPLAY_PROOF_EVIDENCE_PATH="buyer-deletion-replay-evidence.json" npm run audit:buyer-deletion-replay` (after a real Stripe test-mode checkout completion/replay whose source buyer was deleted, suspended, or missing before webhook processing; the dedicated runtime connection must bind an explicit non-production target and pass engine-attested role checks; verifies the local blocked-review order, buyer-PII purge, exact Stripe-bound event through the rollback-only fixed lease, refund ledger, and audit evidence without direct webhook-table SELECT)
- `STRIPE_WEBHOOK_EVENT_ACTIVATION_POSTFLIGHT_CONFIRM=verify-production-stripe-webhook-event-activation-runtime-read-only STRIPE_WEBHOOK_EVENT_ACTIVATION_POSTFLIGHT_RELEASE_COMMIT="<exact-activated-main>" STRIPE_WEBHOOK_EVENT_ACTIVATION_POSTFLIGHT_MAIN_CI_RUN_ID="<successful-ci>" STRIPE_WEBHOOK_EVENT_ACTIVATION_POSTFLIGHT_MIGRATION_RUN_ID="<successful-migration-run>" STRIPE_WEBHOOK_EVENT_ACTIVATION_POSTFLIGHT_EVIDENCE_PATH="stripe-webhook-event-activation-production-postflight-<exact-activated-main>.json" npm run ops:stripe-webhook-event-activation-postflight` (after policyless StripeWebhookEvent activation only; uses the actual pooled production runtime role in a read-only transaction, never the owner role, and retains sanitized mode-0600 evidence)
- `SELLER_PAYOUT_EVENT_ACTIVATION_POSTFLIGHT_CONFIRM=verify-production-seller-payout-event-activation-runtime-read-only SELLER_PAYOUT_EVENT_ACTIVATION_POSTFLIGHT_RELEASE_COMMIT="<exact-activated-main>" SELLER_PAYOUT_EVENT_ACTIVATION_POSTFLIGHT_MAIN_CI_RUN_ID="<successful-ci>" SELLER_PAYOUT_EVENT_ACTIVATION_POSTFLIGHT_MIGRATION_RUN_ID="<successful-migration-run>" SELLER_PAYOUT_EVENT_ACTIVATION_POSTFLIGHT_EVIDENCE_PATH="seller-payout-event-activation-production-postflight-<exact-activated-main>.json" npm run ops:seller-payout-event-activation-postflight` (after policyless SellerPayoutEvent activation only; uses the actual pooled production runtime role in a repeatable-read/read-only transaction, never the owner role, and retains sanitized mode-0600 evidence)
- `SELLER_PAYOUT_EVENT_FORCE_POSTFLIGHT_CONFIRM=verify-production-seller-payout-event-force-runtime-read-only SELLER_PAYOUT_EVENT_FORCE_POSTFLIGHT_RELEASE_COMMIT="<exact-force-main-with-postflight-package>" SELLER_PAYOUT_EVENT_FORCE_POSTFLIGHT_MAIN_CI_RUN_ID="<successful-ci>" SELLER_PAYOUT_EVENT_FORCE_POSTFLIGHT_MIGRATION_RUN_ID="32672434812" SELLER_PAYOUT_EVENT_FORCE_POSTFLIGHT_EVIDENCE_PATH="seller-payout-event-force-production-postflight-<exact-force-main-with-postflight-package>.json" npm run ops:seller-payout-event-force-postflight` (after SellerPayoutEvent FORCE only; requires the exact clean source and actual pooled production runtime, rejects owner/direct/aliased URLs, proves policyless ENABLE plus FORCE in an engine-read-only transaction, and retains evidence distinct from Phase A)
- `STRIPE_WEBHOOK_EVENT_FORCE_POSTFLIGHT_CONFIRM=verify-production-stripe-webhook-event-force-runtime-read-only STRIPE_WEBHOOK_EVENT_FORCE_POSTFLIGHT_RELEASE_COMMIT="<exact-force-main>" STRIPE_WEBHOOK_EVENT_FORCE_POSTFLIGHT_MAIN_CI_RUN_ID="<successful-ci>" STRIPE_WEBHOOK_EVENT_FORCE_POSTFLIGHT_MIGRATION_RUN_ID="<successful-migration-run>" STRIPE_WEBHOOK_EVENT_FORCE_POSTFLIGHT_EVIDENCE_PATH="stripe-webhook-event-force-production-postflight-<exact-force-main>.json" npm run ops:stripe-webhook-event-force-postflight` (after the separate StripeWebhookEvent FORCE migration only; uses the actual pooled production runtime role in a read-only transaction and retains distinct sanitized mode-0600 evidence)
- `CHECKOUT_STOCK_RESERVATION_FORCE_POSTFLIGHT_CONFIRM=verify-production-checkout-stock-reservation-force-runtime-read-only CHECKOUT_STOCK_RESERVATION_FORCE_POSTFLIGHT_RELEASE_COMMIT="<exact-force-main>" CHECKOUT_STOCK_RESERVATION_FORCE_POSTFLIGHT_MAIN_CI_RUN_ID="<successful-ci>" CHECKOUT_STOCK_RESERVATION_FORCE_POSTFLIGHT_MIGRATION_RUN_ID="<successful-migration-run>" CHECKOUT_STOCK_RESERVATION_FORCE_POSTFLIGHT_EVIDENCE_PATH="checkout-stock-reservation-force-production-postflight-<exact-force-main>.json" npm run ops:checkout-stock-reservation-force-postflight` (after the separate CheckoutStockReservation FORCE migration only; requires the clean exact release and actual pooled production runtime role, rejects owner/direct/aliased URLs, proves exact FORCE catalog and denial inside an engine-read-only transaction, and retains distinct sanitized mode-0600 evidence)
...
- Stripe platform snapshot, classic Connect payout, and Connect v2 thin webhook delivery, including the read-only artifact from `npm run audit:stripe-webhooks`, screenshots or exported evidence of the exact event subscriptions and classic Connect source scope listed above, and separate signing-secret matching evidence for `STRIPE_WEBHOOK_SECRET`, `STRIPE_CONNECT_WEBHOOK_SECRET`, and `STRIPE_V2_WEBHOOK_SECRET`. Connect v2 signed delivery remains launch-blocking even though it is not a `StripeWebhookEvent` RLS-activation prerequisite.
...
- Stripe test-mode buyer-deletion replay proof artifact from `npm run audit:buyer-deletion-replay`, covering a real paid Checkout Session whose original buyer was deleted, suspended, or missing before webhook processing, with blocked review state, purged buyer snapshot fields, rollback-only fixed-lease `processed` evidence for the exact Stripe-bound event, automatic refund evidence, and local audit rows.
