...
- Stripe platform snapshot, classic Connect payout, and Connect v2 thin webhook delivery, including the read-only artifact from `npm run audit:stripe-webhooks`, screenshots or exported evidence of the exact event subscriptions and classic Connect source scope listed above, and separate signing-secret matching evidence for `STRIPE_WEBHOOK_SECRET`, `STRIPE_CONNECT_WEBHOOK_SECRET`, and `STRIPE_V2_WEBHOOK_SECRET`.
