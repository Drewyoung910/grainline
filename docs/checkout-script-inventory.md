...
- `/listing/[id]` — Buy Now modal; mounts `src/components/BuyNowCheckoutModal.tsx`; `EmbeddedCheckoutPanel` is dynamically imported only after server-side checkout session creation and entry to the payment step.
...
| `https://js.stripe.com` | `@stripe/stripe-js` via `loadStripe()` in `src/components/EmbeddedCheckoutPanel.tsx` | `/cart`, Buy Now modal payment step | Stripe Embedded Checkout; card data is collected by Stripe-hosted payment UI | Must be loaded directly from Stripe. Do not self-host or bundle. Do not add stale SRI hashes unless Stripe explicitly supports the exact URL/update behavior. Listing pages must not load Stripe.js until a real buyer reaches the payment step. |
