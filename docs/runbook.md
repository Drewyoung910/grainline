...
- The compatible application deployment is now complete at exact main
  ...
  Do not treat those 401s as authenticated checkout proof. The separate
  fail-closed production smoke passed from exact main
  `e9d343b6f316ceb1c75553aec77e9f310a12d802`, CI `31829740992`, proving the
  real cart/Buy Now, retry/resume/rollback, stock-restore and signed-expiry
  matrix with every cleanup invariant true. Follow
...
- CheckoutStockReservation Phase A is live from exact main
  ...
  changes and `migrationDirectoryCreated=false`. The separate promoted release
  verifier is
...
1. Confirm the platform snapshot endpoint is `/api/stripe/webhook`.
2. Confirm the deployed `STRIPE_WEBHOOK_SECRET` matches that platform endpoint secret.
3. Confirm the platform endpoint is subscribed only to the handled platform snapshot events:
   ...
   no other events. Do not add `payment_intent.*` events unless checkout payment
   methods and webhook failure handling are expanded.
4. Confirm the classic Connect endpoint is `/api/stripe/webhook/connect`, was
   created with connected-account source scope, uses the distinct
   `STRIPE_CONNECT_WEBHOOK_SECRET`, and subscribes only to `payout.failed`.
5. Confirm the Connect v2 thin-event endpoint is `/api/stripe/webhook/v2`, uses
   `STRIPE_V2_WEBHOOK_SECRET`, and is subscribed only to the `v2.core.account`
   account-notification event family.
6. Before launch, run the read-only provider subscription proof:
   ...
   Retain the sanitized artifact with launch evidence. This proves all three
   endpoint URLs and exact event families from Stripe's APIs. It does not prove
   deployed `STRIPE_WEBHOOK_SECRET`, `STRIPE_CONNECT_WEBHOOK_SECRET` or
   `STRIPE_V2_WEBHOOK_SECRET` values, and the classic API does not expose the
   Connect source flag; keep separate Stripe/Vercel dashboard evidence for
   endpoint-secret matching, source scope and rotation records.
7. In Stripe Dashboard, filter failed events and replay after the app is healthy.
8. Verify `StripeWebhookEvent` rows show processed state for replayed events.
9. For checkout or refund incidents, compare Stripe charge/payment/refund IDs against `OrderPaymentEvent`.
9. If a seller refund, staff case refund, or blocked-checkout refund was accepted by Stripe, confirm the refund ID is durably recorded on the `Order`, in `OrderPaymentEvent`, and in the related `SystemAuditLog` entry. For reverse-transfer refunds, also inspect `metadata.refundAccounting.transferReversalId` and `transferReversalAmountCents` when Stripe returned them so buyer refund amount and seller recovery are not conflated. If the Stripe webhook delivery is still failed, replay it rather than issuing another manual refund first; the blocked-checkout refund idempotency key is session-scoped. If the original seller became banned or soft-deleted before the first local record, do not reactivate the account or issue another refund: use the exact Admin-PIN refund reconciliation so the database can derive the one reviewed source-history exception.
