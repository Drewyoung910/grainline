...
| CC-A11 | Accepted launch requirement | Damage/not-as-described disputes have no evidence attachment model even though the Terms say staff review photos. Adding sensitive evidence after Case RLS would require another parent-scoped authority and retention rollout. | Include a first-party-upload-backed `CaseMessageAttachment` in the tightly coupled Case group before policy SQL. Allow reviewed image/PDF evidence only, verify object ownership and metadata at persistence time, inherit parent Case visibility, and define export/deletion/retention behavior. Do not accept arbitrary caller URLs. |
...
5. Include first-party CaseMessage evidence attachments in this group. Their
   parent visibility, upload verification, retention, export and deletion
   behavior must be proven before policy SQL.
