...
| Seventeen-member zero-direct prefix scope | `d750b6eb` / full native CI `34276777170` accepted 18-state, 36-function, 38-schema-record, four-table and restricted-role graph checks; global authority/configuration adapter is the current isolated successor | New exact-head native/full acceptance and serialized execution composition; no production entrypoint |
