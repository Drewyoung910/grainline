...
| `CheckoutStockReservation` | `BLOCKED_DESIGN` | Order, payment and shipping | Reservation payload and buyer or seller identifiers; checkout, Stripe and expiry repair | Actor/service partition is audited in `docs/checkout-stock-reservation-rls-audit.md`; next prove source-derived creation, generation-fenced repair, bounded resume/export and account cleanup in disposable PostgreSQL before compatible preparation |
