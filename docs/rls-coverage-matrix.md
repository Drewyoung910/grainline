...
| `Case` | `BLOCKED_DESIGN` | Case and case message | Dispute narrative, status and refund identifiers; buyer, seller, staff, cron and Stripe | Phase 4 database preparation, compatible application and the invariant-only migration are live and postflighted in production with RLS still off, FORCE off and zero policies. The four-function compatible read-mode convergence is staged and byte-pinned but not applied. The 80-reference baseline resolves to 79 machine-checked conversions plus one retired unused helper, zero ordinary direct access and 27 purpose-bound operations. Policyless ENABLE and FORCE remain separate later gates |
...
| `CaseMessage` | `BLOCKED_DESIGN` | Case and case message | Private dispute discussion; buyer, seller and staff | Phase 3 proved durable source-derived author kind, parent authority, bounded keyset history and locked timestamp/state transitions. Phase 4 database preparation, compatible application and invariant enforcement are live and postflighted with RLS off and zero ordinary direct Case-family access. The compatible read-mode candidate changes only four Case projection modes; activation remains a separate later release |
| `CaseMessageAttachment` | `BLOCKED_DESIGN` | Case and case message | Private dispute image evidence; inherits exact parent Case visibility | The DirectUpload private-object authority preparation and Case compatible database preparation are live while the feature remains disabled and Case-family RLS remains off. Promote and verify the compatible app before separately activating parent-derived authority, signed reads and lifecycle cleanup/export/retention |
