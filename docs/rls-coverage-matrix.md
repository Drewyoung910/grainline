...
| `StripeWebhookEvent` | `ALTERNATIVE_REVIEW` | Provider event ledgers | Six source-pinned fixed lease/maintenance functions; direct ordinary source removed | Compatible preparation is live; policyless ENABLE candidate and PUBLIC-safe rollback are isolated; compatible app deploy/drain, fresh exact-head PostgreSQL proof, activation and actual pooled-runtime postflight remain required; FORCE later |
