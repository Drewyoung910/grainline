...
database isolation program. Snapshot scope: 67 Prisma models.
...
`OrderPaymentEvent` closed its posture-only FORCE release from exact main
`6a20981b0af68f8322b6306715fc117e0826e36e`, CI `33443669979`, guarded
...
- `RLS_LIVE_PHASE_A_PENDING_POSTFLIGHT`: policyless Phase A and protected
  owner-side scope proofs are live, but the distinct actual pooled-runtime
  read-only postflight and sanitized evidence remain pending.
...
> **Current OrderPaymentEvent FORCE acceptance (2026-08-31):** PR #369 merged
> exact head `5db226fcd7a2f4ebb88e19bf85b4e9c27c2f3fea` as main
> `6a20981b0af68f8322b6306715fc117e0826e36e`; exact-main CI `33443669979`
> passed all 302 steps. Guarded production run `33445073482` applied only
> `20260831010000_force_order_payment_event_rls`, converged the reviewed
> grants, and passed migration status, the global grant/RLS audit, and the
> exact FORCE scope proof. The distinct actual pooled
> `grainline_app_runtime` postflight then passed ten checks inside an
> engine-attested repeatable-read/read-only transaction: policyless ENABLE plus
> FORCE, zero runtime/PUBLIC table or column authority, the exact 29-function
> catalog, direct-table and retired-entry denial, five retained read boundaries,
> and the fixed-writer read-only fence. It exported no rows and recorded
> `productionChangedByPostflight=false`. Retain sanitized mode-`0600` evidence
> SHA-256
> `d63cea7bd6a95232790aef4ecd4b279ae837bada1bad7cb80ef6aa604671eea1`.
> `Order`, `OrderItem`, and `OrderShippingRateQuote` remain separate domains.
>
> **Prior OrderPaymentEvent Phase-A acceptance (2026-08-31):** exact main
> `94dbe98ae5e7fbf95989be690fc20d47e76cdb12` passed CI `33357911021`.
> Guarded production run `33358695448` applied only
> `20260830030000_enable_order_payment_event_rls`, converged grants, and passed
> migration status, the global grant/RLS audit and both exact owner-side scope
> proofs. Production now has policyless ENABLE, explicit `NO FORCE`, zero
> runtime/PUBLIC table or column authority, zero policies, and the exact
> 16-runtime / 13-private function partition. Owner-side acceptance did not
> substitute for the distinct actual pooled `grainline_app_runtime` proof.
> PR #364 then merged the exact pooled-runtime
> postflight as main `aec47e6a...`; CI `33361381594` passed and the actual
> engine-read-only proof accepted Phase A with sanitized evidence SHA-256
...
>
> **Prior OrderPaymentEvent release note (2026-08-30):** the transition
...
> smokes and predecessor drain; conversion of the remaining direct-`Order`
...
> **Current zero-direct Order candidate (2026-09-05):** draft PR #429 leaves
> zero direct `Order`, `OrderItem`, and `OrderShippingRateQuote` access under
> `src`. The functional audit found two
...
> after clearing the current account ID. Sixteen byte-pinned compatible
> migrations are staged with disposable PostgreSQL proof for a bounded
> reservation `sourceSnapshot`, a
> made-to-order source-only reservation, explicit Order deauthorization fields,
> and a private immutable event-application ledger. The new columns and ledger
> are represented in Prisma and in this 67-model matrix. Nothing in this
> candidate has been applied to production.
> Both paid checkout writers and the post-payment Order graph/count read are
...
> Stripe event window: 30 days old, ten minutes of positive clock skew, and
> bounded database allowances around their respective 60-second platform and
> 30-second Accounts-v2 route ceilings. This closes a candidate-only
> 8-day/5-minute mismatch without weakening event-generation or source-object
> authority. The terminal seller cleanup is now bound to the reachable,
...
> Order review/refund fields to the route.
> Accept PostgreSQL 16 CI, promote the compatible prefix database-first, prove
> the deployed webhook, drain predecessor
...
| `Order` | `PLANNED_RLS` | Order, payment and shipping | Buyer PII, addresses, provider IDs, fulfillment and refunds; buyer, durable Order seller, staff, Stripe, Shippo and jobs | The current isolated candidate inventory is zero direct Order, OrderItem and quote source files. The target is policyless ENABLE then FORCE with zero direct runtime/PUBLIC authority. Historical-authority compatibility and fixed participant, export, eligibility, aggregate, analytics, metrics, fulfillment, refund, label, staff and lifecycle operations have disposable PostgreSQL proof. The checkout, fulfillment, refund, label, staff-mutation, seller-ban, staff-read and account-deletion audits corrected concrete product and concurrency defects rather than treating predecessor logic as automatically sound. Staff screens and the three audited staff mutations use a separately authenticated bounded `grainline_staff_read_runtime` candidate; the historical name is retained, but its exact surface is two reads, three mutations and one short-lived staff-capability mint with no table authority. The Server Actions repeat the signed Admin-PIN check. Account deletion now has an actor-bound fixed blocker/scrub candidate using durable seller identity, signed totals, database time and an after-lock recheck. Neither candidate is deployable until its exact database role/functions/grants are provisioned and proved first. Both paid checkout families now use the isolated fixed creation candidate, removing the duplicated direct writers and all direct OrderItem access. Post-payment notification/email input, low-stock state and first-sale classification now come from a separate active-event/session-bound fixed projection with blocked-order non-disclosure and deterministic concurrent-first-sale behavior. Exact-session idempotency now uses a separate active-event/session-bound closed classifier, including database-clock stale-lock recovery, without returning raw Order state. The blocked-refund transition family now uses a separate active-event/session-bound fixed review operation with database-derived refund and dispute precedence; seller deauthorization already uses its isolated fixed candidate. Staff-role provisioning, Stripe migration-tree integration, production inspection, compatible deployment/smoke/drain and final seller-key convergence remain required. The first authenticated route smoke is explicitly unaccepted: its quantity-two checkout and signed-expiry phase passed, then an invalid same-address Shippo fixture and a cleanup timestamp-without-time-zone round-trip defect failed closed. Exact main `9219201e` / CI `33612678508` completed the one accepted legacy cleanup recovery with zero persistent mutable residue; sanitized evidence SHA-256 `accb96c3c59c9cd1e9d0df8f12e6ef3161cf46f684ac5a4d422f07a97ead7239`. A fresh full smoke remains the gate; see `docs/order-authenticated-route-smoke-plan-20260902.md`. Convert and activate `Order` first, then audit/activate `OrderItem`, then `OrderShippingRateQuote` as separate releases. See `docs/order-core-pre-rls-audit.md`, `docs/order-stripe-webhook-authority.md`, `docs/order-account-deletion-authority.md`, `docs/order-label-product-authority-audit.md`, `docs/order-fulfillment-receipt-product-audit.md`, `docs/order-fulfillment-authority.md`, `docs/order-core-history-compatibility.md`, `docs/order-participant-list-authority.md`, `docs/order-participant-summary-authority.md`, `docs/order-participant-cursor-authority.md`, `docs/order-participant-detail-authority.md`, `docs/order-participant-detail-projection.md`, `docs/order-checkout-receipt-authority.md`, `docs/order-staff-read-authority.md`, `docs/order-participant-export-authority.md`, `docs/order-eligibility-authority.md`, `docs/order-public-aggregate-authority.md`, `docs/order-seller-analytics-authority.md` and `docs/order-seller-metrics-authority.md` |
...
| `OrderPaymentEvent` | `RLS_LIVE_FORCE` | Order, payment and shipping | Append-only payment/refund/dispute service evidence; signed Stripe, seller/staff refund authorities, bounded buyer/seller/staff projections and aggregate jobs | Dedicated audit pins a 26-file historical baseline and 25-file current source matcher and selects policyless ENABLE then FORCE with zero direct table authority. The compatible production stack now covers launch-safe full-refund semantics, canonical dispute consumers, refund-only export projections, UTC generation-fenced claims, restart-safe blocked-checkout handoff, atomic record/finalize plus participant delivery, two source-bound signed refund/dispute operations, evidence-bound ambiguous provider reconciliation, database-derived first-write recovery if the original seller becomes banned/deleted, and atomic staff Case finalization with durable buyer/seller Notification plus email-outbox reservation. Recovery reuses exact source-validating functions and adds no caller-controlled target or generic runtime operation. Exact main `8f4cf2df34a9f700adebc910107ac2dbb878054a`, CI `32792800761`, inspection `32793276224` and guarded run `32793394895` applied all five migrations and proved RLS remains off with predecessor CRUD retained. Exact main `5d3b402317084d9d2af6b8bdf52300a800eda0d8`, CI `32795444295`, and its actual pooled-runtime postflight accepted the prepared catalog and boundaries without mutation. Exact main `a09827e0a641ec2f7e228520661cd7e74625bb0d` is deployed as `dpl_8FMq11zfZT166Dve7Vf6sTJTXFzX`; blocked-checkout delivery and transfer-binding compatibility are live while RLS remains off. The first genuine paid automatic proof exposed a destination-transfer visibility race and was not accepted. Its exact 541-cent refund and 475-cent transfer were reconciled once, all temporary fixtures were removed, and sanitized evidence at SHA-256 `d3a6ab9a109de1d607920e72ec92ba8811c3971104f079cde7e8525c504ba4f7` explicitly records `automaticProductionProofPassed=false`. Guarded run `33176428000` applied the exact signed-refund omitted-identity successor; its original final scope failed only on obsolete predecessor-body proof composition. PR #303 merged as exact main `4ea201c411afd5e065200f81dbbf18d9dd5044d1` with CI `33190374131`; restart-safe run `33194758799` skipped migration replay and passed status, the global audit and corrected final scope. The distinct pooled-runtime postflight passed with sanitized evidence SHA-256 `7849c8383164ae46d94bd8522710c8dbfdd1037da1e23281db1c3ef3e5b9e477`, so signed-refund compatibility is accepted with RLS off and predecessor CRUD retained. The first genuine signed-dispute delivery then failed closed before side effects because the predecessor admitted synthetic `dp_` identifiers while Stripe uses canonical `du_`; the exact journal is preserved, success evidence is absent, and the isolated byte-pinned correction changes only that predicate with grants and RLS posture unchanged. The signed dispute, fresh automatic blocked-checkout, seller full-refund, and staff Case full-refund proof families are now accepted. The invariant and five-function read-authority migrations are accepted in production at exact main `513053dc6f2f6fb527f85e45fe3a18a8317fa701` with main CI `33295803412`, inspection `33296114340`, guarded runs `33296358390` and `33296422900`, and a distinct successful pooled-runtime read-only postflight. The converted application is live from exact main `07eb9fc57bcec4d2fbac4d9ffc58b814ff78f5a8`, CI `33297246142`, as READY deployment `dpl_7UeENeZebXL9yL481DWrXkDpWd4R`; predecessor `dpl_2WkGbkiDdD8ySQYnCTur7ND3n2kd` remains READY and undrained. The aggregate-authority migration is now accepted in production at exact main `298088d55901c7096579766adaf9f35f1ead8085`, CI `33303933012`, aggregate-only read-only inspection `33304264914` and guarded run `33304372055`: two anti-forgery Order projections, three runtime-private helpers and two exact triggers are live with zero projection mismatches. RLS remains off and predecessor CRUD remains retained. The distinct pooled-runtime postflight, compatible application deployment and bounded authenticated aggregate/review smoke are accepted at exact main `6a74f1dd385035f2ff376d79a482ca989cf4ab02`, CI `33309431664`, and sanitized evidence SHA-256 `5ec5518ccc3b0cdfd6c3e8542d9f57f722029d7dfdda5db9f4e50d22ddb633ee`. Phase A was accepted from exact main `aec47e6a104f1fa54b6ee0e894751850d51390ec`, CI `33361381594`, migration run `33358695448` and pooled-runtime evidence SHA-256 `d4acc792856d0a3260cff9d597a27d6335650b2820536175f4f725185e7c7bfd`. FORCE is accepted from exact main `6a20981b0af68f8322b6306715fc117e0826e36e`, CI `33443669979`, guarded run `33445073482`, and pooled-runtime evidence SHA-256 `d63cea7bd6a95232790aef4ecd4b279ae837bada1bad7cb80ef6aa604671eea1`; production is policyless ENABLE plus FORCE with zero direct runtime/PUBLIC table authority and the exact 16-runtime/13-private function partition. Do not bundle quote, Order or OrderItem activation. See `docs/order-payment-event-pre-rls-audit.md`, `docs/order-payment-event-aggregate-authority.md`, `docs/order-payment-event-compatible-production-preparation.md`, `docs/order-payment-event-refund-contract.md`, `docs/order-payment-event-dispute-state.md`, `docs/order-payment-event-account-export.md`, `docs/order-payment-event-refund-claim-generation.md`, `docs/order-payment-event-refund-record-authority.md`, `docs/order-payment-event-blocked-checkout-refund-delivery.md`, `docs/order-payment-event-case-refund-delivery.md`, `docs/order-payment-event-signed-authority-design.md`, `docs/order-payment-event-signed-refund-identity.md`, `docs/order-payment-event-signed-dispute-identity.md` and `docs/order-payment-event-refund-reconciliation.md` |
...
| `OrderStaffCapability` | `COMPATIBLE_CANDIDATE` | Order isolated staff authority | Five-minute one-use actor, target and restore-payload authorization; isolated staff mint and atomic ordinary-transaction consume only | The seller-ban audit found that the original two Order review functions trusted a caller-supplied ADMIN id while remaining callable by ordinary runtime. The corrected candidate creates this table as policyless ENABLE plus FORCE with zero runtime/PUBLIC table authority. Only the exact isolated staff session can mint an actor/target/operation-bound capability; the existing ordinary ban transaction atomically consumes it, binds restore payload bytes, and rolls consumption back with the surrounding User, SellerProfile, commission and audit changes. Prove expiry, replay denial, payload substitution denial, ordinary mint denial, direct-table denial and exact six-function staff catalog before compatible deployment |
...
| `SellerDeauthorizationApplication` | `COMPATIBLE_CANDIDATE` | Order and signed Stripe lifecycle | Immutable signed terminal-account closure replay evidence; fixed Stripe service operation only | The isolated zero-direct prefix creates this ledger with policyless ENABLE plus FORCE, zero runtime/PUBLIC table authority, and an immutable UPDATE/DELETE trigger. Its historical name is retained as internal compatibility terminology, but the application operation accepts only the provider-subscribed `v2.core.account.closed` event on the separately signed Accounts-v2 route and derives seller and Order effects from that active signed event generation. The dead classic OAuth deauthorization family is rejected. Disposable engine tests cover replay, rollback, forged generations, wrong event family, direct denial, and complete timestamp/event witnesses. PostgreSQL 16 CI, compatible production preparation, and the distinct pooled-runtime postflight remain required |
...
> merged as exact main `6a74f1dd385035f2ff376d79a482ca989cf4ab02`;
> exact-main CI `33309431664` passed. The bounded authenticated smoke
> re-attested the exact deployment and predecessor, proved unauthenticated
> review denial, rendered `/account` with status 200 and reached the converted
> locked review-eligibility query, which returned the expected delivered-order
> 403 for a fresh nonexistent listing. It created zero database, payment,
> provider or review fixtures; revoked its one canary session; reset only exact
> transient review/account-cache state; and removed its restart journal. Retain
...
> The current gate is conversion and proof of the remaining transition,
...
6. Continue the Order/payment/shipping program: StripeWebhookEvent policyless
   ...
   The invariant release is byte-pinned at migration
   ...
   `33295803412`, fresh inspection `33296114340` and guarded runs
   `33296358390` / `33296422900` applied the invariants and five bounded read
   functions. The distinct actual pooled-runtime postflight passed with
   sanitized mode-`0600` evidence SHA-256
   `da16187441681f0bf5bfd394cb3ec14c59ea5fc0f496812ae20bb5d4b0749a17`.
   RLS remains off and predecessor CRUD remains retained. Compatible app
   deployment and smoke plus conversion of every remaining direct consumer
   precede predecessor drain and separate ENABLE/FORCE releases.
