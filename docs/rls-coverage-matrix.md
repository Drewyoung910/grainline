...
| `Case` | `RLS_LIVE_PHASE_A` | Case and case message | Dispute narrative, status and refund identifiers; buyer, seller, staff, cron and Stripe | Policyless ENABLE, zero policies, zero direct runtime table/column authority and the 27-operation fixed authority are live and pooled-runtime accepted in migration run `30939836526`. The posture-only FORCE release is byte-pinned and merged at exact main `5c1564a8e6994497987fe62e28b61db03737285c`, but is not live |
...
| `StripeWebhookEvent` | `ACTIVATION_CANDIDATE` | Provider event ledgers | Six fixed lease/maintenance functions; direct ordinary source removed | Policyless ENABLE candidate prepared; exact-head PostgreSQL proof and separate production activation still required; FORCE later |
