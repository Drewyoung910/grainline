...
| `Order` | `PLANNED_RLS` | Order, payment and shipping | Buyer PII, addresses, provider IDs, fulfillment and refunds; buyer, durable Order seller, staff, Stripe, Shippo and jobs | Fresh audit pins 41 direct source files and selects policyless ENABLE then FORCE with zero direct runtime/PUBLIC authority. Durable seller keys and service-ledger prerequisites are live, but consumers still use mutable Listing ownership/history, mixed actor projections are absent, raw shipping quotes cross export, and write/maintenance families remain direct. Convert `Order` first, then `OrderItem`, then `OrderShippingRateQuote` as separate releases. See `docs/order-core-pre-rls-audit.md` |
