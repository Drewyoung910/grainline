...
| `Notification` | `PLANNED_RLS` | Bucket B Notification | Direct user-owned reads and mark-read updates with cross-user and system creation | All 54 emission paths have source-derived payload/family validation drafts and the shared block-race lock protocol is statically guarded; provisioning/audit preserve exact `SELECT` + `UPDATE(read)`, initial `NO FORCE`, two policies, and the 25-function public/runtime ACL split. Disposable PostgreSQL now proves all ten granted creation families, all 26 family-dispatched private-core source types, all 38 successful source/type pairs and their security-relevant action/recipient variants, replay, forged-recipient rejection, back-in-stock claim behavior, and both block orderings. A byte-pinned split proof also demonstrates preparation compatibility, locked activation, Prisma migration ordering, the migration-derived generic grant audit, and database-first rollback with exact activation restoration; the technically green all-at-once candidate was rejected because it could not bridge old and new app releases safely. The activation purge remains intentionally irreversible. The synthetic provider result rejects the interactive wrapper and selects one-statement recipient RPCs directionally but failed the generic gate. Two real-table provider slots were separately consumed without replay by invalid `pg_catalog.current_user` and `pg_catalog.exists(...)` harness queries; both disposable environments were fully removed and both query shapes now have regressions. A third predeployment-only attempt consumed no slot and exposed a standalone-TSX CommonJS bootstrap defect; it was fully removed and the preflight bootstrap is guarded. The fourth attempt passed local preflight and provider slot 1, then failed slot 2 only because the fixed within-slot 2x p95 comparison measured a symmetric first-workload concurrency ramp; it was fully removed, the gate now primes each side at full concurrency, and a fresh two-slot proof remains mandatory. Inspect legacy aggregates, keep any purge atomic with activation, then pass a fresh candidate-aligned authenticated real-table/route and application-deployment rollback gate before staging |
