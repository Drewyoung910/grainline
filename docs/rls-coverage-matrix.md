...
| `Order` | `PLANNED_RLS` | Order, payment and shipping | Buyer PII, addresses, provider IDs, fulfillment and refunds; buyer, durable Order seller, staff, Stripe, Shippo and jobs | Fresh audit pins 41 direct source files and selects policyless ENABLE then FORCE with zero direct runtime/PUBLIC authority. Historical-authority compatibility and four fixed buyer/seller count/keyset list operations now have isolated candidates with disposable PostgreSQL proof. Detail/staff/export/aggregate/write conversion, raw shipping-quote export, production inspection and final seller-key convergence remain required. Convert `Order` first, then `OrderItem`, then `OrderShippingRateQuote` as separate releases. See `docs/order-core-pre-rls-audit.md`, `docs/order-core-history-compatibility.md` and `docs/order-participant-list-authority.md` |
