...
| `OrderPaymentEvent` | `COMPATIBLE_PREPARATION_LIVE` | Order, payment and shipping | Append-only payment/refund/dispute service evidence; signed Stripe, seller/staff refund authorities, bounded buyer/seller/staff projections and aggregate jobs | Dedicated audit pins 26 semantic application surfaces and selects policyless ENABLE then FORCE with zero direct table authority. The compatible production stack now covers launch-safe full-refund semantics, canonical dispute consumers, refund-only export projections, UTC generation-fenced claims, restart-safe blocked-checkout handoff, atomic record/finalize plus participant delivery, two source-bound signed refund/dispute operations, evidence-bound ambiguous provider reconciliation, database-derived first-write recovery if the original seller becomes banned/deleted, and atomic staff Case finalization with durable buyer/seller Notification plus email-outbox reservation. Recovery reuses exact source-validating functions and adds no caller-controlled target or generic runtime operation. Exact main `8f4cf2df34a9f700adebc910107ac2dbb878054a`, CI `32792800761`, inspection `32793276224` and guarded run `32793394895` applied all five migrations and proved RLS remains off with predecessor CRUD retained. Exact main `5d3b402317084d9d2af6b8bdf52300a800eda0d8`, CI `32795444295`, and its actual pooled-runtime postflight accepted the prepared catalog and boundaries without mutation. Exact main `2820986538c0d64f035defce052ba4ad0de1b3fb` was the compatibility predecessor deployment. The 2026-08-25 re-audit found the blocked-checkout refund used the `NEW_ORDER` preference and omitted refund email. Exact main `a6593516be9fd5531e867aea43b4bbf6319f3094`, CI `32900648444` and guarded run `32902265239` applied its byte-pinned compatibility correction and proved `delivery-compatible`, `OrderPaymentEvent` RLS off, predecessor CRUD retained and Notification FORCE/private-core authority unchanged. The same source is deployed as READY production deployment `dpl_JCmwmKQVwTnvMB2nk7XwYFvQR5xA`; its actual pooled-runtime read-only postflight passed the 14-function catalog and predecessor/private boundaries with retained evidence SHA-256 `5da86ae1aaf0d6ab2a327173cc13e0bf6d8cda3e2bfd9cd5563baab47dc0249e`. The provider proof is not run. A separate four-command live-proof operator is staged: it creates the Session through the authenticated route, serves only loopback Stripe Embedded Checkout, requires genuine signed completion/refund delivery, prevents paid abort, proves zero wrong `NEW_ORDER` delivery and cleans all application fixtures while retaining two processed webhook leases. Neither staging nor its local tests are production proof. Remaining gates are the provider proof/retirement sequence, all four authority-family live proofs, append-only/taxonomy/currency/source invariants, actor-safe projections/aggregates, predecessor drain and separate ENABLE/FORCE releases. Do not bundle quote, Order or OrderItem activation. See `docs/order-payment-event-pre-rls-audit.md`, `docs/order-payment-event-compatible-production-preparation.md`, `docs/order-payment-event-refund-contract.md`, `docs/order-payment-event-dispute-state.md`, `docs/order-payment-event-account-export.md`, `docs/order-payment-event-refund-claim-generation.md`, `docs/order-payment-event-refund-record-authority.md`, `docs/order-payment-event-blocked-checkout-refund-delivery.md`, `docs/order-payment-event-case-refund-delivery.md`, `docs/order-payment-event-signed-authority-design.md` and `docs/order-payment-event-refund-reconciliation.md` |
