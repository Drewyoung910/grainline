...
| `OrderPaymentEvent` | `BLOCKED_DESIGN` | Order, payment and shipping | Append-only payment/refund/dispute service evidence; signed Stripe, seller/staff refund authorities, bounded buyer/seller/staff projections and aggregate jobs | Dedicated audit is complete, pins 26 semantic application surfaces and selects policyless ENABLE then FORCE with zero direct table authority. Before compatible design, close raw account-export metadata, refund-claim ABA, latest-dispute consistency, append-only/taxonomy/currency/event-time invariants, and launch partial-refund semantics; then run a fresh aggregate-only production inspection. Do not bundle quote, Order or OrderItem activation. See `docs/order-payment-event-pre-rls-audit.md` |
