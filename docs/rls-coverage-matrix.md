...
| `Case` | `BLOCKED_DESIGN` | Case and case message | Dispute narrative, status and refund identifiers; buyer, seller, staff, cron and Stripe | Phase 4 database preparation, compatible application, invariant enforcement and the four-function compatible read-mode convergence are live and postflighted in production with RLS still off, FORCE off and zero policies. The 80-reference baseline resolves to 79 machine-checked conversions plus one retired unused helper, zero ordinary direct access and 27 purpose-bound operations. Policyless ENABLE and FORCE remain separate later gates |
...
| `CaseMessage` | `BLOCKED_DESIGN` | Case and case message | Private dispute discussion; buyer, seller and staff | Phase 3 proved durable source-derived author kind, parent authority, bounded keyset history and locked timestamp/state transitions. Phase 4 database preparation, compatible application, invariant enforcement and compatible read-mode convergence are live and postflighted with RLS off and zero policies. Activation remains a separate later release |
