...
| `DirectUpload` | `PLANNED_RLS` | Direct upload | User-owned upload claim state with cleanup jobs; the compatible private-object schema can also retain non-public Case/Message keys | Follow `docs/direct-upload-rls-audit.md`: deny direct runtime table access, use fixed record/verify/reference/release/cleanup/export operations plus a shared-public/exclusive-private reference ledger, and complete the separate CM-A21 rollout before private-object production promotion |
