...
snapshot with completed production RLS acceptance. SellerPayoutEvent closed its distinct actual
...
`OrderRefundReconciliation` is the thirteenth production RLS table, but remains
`RLS_LIVE_FORCE_PENDING_POSTFLIGHT` until its distinct actual pooled-runtime
proof is retained. Every other row is **not active RLS** and remains work to
