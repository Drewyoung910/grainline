...
| `OrderPaymentEvent` | `BLOCKED_DESIGN` | Order, payment and shipping | Append-only payment/refund/dispute service evidence; signed Stripe, seller/staff refund authorities, bounded buyer/seller/staff projections and aggregate jobs | Dedicated audit is complete, pins 26 semantic application surfaces and selects policyless ENABLE then FORCE with zero direct table authority. Prepared application corrections establish launch-safe seller refund semantics, canonical latest-per-dispute consumers, distinct refund-only account-export projections, a database-derived generation claim, restart-safe blocked-checkout handoff and atomic fixed record/finalize authority for seller/blocked-checkout full refunds. A stacked crash-safety refinement commits each source-validated participant Notification and deterministic seller-refund EmailOutbox reservation with the fixed finalizer so provider retries cannot lose delivery. The next compatible candidate converts signed platform refund/dispute writes to two source-bound fixed operations, adds typed provider time, serializes charge/Order/dispute transitions and fails closed on conflicting same-second state while retaining reconciliation evidence. Its byte seal and disposable PostgreSQL proof pass, but none of these stacked candidates is merged, deployed, production-applied or activation evidence. Remaining gates include evidence-based ambiguous-claim reconciliation, staff families, append-only/taxonomy/currency/source invariants, actor-safe projections/aggregates, fresh aggregate-only production inspection, converted-deployment signed provider/retry proof, predecessor drain and separate ENABLE/FORCE releases. Do not bundle quote, Order or OrderItem activation. See `docs/order-payment-event-pre-rls-audit.md`, `docs/order-payment-event-refund-contract.md`, `docs/order-payment-event-dispute-state.md`, `docs/order-payment-event-account-export.md`, `docs/order-payment-event-refund-claim-generation.md`, `docs/order-payment-event-refund-record-authority.md` and `docs/order-payment-event-signed-authority-design.md` |
