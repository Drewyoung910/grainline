...
| `Order` | `PLANNED_RLS` | Order, payment and shipping | Buyer PII, addresses, provider IDs, fulfillment and refunds; buyer, durable Order seller, staff, Stripe, Shippo and jobs | Fresh audit pins 41 direct source files and selects policyless ENABLE then FORCE with zero direct runtime/PUBLIC authority. The isolated compatibility candidate now derives converted seller authority from `Order.sellerProfileId` and renders retained item facts through a strict checkout snapshot reader; mixed actor projections, raw shipping-quote export, remaining aggregates and write/maintenance families still require conversion. Convert `Order` first, then `OrderItem`, then `OrderShippingRateQuote` as separate releases. See `docs/order-core-pre-rls-audit.md` and `docs/order-core-history-compatibility.md` |
