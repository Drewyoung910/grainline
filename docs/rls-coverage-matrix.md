...
| `Notification` | `PLANNED_RLS` | Bucket B Notification | Direct user-owned reads and mark-read updates with cross-user and system creation | Eight emission paths have family validation drafts; narrow the remaining 46, resolve legacy cleanup, and choose the recipient hot-read design before staging |
