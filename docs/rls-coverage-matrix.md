...
| `Order` | `PLANNED_RLS` | Order, payment and shipping | Buyer PII, addresses, provider IDs, fulfillment and refunds; buyer, durable Order seller, staff, Stripe, Shippo and jobs | The current inventory is 16 direct Order source files and 4 direct OrderItem source files; the target is policyless ENABLE then FORCE with zero direct runtime/PUBLIC authority. Historical-authority compatibility and fixed buyer/seller list, bounded participant-summary and bidirectional cursor pages, corrected v3 participant detail, paid-only checkout receipt, account-export, eligibility, public-aggregate, seller-private analytics and Guild/service Order-facts projections now have isolated candidates with disposable PostgreSQL proof. The detail product audit removes dead counterparty messaging actions, enforces active actors in PostgreSQL, suppresses purged notes and stale label material, derives actor-specific Listing links, and restores the complete allowlisted historical snapshot after catching an over-narrowed v2 predecessor. The checkout audit fixes mutable receipt identity, a no-wait webhook retry and duplicate receipt rendering while preserving Stripe as the only Order writer. The fulfillment audit moves pickup completion and the Case-window timestamp from seller assertion to buyer confirmation, rejects unpaid/disputed receipt transitions and removes dead transition vocabulary. Its compatible fixed-authority checkpoint replaces the two direct route writers with source-audited seller fulfillment, buyer receipt and seller-note operations, and co-commits source Notifications plus deterministic email-outbox reservations; it remains unapplied. The label product audit separates ambiguous provider claims from purchased labels, freezes new checkout package facts, binds provider mode/rate/money, restores shipped Notification/email parity, removes raw label URLs from the new seller projection and adds a private exact-claim reconciliation path. Runtime cannot clear an ambiguous fence; the bounded owner operator records exact success or releases exact provider error only. Provider absence is diagnostic and stays fenced, and a transaction-ID hint cannot bypass exhaustive same-rate uniqueness proof. Its compatible candidate and operator are locally proven but unapplied; aggregate duplicate/provider-package counts plus compatible deploy/drain and predecessor-projection grant retirement remain gates. The seller-analytics audit corrects 24-hour cart-abandonment meaning, temporal purchase evidence, deterministic recent-item selection and in-database repeat-buyer aggregation. The Guild audit additionally prevents mutable Listing ownership from rewriting historical sales and shipping qualification. A dormant staff queue/detail candidate requires a separate `grainline_staff_read_runtime` session role and live staff row; it grants neither PUBLIC nor ordinary-runtime execution pending credential isolation. Staff-role provisioning, SellerMetrics/maintenance-write conversion, remaining write-family conversion, production inspection and final seller-key convergence remain required. The first authenticated route smoke is explicitly unaccepted: its quantity-two checkout and signed-expiry phase passed, then an invalid same-address Shippo fixture and a cleanup timestamp-without-time-zone round-trip defect failed closed. Exact main `9219201e` / CI `33612678508` completed the one accepted legacy cleanup recovery with zero persistent mutable residue; sanitized evidence SHA-256 `accb96c3c59c9cd1e9d0df8f12e6ef3161cf46f684ac5a4d422f07a97ead7239`. A fresh full smoke remains the gate; see `docs/order-authenticated-route-smoke-plan-20260902.md`. Convert `Order` first, then `OrderItem`, then `OrderShippingRateQuote` as separate releases. See `docs/order-core-pre-rls-audit.md`, `docs/order-label-product-authority-audit.md`, `docs/order-fulfillment-receipt-product-audit.md`, `docs/order-fulfillment-authority.md`, `docs/order-core-history-compatibility.md`, `docs/order-participant-list-authority.md`, `docs/order-participant-summary-authority.md`, `docs/order-participant-cursor-authority.md`, `docs/order-participant-detail-authority.md`, `docs/order-participant-detail-projection.md`, `docs/order-checkout-receipt-authority.md`, `docs/order-staff-read-authority.md`, `docs/order-participant-export-authority.md`, `docs/order-eligibility-authority.md`, `docs/order-public-aggregate-authority.md`, `docs/order-seller-analytics-authority.md` and `docs/order-seller-metrics-authority.md` |
