...
| `SellerPayoutEvent` | `COMPATIBLE_PREPARATION_LIVE` | Order, payment and shipping | Retained payout-failure projection; seller, separately signed Stripe service and future audited staff support | Domain audit, same-commit zero-row/zero-anomaly inspection and compatible production preparation are accepted. Three fixed runtime functions are live while RLS remains off and predecessor CRUD remains retained. PR #226 merged the three-consumer fixed-operation app conversion at exact main `99591a8f93c45f9324fb834fcbc1ea525867ace8`, with strict payout-notification retry semantics and exact-main CI `31925636570`; it is not deployed. Next is exact-source deployment, the linked-seller signed production proof, predecessor drain and separate policyless ENABLE/FORCE releases. See `docs/seller-payout-event-compatible-authority-release.md`, `docs/seller-payout-event-compatible-app-conversion.md` and `docs/seller-payout-event-linked-production-proof.md` |
...
6. Continue the Order/payment/shipping program: StripeWebhookEvent policyless
   ...
   review, merge and deploy the converted app with predecessor CRUD retained
   before the linked-seller proof and drain. See
