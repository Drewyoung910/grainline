...
| `Notification` | `PLANNED_RLS` | Bucket B Notification | Direct user-owned reads and mark-read updates with cross-user and system creation | Isolated metadata/service drafts exist; narrow the 46 source-less create paths, resolve legacy cleanup, and choose the recipient hot-read design before staging |
