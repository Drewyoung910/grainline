...
| `CheckoutStockReservation` | `COMPATIBLE_CANDIDATE` | Order, payment and shipping | Reservation payload and buyer or seller identifiers; checkout, Stripe and expiry repair | Zero direct reservation delegates remain under `src`. The byte-pinned compatible migration and 15-operation reservation catalog are packaged and CI-isolated behind the separate StripeWebhookEvent FORCE proof; immutable source-object binding uses an atomic bound-begin overload with a private binder. Policyless activation and later posture-only FORCE now have disposable PostgreSQL activation/rollback/tamper proofs, dynamic global grant-audit dispositions, strict actual pooled-runtime read-only postflights, and read-only byte-pinned candidate builders that cannot create Prisma migrations. The exact findings and gates are retained in `docs/checkout-stock-reservation-rls-audit.md`, `docs/checkout-stock-reservation-activation-plan.md` and `docs/checkout-stock-reservation-force-plan.md`. Production is unchanged and all production runners remain unwired. Next: merge and release compatible preparation, deploy fixed operations, prove predecessor drain, inspect legacy rows, separately promote Phase A, then separately promote FORCE |
