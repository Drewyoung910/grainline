...
| `CheckoutStockReservation` | `BLOCKED_DESIGN` | Order, payment and shipping | Reservation payload and buyer or seller identifiers; checkout, Stripe and expiry repair | `docs/checkout-stock-reservation-rls-audit.md` records the isolated app conversion with zero direct reservation delegates and the 15-operation authority draft's disposable PostgreSQL proof. Immutable Stripe source-object binding now uses an atomic bound-begin overload with a private binder, and active event rows are locked through signed transitions. Production is unchanged. Next finish semantic-call regression coverage and migration packaging before compatible preparation |
