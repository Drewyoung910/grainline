...
`OrderPaymentEvent` Phase A is now live pending its distinct pooled-runtime
postflight. Every remaining row is **not active RLS** and remains work to
...
> 16-runtime / 13-private function partition. The distinct actual pooled
> `grainline_app_runtime` postflight is the current gate; FORCE remains a
> separate release afterward. `Order`, `OrderItem` and
> `OrderShippingRateQuote` remain out of scope.
>
