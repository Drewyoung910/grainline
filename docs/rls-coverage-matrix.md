...
| `Notification` | `PLANNED_RLS` | Bucket B Notification | Direct user-owned reads and mark-read updates with cross-user and system creation | All 54 emission paths have source-derived payload/family validation drafts and the shared block-race lock protocol is statically guarded; provisioning/audit now preserve exact `SELECT` + `UPDATE(read)`, initial `NO FORCE`, two policies, and the 25-function public/runtime ACL split. The 2026-07-22 synthetic provider result rejects the interactive wrapper and selects one-statement recipient RPCs directionally but failed the generic gate. Expand PostgreSQL fixtures beyond the currently exercised social/follow family, inspect legacy aggregates, keep any purge atomic with activation, finish SQL/authority review, and pass a fresh candidate-aligned real-table/route gate before staging |
