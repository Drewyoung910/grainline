...
| `Case` | `RLS_LIVE_FORCE` | Case and case message | Dispute narrative, status and refund identifiers; buyer, seller, staff, cron and Stripe | Policyless ENABLE plus FORCE, zero policies, zero direct runtime table/column authority and the 27-operation fixed authority are live. Guarded migration run `30953378226` and the exact pooled-runtime FORCE postflight passed from main `9e5d87f4c5b4a529bc84c6c2cf077778fe553186`; retain `docs/case-force-production-release.md` |
...
| `CaseMessage` | `RLS_LIVE_FORCE` | Case and case message | Private dispute discussion; buyer, seller and staff | Policyless ENABLE plus FORCE and zero direct runtime table/column authority are live with parent-bound fixed reads/writes, invariant triggers and pooled-runtime denial proof in run `30953378226` |
| `CaseMessageAttachment` | `RLS_LIVE_FORCE` | Case and case message | Private dispute image evidence; inherits exact parent Case visibility | Policyless ENABLE plus FORCE and zero direct runtime table/column authority are live and accepted with the Case family. Case evidence stays disabled; signed-read promotion and cleanup scheduling remain separate |
...
5. Case-family policyless ENABLE plus FORCE and the exact pooled-runtime
   postflight are complete. Case evidence enablement, cleanup scheduling and
   provider/token changes remain outside that database boundary.
