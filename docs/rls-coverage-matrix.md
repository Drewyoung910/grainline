...
| `Case` | `BLOCKED_DESIGN` | Case and case message | Dispute narrative, status and refund identifiers; buyer, seller, staff, cron and Stripe | Phase 4 database preparation is complete in production with RLS still off. The compatible application package resolves the 80-reference baseline to 79 machine-checked conversions plus one retired unused helper, zero ordinary direct access and 27 purpose-bound operations. Promote the compatible app and verify it before the separately reviewed invariant/read-mode/ENABLE/FORCE releases |
...
| `CaseMessage` | `BLOCKED_DESIGN` | Case and case message | Private dispute discussion; buyer, seller and staff | Phase 3 proved durable source-derived author kind, parent authority, bounded keyset history and locked timestamp/state transitions. Phase 4 database preparation is live with RLS off, and the compatible application package removes all ordinary direct Case-family access; verify that deployment before separate activation |
