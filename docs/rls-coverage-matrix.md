...
| `CheckoutStockReservation` | `COMPATIBLE_CANDIDATE` | Order, payment and shipping | Reservation payload and buyer or seller identifiers; checkout, Stripe and expiry repair | Zero direct reservation delegates remain under `src`. The byte-pinned compatible migration and 15-operation reservation catalog are packaged and CI-isolated behind the separate StripeWebhookEvent FORCE proof; immutable source-object binding uses an atomic bound-begin overload with a private binder. The policyless activation draft now has disposable PostgreSQL activation/rollback and tamper proof, dynamic global grant-audit disposition, a strict actual pooled-runtime read-only postflight, and a read-only byte-pinned candidate builder that cannot create a Prisma migration. The exact findings and gates are retained in `docs/checkout-stock-reservation-rls-audit.md` and `docs/checkout-stock-reservation-activation-plan.md`. Production is unchanged and both production runners remain unwired. Next: merge and release compatible preparation, deploy fixed operations, prove predecessor drain, inspect legacy rows, then separately promote Phase A |
