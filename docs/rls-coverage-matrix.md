...
| `Case` | `BLOCKED_DESIGN` | Case and case message | Dispute narrative, status and refund identifiers; buyer, seller, staff, cron and Stripe | Phase 4 database preparation and compatible application are live in production with RLS still off. The 80-reference baseline resolves to 79 machine-checked conversions plus one retired unused helper, zero ordinary direct access and 27 purpose-bound operations. The invariant-only migration is the active release; read-mode, policyless ENABLE and FORCE remain separate later gates |
...
| `CaseMessage` | `BLOCKED_DESIGN` | Case and case message | Private dispute discussion; buyer, seller and staff | Phase 3 proved durable source-derived author kind, parent authority, bounded keyset history and locked timestamp/state transitions. Phase 4 database preparation and compatible application are live with RLS off and zero ordinary direct Case-family access. Promote and postflight the invariant-only migration before separate read-mode and activation releases |
