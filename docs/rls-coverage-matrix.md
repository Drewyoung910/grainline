...
> floors are 28 Order files and 4 OrderItem files. The release-time
...
> Order read/aggregate cohorts are prepared through
> `20260901070000_prepare_order_seller_metrics_authority`. None is production
...
| `Order` | `PLANNED_RLS` | Order, payment and shipping | Buyer PII, addresses, provider IDs, fulfillment and refunds; buyer, durable Order seller, staff, Stripe, Shippo and jobs | The current inventory is 28 direct Order source files and 4 direct OrderItem source files; the target is policyless ENABLE then FORCE with zero direct runtime/PUBLIC authority. Historical-authority compatibility and fixed buyer/seller list, detail, account-export, eligibility, public-aggregate, seller-private analytics and Guild/service Order-facts projections now have isolated candidates with disposable PostgreSQL proof. The seller-analytics audit corrects 24-hour cart-abandonment meaning, temporal purchase evidence, deterministic recent-item selection and in-database repeat-buyer aggregation. The Guild audit additionally prevents mutable Listing ownership from rewriting historical sales and shipping qualification. A dormant staff queue/detail candidate requires a separate `grainline_staff_read_runtime` session role and live staff row; it grants neither PUBLIC nor ordinary-runtime execution pending credential isolation. Participant page conversion, staff-role provisioning, SellerMetrics/maintenance-write conversion, production inspection and final seller-key convergence remain required. Convert `Order` first, then `OrderItem`, then `OrderShippingRateQuote` as separate releases. See `docs/order-core-pre-rls-audit.md`, `docs/order-core-history-compatibility.md`, `docs/order-participant-list-authority.md`, `docs/order-participant-detail-authority.md`, `docs/order-staff-read-authority.md`, `docs/order-participant-export-authority.md`, `docs/order-eligibility-authority.md`, `docs/order-public-aggregate-authority.md`, `docs/order-seller-analytics-authority.md` and `docs/order-seller-metrics-authority.md` |
