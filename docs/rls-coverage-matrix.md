...
| `SellerDeauthorizationApplication` | `COMPATIBLE_CANDIDATE` | Order and signed Stripe lifecycle | Immutable signed deauthorization replay evidence; fixed Stripe service operation only | The isolated zero-direct prefix creates this ledger with policyless ENABLE plus FORCE, zero runtime/PUBLIC table authority, and an immutable UPDATE/DELETE trigger. The application operation derives seller and Order effects from the active signed event generation. Disposable engine tests cover replay, rollback, forged generations, direct denial, and complete timestamp/event witnesses. PostgreSQL 16 CI, compatible production preparation, and the distinct pooled-runtime postflight remain required |
