...
| `Order` | `PLANNED_RLS` | Order, payment and shipping | Buyer PII, addresses, provider IDs, fulfillment and refunds; buyer, durable Order seller, staff, Stripe, Shippo and jobs | The current inventory is 22 direct Order source files and 4 direct OrderItem source files; the target is policyless ENABLE then FORCE with zero direct runtime/PUBLIC authority. Historical-authority compatibility and fixed buyer/seller list, bounded participant-summary and bidirectional cursor pages, corrected v2 participant detail, account-export, eligibility, public-aggregate, seller-private analytics and Guild/service Order-facts projections now have isolated candidates with disposable PostgreSQL proof. The detail product audit removes dead counterparty messaging actions, enforces active actors in PostgreSQL, suppresses purged notes and stale label material, and narrows historical snapshots to receipt fields. The seller-analytics audit corrects 24-hour cart-abandonment meaning, temporal purchase evidence, deterministic recent-item selection and in-database repeat-buyer aggregation. The Guild audit additionally prevents mutable Listing ownership from rewriting historical sales and shipping qualification. A dormant staff queue/detail candidate requires a separate `grainline_staff_read_runtime` session role and live staff row; it grants neither PUBLIC nor ordinary-runtime execution pending credential isolation. Staff-role provisioning, SellerMetrics/maintenance-write conversion, checkout-success and write-family conversion, production inspection and final seller-key convergence remain required. Convert `Order` first, then `OrderItem`, then `OrderShippingRateQuote` as separate releases. See `docs/order-core-pre-rls-audit.md`, `docs/order-core-history-compatibility.md`, `docs/order-participant-list-authority.md`, `docs/order-participant-summary-authority.md`, `docs/order-participant-cursor-authority.md`, `docs/order-participant-detail-authority.md`, `docs/order-participant-detail-projection.md`, `docs/order-staff-read-authority.md`, `docs/order-participant-export-authority.md`, `docs/order-eligibility-authority.md`, `docs/order-public-aggregate-authority.md`, `docs/order-seller-analytics-authority.md` and `docs/order-seller-metrics-authority.md` |
