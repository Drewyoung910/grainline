...
| `CheckoutStockReservation` | `COMPATIBLE_PREPARATION_LIVE` | Order, payment and shipping | Reservation payload and buyer or seller identifiers; checkout, Stripe and expiry repair | The compatible authority and source-consistency migrations are live; exact run `31814032227` applied only the additive successor and passed migration status, global grant/RLS audit and exact-scope proof. Both creation paths use one reviewed source-locking PostgreSQL statement. The separate actual pooled-runtime postflight passed from exact main `ac4c9d2139f5294c5e91edd24acb3dbe71b4976c` with the exact 18-runtime/7-private catalog, engine read-only fence and `productionChangedByPostflight=false`. The compatible application is live at exact main `84a58f0fc818b502564ef6bcd974ff4af3cc4395` / Vercel `dpl_AGN7CU9du5Ln1EsUxHqJUopdDEsw`; alias, health, route fencing and authenticated checkout smoke passed. The smoke ran from exact main `e9d343b6f316ceb1c75553aec77e9f310a12d802`, CI `31829740992`, with sanitized evidence SHA-256 `86b37f18cae8fadb8a126b548455201a7816c74f00731d13fa8a6bf2de8602db`. The exact-ID predecessor drain finalized from main `4ff40f22c70072406168c378cdb13860f9de317b`, CI `31858295911`; evidence SHA-256 `5f3b63675bdc84749b5f8fef25086bc42a5dddba5e87f5a46fa7bf6015322141` proves zero shared-credential predecessors and preserved aliases/health. RLS remains off and direct grants remain temporarily compatible. The policyless activation scaffold includes a read-only byte-pinned candidate builder, disposable PostgreSQL activation/rollback/tamper proof, global grant-audit disposition and strict pooled-runtime postflight; it cannot create a migration or change a database. It verifies the live 18-runtime/7-private predecessor, then retires the two unused legacy creation grants for an activated 16-runtime/9-private partition. Next: finish the isolated proof, then separately stage ENABLE/grant revocation and pooled-runtime proof before FORCE. Exact bytes and evidence remain in `docs/checkout-stock-reservation-source-consistency-release.md`, `docs/checkout-stock-reservation-production-smoke.md`, `docs/checkout-stock-reservation-predecessor-drain.md`, and `docs/checkout-stock-reservation-activation-plan.md` |
