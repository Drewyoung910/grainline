...
| `CaseMessage` | `BLOCKED_DESIGN` | Case and case message | Private dispute discussion; buyer, seller and staff | Phase 3 proved durable source-derived author kind, parent authority, bounded keyset history and locked timestamp/state transitions. Phase 4 database preparation, compatible application and invariant enforcement are live and postflighted with RLS off and zero ordinary direct Case-family access. Promote read-mode and activation only as later separate releases |
