...
| `Order` | `PLANNED_RLS` | Order, payment and shipping | Buyer PII, addresses, provider IDs, fulfillment and refunds; buyer, durable Order seller, staff, Stripe, Shippo and jobs | The current isolated candidate inventory is 1 direct Order source file and 1 direct OrderItem source file—the Stripe webhook in both cases; direct quote access is zero. The target is policyless ENABLE then FORCE with zero direct runtime/PUBLIC authority. Historical-authority compatibility and fixed participant, export, eligibility, aggregate, analytics, metrics, fulfillment, refund, label, staff and lifecycle operations have disposable PostgreSQL proof. The checkout, fulfillment, refund, label, staff-mutation, seller-ban, staff-read and account-deletion audits corrected concrete product and concurrency defects rather than treating predecessor logic as automatically sound. Staff screens use a separately authenticated bounded `grainline_staff_read_runtime` candidate. Account deletion now has an actor-bound fixed blocker/scrub candidate using durable seller identity, signed totals, database time and an after-lock recheck. Neither candidate is deployable until its exact database role/functions/grants are provisioned and proved first. The listing-page review hint now reuses the fixed actor-bound review-eligibility operation, leaving Stripe checkout creation, idempotency, post-payment side effects, blocked-refund review transitions and seller deauthorization as the final Order/OrderItem conversion family. Staff-role provisioning, Stripe conversion, migration-tree integration, production inspection, compatible deployment/smoke/drain and final seller-key convergence remain required. The first authenticated route smoke is explicitly unaccepted: its quantity-two checkout and signed-expiry phase passed, then an invalid same-address Shippo fixture and a cleanup timestamp-without-time-zone round-trip defect failed closed. Exact main `9219201e` / CI `33612678508` completed the one accepted legacy cleanup recovery with zero persistent mutable residue; sanitized evidence SHA-256 `accb96c3c59c9cd1e9d0df8f12e6ef3161cf46f684ac5a4d422f07a97ead7239`. A fresh full smoke remains the gate; see `docs/order-authenticated-route-smoke-plan-20260902.md`. Convert `Order` first, then `OrderItem`, then `OrderShippingRateQuote` as separate releases. See `docs/order-core-pre-rls-audit.md`, `docs/order-account-deletion-authority.md`, `docs/order-label-product-authority-audit.md`, `docs/order-fulfillment-receipt-product-audit.md`, `docs/order-fulfillment-authority.md`, `docs/order-core-history-compatibility.md`, `docs/order-participant-list-authority.md`, `docs/order-participant-summary-authority.md`, `docs/order-participant-cursor-authority.md`, `docs/order-participant-detail-authority.md`, `docs/order-participant-detail-projection.md`, `docs/order-checkout-receipt-authority.md`, `docs/order-staff-read-authority.md`, `docs/order-participant-export-authority.md`, `docs/order-eligibility-authority.md`, `docs/order-public-aggregate-authority.md`, `docs/order-seller-analytics-authority.md` and `docs/order-seller-metrics-authority.md` |
