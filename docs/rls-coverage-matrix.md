...
| `OrderPaymentEvent` | `BLOCKED_DESIGN` | Order, payment and shipping | Append-only payment/refund/dispute service evidence; signed Stripe, seller/staff refund authorities, bounded buyer/seller/staff projections and aggregate jobs | Dedicated audit is complete, pins 26 semantic application surfaces and selects policyless ENABLE then FORCE with zero direct table authority. Prepared application corrections establish launch-safe seller refund semantics, canonical latest-per-dispute consumers, distinct refund-only account-export projections and an isolated database-derived generation claim for seller/blocked-checkout full refunds; none is live from this branch. The claim preparation closes the compatible ABA path but final atomic provider record/finalize and evidence-based reconciliation remain gates alongside typed dispute ordering/equal-second reconciliation, append-only/taxonomy/currency/source invariants and a fresh aggregate-only production inspection. Do not bundle quote, Order or OrderItem activation. See `docs/order-payment-event-pre-rls-audit.md`, `docs/order-payment-event-refund-contract.md`, `docs/order-payment-event-dispute-state.md`, `docs/order-payment-event-account-export.md` and `docs/order-payment-event-refund-claim-generation.md` |
