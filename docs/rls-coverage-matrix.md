...
| `DirectUpload` | `PLANNED_RLS` | Direct upload | User-owned upload claim state with cleanup jobs; the compatible private-object schema can also retain non-public Case/Message keys | CM-A21 requires a separate cross-product owner/verifier/claim/cleanup authority and legacy rollout before private-object production promotion; do not hide it in Message or Case activation |
