...
| `Order` | `PLANNED_RLS` | Order, payment and shipping | Buyer PII, addresses, provider IDs, fulfillment and refunds; buyer, durable Order seller, staff, Stripe, Shippo and jobs | Fresh audit pins 41 direct source files and selects policyless ENABLE then FORCE with zero direct runtime/PUBLIC authority. Historical-authority compatibility, four fixed buyer/seller count/keyset-list operations and separate fixed buyer/seller detail projections now have isolated candidates with disposable PostgreSQL proof. A dormant staff queue/detail candidate additionally requires a separate `grainline_staff_read_runtime` session role and live staff row; it grants neither PUBLIC nor ordinary-runtime execution pending credential isolation. Detail pages are not converted; staff-role provisioning, export/aggregate/write conversion, raw shipping-quote export, production inspection and final seller-key convergence remain required. Convert `Order` first, then `OrderItem`, then `OrderShippingRateQuote` as separate releases. See `docs/order-core-pre-rls-audit.md`, `docs/order-core-history-compatibility.md`, `docs/order-participant-list-authority.md`, `docs/order-participant-detail-authority.md` and `docs/order-staff-read-authority.md` |
