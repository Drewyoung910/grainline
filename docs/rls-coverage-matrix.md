...
| `StripeWebhookEvent` | `ACTIVATION_CANDIDATE` | Provider event ledgers | Six fixed lease/maintenance functions; direct ordinary source removed | Compatible deployment `dpl_CasoctMLsvfcA1Vj2JJcNUFzXQXP`, exact test-mode 10/1/12 topology, separately signed `payout.failed` delivery/retry and fresh zero-issue aggregate health are accepted. Policyless ENABLE candidate is prepared; Connect v2 delivery classification, predecessor drain, final compatibility proof, refreshed exact-head PostgreSQL proof and separate production activation remain. FORCE and live-mode provider evidence are later releases |
