...
| `StripeWebhookEvent` | `ALTERNATIVE_REVIEW` | Provider event ledgers | Six fixed lease/maintenance functions; direct ordinary source removed | Policyless ENABLE candidate prepared; exact-head PostgreSQL proof and separate production activation still required; FORCE later |
