...
| `Order` | `PLANNED_RLS` | Order, payment and shipping | Buyer PII, addresses, provider IDs, fulfillment and refunds; buyer, durable Order seller, staff, Stripe, Shippo and jobs | The current inventory is 31 direct source files and the target is policyless ENABLE then FORCE with zero direct runtime/PUBLIC authority. Historical-authority compatibility and fixed buyer/seller list, detail, account-export, eligibility and public-aggregate projections now have isolated candidates with disposable PostgreSQL proof. The export candidate removes raw quote/provider retry material and participant provider refund IDs; eligibility preserves the review lock; public aggregates expose no row, participant, address or provider identity and reduce the direct Order/OrderItem floors from 35/9 to 31/6. A dormant staff queue/detail candidate additionally requires a separate `grainline_staff_read_runtime` session role and live staff row; it grants neither PUBLIC nor ordinary-runtime execution pending credential isolation. Page/app conversion, staff-role provisioning, seller-private aggregate/write conversion, production inspection and final seller-key convergence remain required. Convert `Order` first, then `OrderItem`, then `OrderShippingRateQuote` as separate releases. See `docs/order-core-pre-rls-audit.md`, `docs/order-core-history-compatibility.md`, `docs/order-participant-list-authority.md`, `docs/order-participant-detail-authority.md`, `docs/order-staff-read-authority.md`, `docs/order-participant-export-authority.md`, `docs/order-eligibility-authority.md` and `docs/order-public-aggregate-authority.md` |
