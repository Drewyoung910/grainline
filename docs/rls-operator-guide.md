...
| `scripts/order-payment-event-activation-production-postflight.mjs` | Exact-clean-source, pooled-runtime-only Phase-A proof: engine-read-only transaction, restricted role identity, policyless ENABLE/NO FORCE posture, exact 29-function and 25-direct-reference catalog, four direct table denials, two retired-entry-point denials, retained reads and a fixed-writer SQLSTATE `25006` fence. Writes only sanitized mode-`0600` evidence. | Owner-side migration success, FORCE, another table's posture, or application/provider behavior is proven. |
