...
| `scripts/conversation-message-compatibility-production-postflight.mjs` | Preserves the authenticated RLS-off compatibility and initial-activation proofs and adds a separately pinned `--post-force` mode. Activated modes prove pooled-runtime posture, exact migration identity when forced, exact policies/grants/FORCE state, context-empty isolation, direct DML denial, authenticated inbox/thread/list/unread/read behavior, foreign denial, cleanup, and zero notification/email side effects. | Arbitrary runtime code execution cannot choose a custom PostgreSQL setting or invoke granted fixed functions. |
...
| `scripts/order-payment-event-activation-production-postflight.mjs` | Exact-clean-source, pooled-runtime-only Phase-A proof: engine-read-only transaction, restricted role identity, policyless ENABLE/NO FORCE posture, exact 29-function and 25-direct-reference catalog, direct SELECT ACL denial plus three DML read-only fences, two retired-entry-point denials, retained reads and a fixed-writer SQLSTATE `25006` fence. Writes only sanitized mode-`0600` evidence. | Owner-side migration success, FORCE, another table's posture, or application/provider behavior is proven. |
