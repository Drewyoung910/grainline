...
- **Hardened 2026-05-18:** newsletter signup and unsubscribe token generation now NFC-normalize email addresses before lowercasing. The claimed self-service unsubscribe-token replay loop is not valid on current code because suppression is permanent and newsletter signup refuses suppressed addresses, but Unicode email variants were a real normalization edge worth closing. Regression coverage lives in `tests/unsubscribe-token.test.mjs` and `tests/account-privacy-observability.test.mjs`.
