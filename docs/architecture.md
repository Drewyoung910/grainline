...
Write paths must persist only first-party Grainline media URLs, and new user-submitted upload URLs must be scoped to the current uploader's R2 key segment and expected endpoint. Edit paths may preserve existing DB-owned media rows/fields for legacy compatibility, but hidden fields must not let one signed-in user attach another user's public Grainline media URL. Image upload routes validate MIME/size/count rules, strip image metadata where applicable, verify object availability, and clean up failed writes. Direct-to-R2 PDF/video uploads are tracked in `DirectUpload` from presign through verify, claim, and cleanup so abandoned successful uploads can be deleted without bucket listing. After DirectUpload RLS activation, abandoned-object deletion runs outside the application boundary through the protected GitHub cleanup worker: the ordinary runtime has no cleanup function or table authority, while the worker has only fixed lease/complete/fail operations and bucket-scoped delete credentials. Chat/file upload paths have different friction than profile/listing image paths.
