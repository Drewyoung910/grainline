...
-- a proven OrderPaymentEvent FORCE regression. This changes only FORCE back
...
SELECT pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended('grainline.order-payment-event.rls.activation', 0)
);

LOCK TABLE public."OrderPaymentEvent" IN ACCESS EXCLUSIVE MODE;

DO $grainline_order_payment_event_force_rollback_preflight$
DECLARE
...
BEGIN
  ...
  IF accepted_table_count <> 1 THEN
    RAISE EXCEPTION
      'OrderPaymentEvent FORCE rollback predecessor drifted';
  END IF;
END
$grainline_order_payment_event_force_rollback_preflight$;

ALTER TABLE public."OrderPaymentEvent" NO FORCE ROW LEVEL SECURITY;

DO $grainline_order_payment_event_force_rollback_postflight$
DECLARE
  accepted_table_count integer;
BEGIN
  SELECT pg_catalog.count(*)::integer
  ...
  IF accepted_table_count <> 1 THEN
    RAISE EXCEPTION
      'OrderPaymentEvent FORCE rollback did not restore Phase A';
  END IF;
END
$grainline_order_payment_event_force_rollback_postflight$;
