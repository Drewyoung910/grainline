...
-- a proven StripeWebhookEvent FORCE regression. This changes only FORCE back
...
DO $grainline_stripe_webhook_event_force_rollback_preflight$
DECLARE
  accepted_table_count integer;
BEGIN
  IF current_user NOT IN ('neondb_owner', 'ci') THEN
    RAISE EXCEPTION
      'StripeWebhookEvent FORCE rollback requires a reviewed migration owner';
  END IF;
  ...
  IF accepted_table_count <> 1 THEN
    RAISE EXCEPTION
      'StripeWebhookEvent FORCE rollback predecessor drifted';
  END IF;
END
$grainline_stripe_webhook_event_force_rollback_preflight$;

ALTER TABLE public."StripeWebhookEvent" NO FORCE ROW LEVEL SECURITY;

DO $grainline_stripe_webhook_event_force_rollback_postflight$
DECLARE
  accepted_table_count integer;
BEGIN
  SELECT pg_catalog.count(*)::integer
  ...
  IF accepted_table_count <> 1 THEN
    RAISE EXCEPTION
      'StripeWebhookEvent FORCE rollback did not restore Phase A';
  END IF;
END
$grainline_stripe_webhook_event_force_rollback_postflight$;
