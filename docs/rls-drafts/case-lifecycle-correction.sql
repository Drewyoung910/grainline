...
DO $case_lifecycle_before$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_proc AS proc
     WHERE proc.oid = pg_catalog.to_regprocedure('public.grainline_case_open(text,text,text,text)')
       ...
       AND pg_catalog.md5(proc.prosrc) = '8bc6b3e61372fa69a9360d3e8fe5ea8f'
    ...
    RAISE EXCEPTION 'Case lifecycle before authority drifted: grainline_case_open';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_proc AS proc
     WHERE proc.oid = pg_catalog.to_regprocedure('public.grainline_case_escalate(text,text)')
       AND CURRENT_USER <> 'grainline_app_runtime'
       AND proc.proowner = (SELECT oid FROM pg_catalog.pg_roles WHERE rolname = CURRENT_USER)
       AND proc.prokind = 'f' AND proc.prosecdef AND NOT proc.proleakproof
       AND proc.provolatile = 'v' AND proc.proparallel = 'u'
       AND proc.proconfig = ARRAY['search_path=pg_catalog']::text[]
       AND pg_catalog.md5(proc.prosrc) = '7fb704ae47cceb2ac55b60aa61618e91'
       AND pg_catalog.has_function_privilege('grainline_app_runtime', proc.oid, 'EXECUTE')
       AND NOT EXISTS (
         SELECT 1 FROM pg_catalog.aclexplode(COALESCE(proc.proacl,
           pg_catalog.acldefault('f', proc.proowner))) AS acl
    ...
    RAISE EXCEPTION 'Case lifecycle before authority drifted: grainline_case_escalate';
  END IF;
END
$case_lifecycle_before$;
    ...
    IF actor_is_staff THEN
      ...
      IF NOT FOUND
         ...
         OR existing_audit.metadata->>'previousStatus'
              NOT IN ('OPEN', 'IN_DISCUSSION', 'PENDING_CLOSE')
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_proc AS proc
     WHERE proc.oid = pg_catalog.to_regprocedure('public.grainline_case_open(text,text,text,text)')
       ...
       AND proc.provolatile = 'v' AND proc.proparallel = 'u'
       AND proc.proconfig = ARRAY['search_path=pg_catalog']::text[]
       AND pg_catalog.md5(proc.prosrc) = '27fd964619733b6ea6bb5410fdf6a531'
       AND pg_catalog.has_function_privilege('grainline_app_runtime', proc.oid, 'EXECUTE')
       AND NOT EXISTS (
         SELECT 1 FROM pg_catalog.aclexplode(COALESCE(proc.proacl,
           pg_catalog.acldefault('f', proc.proowner))) AS acl
    ...
    RAISE EXCEPTION 'Case lifecycle after authority drifted: grainline_case_open';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_proc AS proc
     WHERE proc.oid = pg_catalog.to_regprocedure('public.grainline_case_escalate(text,text)')
       ...
       AND proc.provolatile = 'v' AND proc.proparallel = 'u'
       AND proc.proconfig = ARRAY['search_path=pg_catalog']::text[]
       AND pg_catalog.md5(proc.prosrc) = '1e7d12bf3e218b995f01e88fcfa71488'
       AND pg_catalog.has_function_privilege('grainline_app_runtime', proc.oid, 'EXECUTE')
       AND NOT EXISTS (
         SELECT 1 FROM pg_catalog.aclexplode(COALESCE(proc.proacl,
           pg_catalog.acldefault('f', proc.proowner))) AS acl
    ...
    RAISE EXCEPTION 'Case lifecycle after authority drifted: grainline_case_escalate';
  END IF;
END
$case_lifecycle_after$;
