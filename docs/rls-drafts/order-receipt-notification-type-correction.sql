-- DRAFT ONLY: notification input validation; not a staged migration or production operator.
...
DO $notification_input_before$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_proc AS proc
     WHERE proc.oid = pg_catalog.to_regprocedure('public.grainline_notification_create_core(text,text,public."NotificationType",text,text,text)')
       ...
       AND pg_catalog.md5(proc.prosrc) = '9b40f25710d5ee3cac009a0caa903a5a'
       AND pg_catalog.has_function_privilege('grainline_app_runtime', proc.oid, 'EXECUTE') = false
       AND NOT EXISTS (
    ...
    RAISE EXCEPTION 'notification input before authority drifted: grainline_notification_create_core';
  END IF;
END
$notification_input_before$;
...
BEGIN
  ...
  IF p_type IS NULL OR (p_source_type = 'blog_comment'
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_proc AS proc
     WHERE proc.oid = pg_catalog.to_regprocedure('public.grainline_notification_create_core(text,text,public."NotificationType",text,text,text)')
       ...
       AND proc.provolatile = 'v' AND proc.proparallel = 'u'
       AND proc.proconfig = ARRAY['search_path=pg_catalog']::text[]
       AND pg_catalog.md5(proc.prosrc) = '71047a1fc02160624162cc47fa981f27'
       AND pg_catalog.has_function_privilege('grainline_app_runtime', proc.oid, 'EXECUTE') = false
       AND NOT EXISTS (
         SELECT 1 FROM pg_catalog.aclexplode(COALESCE(proc.proacl,
           pg_catalog.acldefault('f', proc.proowner))) AS acl
    ...
    RAISE EXCEPTION 'notification input after authority drifted: grainline_notification_create_core';
  END IF;
END
$notification_input_after$;
