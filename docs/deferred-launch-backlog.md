...
| Stripe webhook subscriptions | Launch blocker | Provider evidence for snapshot and Connect v2 thin webhook endpoints, exact event subscriptions, and signing-secret matching. | `npm run audit:stripe-webhooks` passes with live Stripe read-only credentials and a retained sanitized artifact for `/api/stripe/webhook` plus `/api/stripe/webhook/v2` URLs/event families; screenshots or exported provider/deploy evidence show separate `STRIPE_WEBHOOK_SECRET` and `STRIPE_V2_WEBHOOK_SECRET` matching because Stripe does not return endpoint secrets after creation. |
