...
| Cross-domain Case correctness corrections | Launch blocker | Legacy staff labels, UTC money-path clocks, staff-resolution replay eligibility/final stock guard, and the Order-PII prune/chargeback race verified in `docs/verified-cross-domain-pre-rls-findings-20260901.md`. | Additive migrations redefine only the affected effective functions, preserve applied migration bytes, pass non-UTC/replay/concurrency PostgreSQL proofs, and close distinct guarded production postflights. |
| Exact charged Order total and refunded UI state | Conditional blocker | `Order` lacks a signed provider-total witness, and fully refunded unfulfilled Orders can still render preparing/pending copy. | Persist and validate nullable `chargedTotalCents` from signed Checkout Session evidence with legacy classification; make refund/payment presentation suppress impossible active-fulfillment copy without rewriting historical logistics state. Close before core Order Phase A. |
