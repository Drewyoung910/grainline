...
| Buyer shipping quote provider smoke | Launch blocker | Source validation now rejects malformed/duplicate provider IDs, requires normalized city/state/country, preserves pickup fallback and exposes retry, but source tests cannot prove current Shippo account/address acceptance or real no-rate behavior. | With Stripe test mode and reviewed Shippo credentials, one valid shipping destination returns at least one provider-backed rate whose ID can be re-quoted for label purchase; one provider no-rate/error case keeps pickup available and exposes retry without inventing a rate ID. Retain sanitized evidence only. |
| Continuous accepted-table RLS canaries | Launch blocker | Periodic production posture/denial monitoring across every table already accepted as RLS live, rather than relying only on release-time postflights. | A scheduled read-only canary covers the exact accepted table inventory, fails on RLS/FORCE/policy/grant drift, verifies the pooled runtime identity and emits sanitized alert evidence without row data. |
| SellerProfile public-projection regression guard | Conditional blocker | Broad public SellerProfile selection remains possible while public and private operational fields share one base table. | Before SellerProfile RLS/design activation, recursively inventory public readers, require reviewed allowlist projections, and fail CI on new broad public SellerProfile selects/includes. |
| Order support and schema follow-ups | Decision required | Friendly support order numbers, durable checkout-group product semantics, and retirement of unused historical tax-reversal columns. | Decide buyer/support semantics for public order numbers and grouped checkout; implement with collision/migration tests if accepted; prove tax-reversal columns unused before a separate cleanup migration. |
