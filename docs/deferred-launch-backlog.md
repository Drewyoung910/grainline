...
| Stripe webhook subscriptions | Launch blocker | Provider evidence for snapshot and Connect v2 thin webhook endpoints, exact event subscriptions, and signing-secret matching. The 2026-08-08 test-mode read-only artifact failed: classic is missing 11 handled events and has four extras; v2 has three unused `account_person` events. | Correct the exact provider event sets in a separate reviewed provider boundary, then require `npm run audit:stripe-webhooks` to pass with read-only credentials and retain the sanitized artifact for both URLs/event families; screenshots or exported provider/deploy evidence must show separate `STRIPE_WEBHOOK_SECRET` and `STRIPE_V2_WEBHOOK_SECRET` matching because Stripe does not return endpoint secrets after creation. |
