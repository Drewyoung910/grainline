...
| --- | --- | --- | --- |
| RLS staging and first table policy | Conditional blocker | Live Neon-like pooling/context gate, route-level prototype tests, wrapper coverage guard, and first `Notification`/`SavedSearch` policy migrations. | Staging gate evidence passes; first policy migration includes route tests, rollback notes, wrapper coverage, and CI; production RLS remains disabled until those pass. |
...
| Cloudflare R2 posture and smoke | Launch blocker | ListBucket/public bucket posture plus production upload smoke/public-availability proof. | Dashboard/CLI evidence shows no public listing/ListBucket exposure; processed upload and direct upload/verify smoke pass with production credentials. |
