...
- **Edit listing redirect behavior**: `updateListing` redirects based on the post-edit listing status, never blindly to `publicListingPath`. ACTIVE/SOLD/SOLD_OUT redirect to the public listing page; DRAFT/HIDDEN/REJECTED redirect to `/dashboard/listings/[id]/edit?saved=1`; PENDING_REVIEW redirects to `/dashboard/listings/[id]/edit?saved=pending`. The edit page renders a top-of-page banner reading the saved query param. Public listing path 404s for non-public statuses, so a status-aware redirect is required to avoid a "page not found" experience after every draft edit and after every active edit that triggers AI re-review.
