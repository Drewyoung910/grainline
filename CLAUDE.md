...
- **Stripe webhook idempotency order**: reserve the signed outer `event.id` before Workbench thin-event retrieval. Retrieval failures and envelope mismatches should mark that same row failed; unhandled but valid event types should be marked processed. If a signed Stripe snapshot or v2 thin event is already processed, return 200; if it is currently `in_progress` and not stale/reclaimable, return 503 with `Retry-After` so Stripe keeps retrying instead of treating the duplicate delivery as successfully handled.
