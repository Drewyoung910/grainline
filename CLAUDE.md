...
- **Active listing edit photo behavior**: `POST /api/listings/[id]/photos` attaches photos and refreshes the edit page only. It must not run AI review, flip an ACTIVE listing into `PENDING_REVIEW`, redirect the seller to preview, or generate AI alt text by itself. ACTIVE listing review happens when the seller explicitly presses Save; `updateListing` reviews the current text, price, variants, and photo set, backfills missing alt text from that Save-time AI result, and redirects based on the post-save status. Preserve this seller-controlled Save boundary.
...
- **Listing photo creation behavior**: new and custom listing flows share `PhotoManager` and submit `imageUrlsJson` plus `imageAltTextsJson`. Existing-listing photo attachment intentionally stays on `AddPhotosButton` because it goes through the persisted listing photo API and refreshes the edit grid; moderation review for ACTIVE listings remains tied to the seller pressing Save in `updateListing`.
