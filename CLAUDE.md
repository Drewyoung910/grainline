...
- **Public request body bounds behavior**: unauthenticated public form/report endpoints must bound request bodies before JSON parsing or Sentry/report processing. `src/lib/requestBody.ts` owns `readBoundedText()` / `readBoundedJson()` with `Content-Length` pre-check plus streamed byte caps. Current public caps: newsletter 8 KiB, support/data-request 24 KiB, CSP reports 32 KiB. Do not call raw `req.json()` / `request.text()` on those routes.
