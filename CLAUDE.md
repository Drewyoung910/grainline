...
- **Audit-only follow-up queue**: the 2026-05-06 audit-only sweep reopened a small verified backlog in `audit_open_findings.md` after the prior mechanical queue hit zero. All code findings and the Stripe API-version docs drift are fixed; remaining from that sweep is the dedicated Stripe Connect Accounts v2 modernization pass. Treat the audit file as current source of truth before assuming the queue is empty.
