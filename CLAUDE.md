...
- **Admin server-action auth behavior**: admin server actions must re-check `EMPLOYEE | ADMIN` role and suspended/deleted account state inside the action, not rely only on middleware or the admin layout. Middleware still enforces admin role plus signed Admin PIN for admin API calls and admin server-action POSTs, but action-level checks are the defense-in-depth guardrail future refactors must preserve.
