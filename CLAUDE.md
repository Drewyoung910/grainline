...
- **High-friction form styling behavior**: auth entry pages, support/data request forms, buy-now checkout modal, account order cards, admin audit tables, and seller listing/custom-listing forms should use the warm page background where they own the page shell, `card-section`/white card surfaces, explicit neutral borders, rounded-md controls, and `font-display` headings. Do not reintroduce generic `bg-stone-50` page shells or bare `border rounded` controls on these conversion/support surfaces.
