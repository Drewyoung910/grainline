...
- `WREATH_D` (full single path from `gold-laurel-wreath.svg`, viewBox 1200x1200), `BADGE_VIEWBOX`, `STAR_POINTS` (5-point polygon, r_out=203, r_in=81, center 600,579 — derived from `svgpathtools` bbox analysis of star subpath #12, which has bbox 407–793 x 395–763) — module-level constants shared by both icons. No subpath splitting — auto-traced file is not clean vectors. `useId()` for hydration-safe gradient IDs.
- **Guild Member** — `LaurelWreathIcon`: earthy green wreath (`#5B7553 → #3F5D3A → #1F3A1E`, pine/oak tones) + bronze star polygon overlay (`#E8B86D → #B8860B → #8B6914`). Label text color `#15803d` (green-700).
- **Guild Master** — `StarWreathIcon` (replaces `HammerChiselIcon`): gold wreath (`#FFD700 → #D4AF37 → #B8960C`) + diamond star polygon overlay (`#F0F9FF → #E0F2FE → #BAE6FD`). Both polygons same position/size, different gradients. Label text color `#B8960C`.
