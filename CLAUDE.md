...
- `CLAUDE.md` is the active architecture/runbook. Keep current schema, environment, helper ownership, operational rules, and behavior guardrails here.
- Completed audit-pass narratives and old implementation logs live in [CLOSED_AUDIT_HISTORY.md](./CLOSED_AUDIT_HISTORY.md); search that file when you need historical fix context.
- Sections older than 60 days that describe a completed audit pass should move to [CLOSED_AUDIT_HISTORY.md](./CLOSED_AUDIT_HISTORY.md) rather than accumulate inline. Move information, do not delete it.
...
- **Commission helpers**: `commissionState.ts` centralizes commission status transition predicates so PATCH/close/fulfill actions and interest creation share the same OPEN, non-expired, active-buyer guard.
