...
- **Audit-only follow-up queue**: the 2026-05-06 post-fix audit-only sweep reopened a small verified backlog in `audit_open_findings.md` after the prior mechanical queue hit zero. Current count after the Stripe refund and public visibility passes is 4 verified code findings plus 3 verified improvement/architecture items. Treat the audit file as the source of truth before assuming the queue is empty, and do not duplicate its per-finding detail here.
