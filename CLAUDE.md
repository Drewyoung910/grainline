...
- **Address autocomplete behavior**: checkout shipping address, seller ship-from address, and pickup-map search use the shared address autocomplete/dropdown. Keep Nominatim use throttled/debounced and country-scoped to US, keep state-code normalization through `usStates.ts`, and never fall back to county, suburb, neighbourhood, or city district as the city. City extraction intentionally uses official locality fields only (`city -> town -> village -> municipality -> hamlet`), then parses `display_name` as a final fallback while rejecting known sublocality names (`suburb`, `neighbourhood`, `city_district`) and county fragments. If no trustworthy city can be found, leave city blank so the user can correct it; do not write a neighborhood into checkout/shipping city. After a suggestion is selected, the search field clears and the destination address fields show the chosen address. If Nominatim returns no match, the dropdown must show a short "add city or ZIP" hint instead of silently closing.
...
- **Site background behavior**: header, homepage, browse, and listing-detail page backgrounds use solid `bg-[#F7F5F0]` warm cream rather than page-wide amber gradients. Photos, cards, `bg-amber-50` / `bg-amber-50/30` accent sections, and the warm `#EFEAE0` accent (see "Warm color palette behavior") provide the visual rhythm; do not reintroduce full-page or header amber gradients.
- **Warm color palette behavior**: site uses three warm tones in a deliberate hierarchy — body cream `#F7F5F0`, section accent `#EFEAE0` (one shade darker; the `.section-warm` helper class in `globals.css`), and footer forest green `#3F5D3A` (ties into the Guild Member wreath palette so the color is not invented). Use `#EFEAE0` for alternating section panels that should sit a step darker than body (currently: `MakersMapSection` outer container, homepage "Meet a Maker" card). Map widgets inside a `#EFEAE0` panel should set their wrapper to `bg-[#F7F5F0]` so the map area is *lighter* than the surrounding section. The footer is `bg-[#3F5D3A]` with `text-stone-200` body text and `hover:text-white` links; the espresso logo uses `invert opacity-80` to read light on the green. Do not promote `#EFEAE0` to body bg — it is the section accent, not the page bg. Do not invent additional warm tones; pick from these three.
...
- **Upload cleanup behavior**: `/api/upload/image` must delete the processed R2 object if public availability verification fails after `PutObjectCommand`. Failed cleanup attempts should be Sentry-captured with `source: "upload_image_cleanup"`; do not leave written-but-unusable image objects untracked.
...
- **Listing photo creation behavior**: new and custom listing flows share `PhotoManager` and submit `imageUrlsJson` plus `imageAltTextsJson`. Existing-listing photo attachment intentionally stays on `AddPhotosButton` because it goes through the persisted listing photo API and moderation re-review path.
