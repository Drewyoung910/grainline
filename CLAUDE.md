...
- **Audit-only follow-up queue**: the 2026-05-06 post-fix audit-only sweep reopened a small verified backlog in `audit_open_findings.md` after the prior mechanical queue hit zero. Current count is 9 verified code findings plus 3 verified improvement/architecture items; highest priority is the Stripe full-refund transfer-reversal accounting bug. Treat the audit file as the source of truth before assuming the queue is empty, and do not duplicate its per-finding detail here.
...
- **Shipping-label clawback behavior**: label purchase may succeed even when Stripe transfer reversal for label cost fails. Missing transfer IDs and reversal failures must set `Order.reviewNeeded` with a bounded `labelClawbackState.ts` note so the order appears in `/admin/flagged`; Sentry-only evidence is not sufficient.
