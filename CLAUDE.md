...
- **Clerk webhook email behavior**: user webhooks must sync only the address whose ID matches `primary_email_address_id`. If the primary ID/address is missing or empty, log Sentry context and skip the email update instead of falling back to `email_addresses[0]`; Svix verification failures should also be Sentry-captured before returning 400.
