...
- **Authenticated mutation body bounds behavior**: high-cost or sensitive signed-in JSON mutations must also use `readBoundedJson()` before Zod parsing instead of raw `req.json()` / `request.json()`. Current capped clusters: checkout session creation (64 KiB), shipping quotes (32 KiB), direct upload presign/verify (16 KiB), seller broadcasts (32 KiB), admin email (64 KiB), reports/cases/reviews/custom-order/commission creates and status changes (24 KiB, except commission status 8 KiB). Oversized bodies return 413 before downstream Prisma, Stripe, Shippo, R2, email, or notification work. Regression coverage: `tests/authenticated-json-body-bounds.test.mjs`.
...
- **Sentry email telemetry behavior**: public support/data-request routes, email suppression failures, and the central email send helper must not send raw email addresses to Sentry `extra`, `tags`, `contexts`, or user context. Use `hashEmailForTelemetry()` from `src/lib/privacyTelemetry.ts` when correlation is needed, prefer `subjectLength` over raw subject text in central email failure telemetry, and keep regression coverage in `tests/privacy-telemetry.test.mjs` plus `tests/account-privacy-observability.test.mjs`.
