...
- **Address autocomplete behavior**: checkout shipping address, seller ship-from address, and pickup-map search use the shared address autocomplete/dropdown. Keep Nominatim use throttled/debounced and country-scoped to US, keep state-code normalization through `usStates.ts`, and never fall back to county as the city or display county fragments in suggestion labels. City extraction intentionally falls through `city -> town -> village -> municipality -> hamlet -> suburb -> neighbourhood -> city_district -> display_name parsing` because real US Nominatim records often omit `city`; county remains excluded even in fallback parsing. After a suggestion is selected, the search field clears and the destination address fields show the chosen address.
