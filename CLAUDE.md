...
- **Cart size cap behavior**: signed-in cart additions cap each cart item at 99 quantity, each cart at 50 distinct item/variant rows, and each cart at 200 total quantity before creating or incrementing `CartItem` rows. Do not rely only on per-listing quantity caps; a bot can otherwise add 99 of many listings.
