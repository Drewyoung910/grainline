...
- **Public account-state route behavior**: routes kept public for anonymous UX but able to see a Clerk session, such as `/api/me` and `/api/cart`, must enforce banned/deleted users inside the route with `ensureUserByClerkId()` or an equivalent `accountAccessErrorResponse()` path before returning user-specific data.
