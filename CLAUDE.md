...
- The `/sitemap.xml` index route at `src/app/sitemap.xml/route.ts` lists every chunk emitted by `generateSitemaps()`, so robots.txt can keep pointing at the conventional `/sitemap.xml` path without losing chunked listing entries.
...
- **Cron/observability helpers**: `cronMonitor.ts` and `cronMonitorState.ts` provide explicit Sentry check-ins for App Router cron route handlers and classify 5xx cron responses as failed check-ins. Do not rely on `automaticVercelMonitors` for App Router routes.
...
- **UI/runtime helpers**: `apiError.ts`, `messageStreamState.ts`, `money.ts`, `recentlyViewed.ts`, `RecentlyViewedAuthBoundary.tsx`, `reviewPhotoState.ts`, and `stockNotificationState.ts` centralize API error extraction/retry guidance, message-stream terminal status handling, ISO-currency-aware cent formatting, recently-viewed cookie normalization/clearing, auth-boundary privacy cleanup, review-photo caps/dedupe, and stock-notification response state. Client components that care about 429/retry copy should use `readApiErrorMessage()` instead of replacing API responses with generic text; email/order copy should use `formatCurrencyCents()` instead of hand-built `$${amount}` strings.
...
- **Sitemap index behavior**: `/sitemap.xml` is the crawler-facing sitemap index served by `src/app/sitemap.xml/route.ts` and lists every chunk produced by `generateSitemaps()` in `src/app/sitemap.ts` (`/sitemap/0.xml` for static + seller/blog/commission/metro entries, `/sitemap/N.xml` for listing chunks). `robots.txt` advertises `/sitemap.xml`. Chunk-count math lives in `src/lib/sitemapIndex.ts` and is shared with the listing chunk loop; do not duplicate `SITEMAP_CHUNK_SIZE` in `route.ts` and let it drift from `sitemap.ts`.
