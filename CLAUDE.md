...
- **Account/admin/session helpers**: `accountAccessError.ts`, `clerkUserLifecycle.ts`, `clerkSessionSecurity.ts`, `adminEmailRecipient.ts`, `adminPin.ts`, and `requestId.ts` centralize account-state errors, Clerk lifecycle/session invalidation behavior, admin email recipient checks, signed admin PIN cookies, and request correlation IDs. Production admin PIN cookies require `ADMIN_PIN_COOKIE_SECRET`.
