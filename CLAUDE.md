...
- **Cart/checkout/order helpers**: `anonymousCart.ts`, `cartEvents.ts`, `checkoutAmounts.ts`, `checkoutSessionExpiry.ts`, `checkoutStockRestore.ts`, `checkoutSuccessState.ts`, `orderTotals.ts`, `shippingQuoteState.ts`, `stockMutationState.ts`, and `transactionRetry.ts` keep cart merge/count behavior, checkout retry/rollback behavior, amount math, order total display math, stock restoration, and retryable transaction rules testable outside route handlers.
- **Stripe/webhook helpers**: `stripeWebhookState.ts`, `stripeWebhookEventState.ts`, `webhookFailureSpike.ts`, and `webhookFailureSpikeState.ts` own idempotent webhook state, checkout price-drift classification, dispute/refund order updates, failure-spike detection, and retry semantics. Completed checkout email side effects enqueue through `EmailOutbox` with stable dedup keys.
...
- **UI/runtime helpers**: `apiError.ts`, `messageStreamState.ts`, `money.ts`, `recentlyViewed.ts`, `RecentlyViewedAuthBoundary.tsx`, `reviewPhotoState.ts`, and `stockNotificationState.ts` centralize API error extraction/retry guidance, message-stream terminal status handling, ISO-currency-aware cent formatting, strict money-input parsing, recently-viewed cookie normalization/clearing, auth-boundary privacy cleanup, review-photo caps/dedupe, and stock-notification response state. Client components that care about 429/retry copy should use `readApiErrorMessage()` instead of replacing API responses with generic text; email/order copy should use `formatCurrencyCents()` instead of hand-built `$${amount}` strings, and form/API code that converts user-entered dollars to cents should use `parseMoneyInputToCents()` instead of `parseFloat()` / `Number()`.
...
- **Listing/media/content helpers**: `aiReviewSafety.ts`, `blogInput.ts`, `blogVideo.ts`, `formJson.ts`, `listingActionState.ts`, `listingPhotoReview.ts`, `listingSoftDelete.ts`, `messageBodies.ts`, `qualityScoreState.ts`, `responseText.ts`, `searchSuggestionState.ts`, `sellerProfileText.ts`, `uploadKey.ts`, `uploadVerificationToken.ts`, `uploadedFileUrl.ts`, and `clientId.ts` centralize AI-review safety, media input validation, listing form JSON parsing, listing/archive state checks, message body parsing, quality-score penalty terms, search-suggestion query normalization/thresholds, seller-profile text caps, upload ownership/verification, uploaded-file response parsing, and client temporary IDs.
...
- **Audit-only follow-up queue**: the 2026-05-06 audit-only sweep reopened a small verified backlog in `audit_open_findings.md` after the prior mechanical queue hit zero. Four code findings and the Stripe API-version docs drift are fixed; remaining from that sweep are the durable label-clawback retry/reconciliation design and a dedicated Stripe Connect Accounts v2 modernization pass. Treat the audit file as current source of truth before assuming the queue is empty.
...
- **Performance/cache behavior**: listing detail seller rating display reads persisted `SellerRatingSummary` through `getSellerRatingMap()`; do not reintroduce seller-wide `Review` aggregation on listing detail renders. Homepage and browse include auth/block/saved/personalized state, so avoid blanket whole-page caching there unless the personalized pieces are isolated. Raw image tags that remain on high-traffic surfaces should carry intrinsic dimensions to avoid layout shift.
...
- **Required**: `ADMIN_PIN_COOKIE_SECRET`, `UPLOAD_VERIFICATION_SECRET`, `CLERK_WEBHOOK_SECRET`, `RESEND_API_KEY`, `EMAIL_FROM`, `RESEND_WEBHOOK_SECRET`, `UNSUBSCRIBE_SECRET`, `SENTRY_DSN`, and `NEXT_PUBLIC_SENTRY_DSN`.
