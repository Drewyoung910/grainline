...
- **UI/runtime helpers**: `apiError.ts`, `money.ts`, `recentlyViewed.ts`, and `RecentlyViewedAuthBoundary.tsx` centralize API error extraction/retry guidance, ISO-currency-aware cent formatting, recently-viewed cookie normalization/clearing, and auth-boundary privacy cleanup. Client components that care about 429/retry copy should use `readApiErrorMessage()` instead of replacing API responses with generic text; email/order copy should use `formatCurrencyCents()` instead of hand-built `$${amount}` strings.
