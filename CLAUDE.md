...
- `WREATH_D` (full single path from `gold-laurel-wreath.svg`, viewBox 1200x1200), `BADGE_VIEWBOX`, `STAR_POINTS` (oversized 5-point polygon, r_out=140 r_in=55 at center 600,495 — fully covers baked-in star in path) — module-level constants shared by both icons. No subpath splitting — auto-traced file is not clean vectors. `useId()` for hydration-safe gradient IDs.
- **Guild Member** — `LaurelWreathIcon`: smooth green wreath (`#4ADE80 → #22C55E → #166534`) + bronze star polygon overlay (`#E8B86D → #B8860B → #8B6914`). Label text color `#15803d` (green-700).
