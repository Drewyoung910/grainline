...
- **AI alt-text backfill helper**: `src/lib/photoAltTextBackfill.ts` exports `backfillEmptyAltTexts(listingId, altTexts)`. It writes to photos with empty `altText` only — never overwrites seller-provided alt text — and pairs `altTexts[i]` with `photos[i]` in `sortOrder` ascending. Failures are non-fatal and logged in non-production. Every server path that calls `reviewListingWithAI` must call this helper to backfill. Currently wired on `dashboard/listings/new/page.tsx createListing`, `seller/[id]/shop/actions.ts publishListingAction`, `dashboard/listings/[id]/edit/page.tsx updateListing`, and `api/listings/[id]/photos/route.ts` active-listing photo add review. `deletePhotoAction` and `replacePhotoAction` intentionally do not run AI review; Save is the review trigger for ACTIVE listing edits.
- **Active listing photo-add review behavior**: `POST /api/listings/[id]/photos` must not publish new photos onto an ACTIVE listing without review. ACTIVE listing photo uploads first pass the dedicated `listingPhotoAiRatelimit`, then insert the photos, atomically hold the listing in `PENDING_REVIEW`, run `reviewListingWithAI`, call `backfillEmptyAltTexts(listingId, aiResult.altTexts)`, and only restore ACTIVE after an approved/high-confidence result with no flags. The client `AddPhotosButton` redirects the seller to `/listing/[id]?preview=1` when the status is `PENDING_REVIEW`. Do not restore the old "photo add never re-reviews" behavior; it lets sellers attach unreviewed public images after a clean publish.
