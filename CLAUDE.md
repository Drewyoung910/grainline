...
- **Audit-only follow-up queue**: the 2026-05-06 audit-only sweep reopened a small verified backlog in `audit_open_findings.md` after the prior mechanical queue hit zero: five code findings plus two improvement/documentation items. Treat the audit file as current source of truth before assuming the queue is empty.
