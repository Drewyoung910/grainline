...
- **Listing/media/content helpers**: `aiReviewSafety.ts`, `blogInput.ts`, `blogVisibility.ts`, `blogVideo.ts`, `formJson.ts`, `listingActionState.ts`, `listingPhotoReview.ts`, `listingSoftDelete.ts`, `listingVisibility.ts`, `messageBodies.ts`, `qualityScoreState.ts`, `responseText.ts`, `savedListingVisibility.ts`, `searchSuggestionState.ts`, `sellerProfileText.ts`, `sellerVisibility.ts`, `uploadKey.ts`, `uploadVerificationToken.ts`, `uploadedFileUrl.ts`, and `clientId.ts` centralize AI-review safety, media input validation, public blog/listing visibility predicates, listing form JSON parsing, listing/archive state checks, message body parsing, quality-score penalty terms, search-suggestion query normalization/thresholds, seller-profile text caps, active seller visibility, upload ownership/verification, uploaded-file response parsing, and client temporary IDs.
...
- **Audit-only follow-up queue**: the 2026-05-06 post-fix audit-only sweep reopened a small verified backlog in `audit_open_findings.md` after the prior mechanical queue hit zero. Current count after the Stripe refund, public visibility, and observability cleanup passes is 0 verified code findings. The only remaining live-ledger item is the deferred Stripe Connect v2 architecture modernization; treat the audit file as the source of truth before assuming the queue is empty, and do not duplicate its per-finding detail here.
...
- **Active seller follow behavior**: follow GET/POST and account following surfaces use `activeSellerProfileWhere()` so inactive, vacationing, disconnected, banned, or deleted sellers do not look followable. DELETE may still clean up stale follow rows by id.
