...
- `SellerProfile.stripeAccountVersion` and `stripeControllerType` are nullable diagnostics only; `stripeAccountId` and `chargesEnabled` remain the runtime gates. No new env vars are required.
...
- **Stripe account-version behavior**: `SellerProfile.stripeAccountVersion` is a runtime guard, not only diagnostics. Public listing/seller predicates and sellable/order paths must require supported v2 accounts when the field is selected; legacy/unknown connected accounts should not silently stay public or payable after the v2 migration. Connect account-link routes reject existing non-v2 account IDs with a reconnect/support message. Account deletion reads `stripeAccountVersion` and `stripeControllerType`, includes them in Stripe reject telemetry/manual reconciliation notes, and clears both fields during anonymization.
