...
- **Vacation return-date behavior**: seller vacation return dates come from native `YYYY-MM-DD` date inputs, are parsed server-side as noon UTC, and render on public seller profile/shop pages through `LocalDate dateOnly` so the same date is shown consistently without a time component.
