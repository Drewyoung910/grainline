...
DO $grainline_notification_preparation_preflight$
DECLARE
  runtime_role record;
...
BEGIN
  ...
  IF notification_state.relrowsecurity OR notification_state.relforcerowsecurity THEN
    RAISE EXCEPTION 'Notification RLS must be disabled before preparation';
  END IF;
  ...
  IF policy_count <> 0 THEN
    RAISE EXCEPTION 'Notification policies must not exist before preparation';
  END IF;
  ...
  IF candidate_function_count <> 0 THEN
    RAISE EXCEPTION 'Notification preparation RPC names already exist: expected %, got %',
      0, candidate_function_count;
  END IF;
END
$grainline_notification_preparation_preflight$;
...
DO $grainline_notification_preparation_postflight$
DECLARE
  notification_state record;
  policy_count integer;
  candidate_function_count integer;
