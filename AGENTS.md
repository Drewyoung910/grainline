...
Strategic roadmap, recruitment playbooks, referral system phasing, and "things not to do" live in `STRATEGY.md`. AGENTS.md is the codebase contract; STRATEGY.md is what hasn't been built yet and why. Update STRATEGY.md after any session that produces a strategic decision.
...
1. **Single-photo hero** — full-bleed maple-cabinet photograph, overlaid homepage header, "Buy handmade. Buy local. Buy quality." heading, supporting line, and Browse/Map CTAs. Search lives only in the header.
2. **Marketplace statistics** — a semantic four-column `dl` floating exactly halfway across the hero's bottom edge, backed by the five-minute cached `getCachedHomepageStats()` helper.
3. **Find Makers Near You** — `MakersMapSection` with location-aware discovery.
4. **Makers You Follow** — conditional personalized listing/blog row for signed-in users following at least three makers.
5. **Shop by Category** — horizontally scrollable on mobile and a desktop grid.
6. **In the Workshop** — two-maker weekly spotlight using the existing curated/Guild/fallback selection tiers.
7. **New Arrivals** — newest public listings in a horizontal row.
8. **Top Picks** — quality-score-ranked public listings in a horizontal row.
9. **From the Blog** — three recent public posts when available.

The homepage intentionally does not render `RecentlyViewed`, `NewsletterSignup`, or a second search field.
...
### Tooling
- Sentry MCP configured for Codex and VS Code — enables AI-assisted error investigation directly from the Sentry dashboard
...
- **Alternating warm section backgrounds**: "Meet a Maker" and "Stories from the Workshop" `ScrollSection` wrappers get `bg-amber-50/30 rounded-xl px-4 py-6 -mx-4` for subtle warm tint.
...
## Mobile Card Layout + Gradient Fix (2026-04-09)
...
- Before every coding pass, read this `AGENTS.md` and follow the project rules here.
...
- After every audit/fix pass: update `audit_open_findings.md` with status and a short fix note for each closed item, update `AGENTS.md` when architecture/helpers/env/schema behavior changed, run verification, then make a scoped commit before continuing to the next pass.
...
- `audit_open_findings.md` remains the per-finding source of truth. `AGENTS.md` should summarize architectural behavior, reusable helpers, migrations, env vars, and workflow rules only.
...
**Every Codex session must update AGENTS.md at the end** — add or update sections for all features built, all bugs fixed, all schema/API/UI changes made. Keep AGENTS.md as the authoritative reference for the current state of the codebase.
...
- **Stripe `apiVersion`**: `src/lib/stripe.ts` pins `"2025-10-29.clover"` explicitly. Accounts v2 creation overrides only the raw `/v2/core/accounts` request to `"2026-02-25.clover"` in `stripeConnectV2State.ts`. Keep AGENTS.md and the Stripe client/helper versions in sync whenever the SDK/API version changes.
...
### Image loading
- **`loading="lazy"`** added across below-the-fold ListingCard grids/scroll rows, RecentlyViewed, FeedClient, browse surfaces, maker spotlight media, blog covers/avatars, and personalized homepage cards. The current homepage hero is the only intentionally preloaded content image.
...
- **Header layout behavior**: header `<nav>` uses `max-w-[1600px]` matching the body content width — not the previous `max-w-6xl` — so the header has the same visual reach as the page below. The search slot is uncapped `flex-1`, and the desktop actions do not use `ml-auto`; the parent `gap-3` is therefore the exact search-to-actions spacing on home and non-home pages while the search stays as long as available. Homepage desktop navigation uses the larger `lg:gap-12 xl:gap-16` logo-to-glass separation and a uniform `p-2` glass inset.
- **Homepage hero + floating-header behavior (2026-07-15)**: `/` uses the single local `public/hero-maple-cabinets.jpg` photograph through `next/image` with `fill`, `preload`, hero-only `quality={88}`, and a mobile-aware `sizes="(max-width: 639px) 150vw, 100vw"`; the larger mobile source hint accounts for the wide bitmap drawn behind the narrow `object-cover` viewport without disabling responsive optimization. The image is decorative (`alt=""`) because the adjacent hero heading carries the meaning. `object-cover` is intentionally transform-free, the base crop uses a 28% horizontal focal point to reveal more cabinetry while retaining the shadowed text zone, and the 58% vertical focal point shifts the photograph slightly upward. The image itself stays unfiltered; lighter `#2C1F1A` espresso scrims warm the hero and preserve copy contrast without the old near-black cast. The hero uses responsive clamp heights (`520–600px` base, `600–760px` from `sm`). `Header.tsx` branches on `pathname === "/"`: only the homepage header is absolutely overlaid and uses one responsive `.hero-logo-mark`, an exact `#E5DFD2` mask of the shared logo; `.hero-logo-mark` must not declare `display` because that can override responsive utilities and duplicate/overflow the logo. Every other route keeps the espresso logo and warm-cream header in normal document flow. The homepage search/navigation surface uses 26% cream, its search field uses 34%, and the mobile control surface uses 22%; these separate `rounded-2xl` frosted layers intentionally remain more translucent than content popovers. Homepage notification, desktop account, and mobile-menu cards use 58% cream glass with translucent painted children; unread homepage notifications use only a square-edged translucent `#EFEAE0` row background with neutral text. Non-home popovers retain opaque defaults. Do not put `backdrop-filter`, transforms, or `overflow-hidden` on an ancestor of `NotificationBell`, `MessageIconLink`, `UserAvatarMenu`, or `SearchBar`, because their fixed/absolute popovers must stay viewport-positioned and unclipped. The mobile search popup wrapper is inset and transparent, leaving only the frosted field floating over the hero; its `autoFocus` SearchBar opens fallback recommended searches independently of the focus event so mobile Safari still shows useful choices. Hero foregrounds use the existing `#E5DFD2` cream; mobile CTAs are natural-width, and their hover/active states remain frosted. The hero intentionally has no duplicate search, trending-chip row, or mosaic query. `data-home-stats` is a zero-height sibling after the hero whose 46% cream `dl` uses `-translate-y-1/2`, keeping the cached four-stat bar exactly half inside and half outside the hero at every breakpoint.
- **SearchBar visual + behavior contract (2026-07-15)**: the global header search is a clean 46px `rounded-2xl` field with a quiet left submit icon and the placeholder "Search pieces, shops, and more…". `overlay` selects the translucent cream/blur treatment used over the homepage photograph; the default remains the neutral near-white field used on normal pages. Preserve the explicit label, instance-scoped combobox/listbox IDs, 200-character cap, encoded navigation, latest-request-wins request IDs, `AbortController` cancellation, keyboard listbox navigation, popular-search fallback, and clear-button behavior. The header switches to the mobile/tablet control group below `lg`; all compact controls retain 44px targets. The mobile search toggle exposes `aria-expanded`/`aria-controls`, and its newly mounted search field receives focus so typing works after one tap.
...
- **Media URL validation behavior**: new write paths for user-supplied media must use `isFirstPartyMediaUrl()` or `filterFirstPartyMediaUrls()` so only Grainline-controlled R2/CDN URLs can be newly persisted. `isR2PublicUrl()` remains intentionally broader for legacy UploadThing/display compatibility in emails and AI review; do not use it as a first-party write-path validator.
- **Upload cleanup behavior**: `/api/upload/image` must delete the processed R2 object if public availability verification fails after `PutObjectCommand`; `/api/upload/verify` must delete direct-uploaded objects whose size/type does not match the signed verification token. Failed cleanup attempts should be Sentry-captured with `source: "upload_image_cleanup"` or `source: "upload_verify_cleanup"`; do not leave written-but-unusable media objects untracked.
- **Unsubscribe mutation behavior**: `GET /api/email/unsubscribe` verifies the signed token and renders a confirmation page only. `POST /api/email/unsubscribe` is the only mutating path, preserving `List-Unsubscribe-Post` one-click support while preventing scanner/prefetch GET requests from changing email preferences.
...
- **Sentry email telemetry behavior**: public support/data-request routes and email suppression failures must not send raw email addresses to Sentry `extra`, `tags`, `contexts`, or user context. Use `hashEmailForTelemetry()` from `src/lib/privacyTelemetry.ts` when correlation is needed, and keep regression coverage in `tests/privacy-telemetry.test.mjs`.
...
- **Message route/action account-state behavior**: message list/read/stream routes must verify current-user conversation participation before returning message rows. Message thread server actions that mutate thread state or send messages must reject banned/deleted local accounts inside the action, not only through middleware. Non-critical custom-order/message email failures should be Sentry-captured with conversation/user IDs, not swallowed silently.
...
- **Popover/motion accessibility behavior**: header/account popovers should expose `aria-controls`/expanded state and close when keyboard focus leaves them. Admin mobile navigation stays semantic navigation with `aria-current="page"` instead of tab roles.
