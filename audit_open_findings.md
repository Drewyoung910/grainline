...
7. **[VISUAL POLISH FIXED 2026-05-07] Several high-friction forms still drifted from the design tokens.** `/sign-in`, Clerk post-click `/sign-up`, `/support`, `/legal/data-request`, the buy-now checkout modal, custom-listing form, account orders list, and admin audit table now use the warm shell where appropriate, Clerk appearance variables, `card-section`/white surfaces, explicit neutral borders, rounded-md controls, and `font-display` headings. This closes the immediate real-user styling gaps without attempting a broad redesign of low-traffic historical/admin views. Regression coverage: `tests/verified-audit-followups.test.mjs`.
...
12. **[ARCHITECTURE OPEN] Stripe Connect onboarding still uses legacy Express account creation.** `src/app/api/stripe/connect/create/route.ts` creates Express accounts; Stripe's current Connect guidance for new platforms is Accounts v2. This remains a dedicated modernization branch, not a bug-fix patch.
