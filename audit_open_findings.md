...
- **Sitemap index 404 closed (2026-05-01).** `robots.txt` advertised `https://thegrainline.com/sitemap.xml` but Next.js's `generateSitemaps()` only emits chunked URLs at `/sitemap/N.xml`; the bare `/sitemap.xml` returned 404 for ~5 days, silently breaking sitemap-driven URL discovery. Added `src/app/sitemap.xml/route.ts` plus the Prisma-free chunk-count helper `src/lib/sitemapIndex.ts` (with a regression test) so the conventional path now serves a `<sitemapindex>` listing every chunk. CLAUDE.md updated with the new behavior note. Found while doing a post-outage status check, not from the audit doc.
