...
Latest mechanical open-heading count after the 2026-05-06 label-clawback follow-up pass: **0** broad unclosed numbered findings in the prior mechanical fix queue. The 2026-05-06 post-fix audit-only pass opened **9 verified code findings** plus **3 verified improvement/architecture items**. Highest priority is the Stripe full-refund transfer-reversal accounting bug; it should be fixed in a dedicated payment pass with Stripe test-mode replay evidence before launch. Historical sections still contain raw open/design/proven-stale notes for traceability; each future pass should keep verifying reproducibility before code changes.

2026-05-06 post-fix audit-only pass made no application-code changes and found these verified open items:

1. **[HIGH OPEN] Full Stripe refunds with tax can under-reverse seller transfers.** `src/lib/marketplaceRefunds.ts:85-103` splits a full refund into a seller-portion refund with `reverse_transfer: true` and a tax-only refund without transfer reversal, while checkout uses destination charges with a manual `transfer_data.amount` seller transfer (`src/app/api/cart/checkout/single/route.ts:462-468`, `src/app/api/cart/checkout-seller/route.ts:516-522`). Stripe reverses transfers proportionally for partial refunds, so refunding only `sellerPortionCents` first can leave part of the seller transfer unreversed when tax exists. Current tests encode the risky assumption at `tests/marketplace-refunds.test.mjs:26-58`. Fix direction: payment-only pass, replay full-tax refunds in Stripe test mode, and either issue one full reverse-transfer refund or move to explicit application-fee accounting; also remove or prove `refund_application_fee` is valid under the manual-transfer model.
