...
7. **[HARDENED 2026-05-13] Admin server actions now reject suspended/deleted staff inside the action.** Middleware already blocks suspended/deleted accounts and requires Admin PIN for admin server-action POSTs, but admin order/support/blog/broadcast actions now also select `banned`/`deletedAt` and fail closed before mutation. `admin/verification/page.tsx` already had the stronger helper. Regression coverage: `tests/admin-action-guardrails.test.mjs`.
