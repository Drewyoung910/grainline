...
Latest mechanical open-heading count after the 2026-05-06 Stripe full-refund accounting pass: **0** broad unclosed numbered findings in the prior mechanical fix queue. The 2026-05-06 post-fix audit-only pass now has **8 verified code findings** plus **3 verified improvement/architecture items** remaining. The Stripe full-refund transfer-reversal accounting bug was closed with Stripe test-mode replay evidence; next highest-priority batch is the public visibility consistency cluster. Historical sections still contain raw open/design/proven-stale notes for traceability; each future pass should keep verifying reproducibility before code changes.

2026-05-06 post-fix audit-only pass made no application-code changes and found these verified items; fixed entries remain here for traceability while the live count above tracks what remains open:
...
2. **[MEDIUM OPEN] Blog public visibility rules are inconsistent across page/API surfaces.** The main blog page filters only `BlogPost.status` plus author banned/deleted state (`src/app/blog/page.tsx:61-68`, `src/app/blog/page.tsx:129-146`) and its tag cloud also ignores seller profile visibility (`src/app/blog/page.tsx:156-164`). Blog detail has the same gap (`src/app/blog/[slug]/page.tsx:69-106`), while the newer search API already has stricter seller-profile visibility (`src/app/api/blog/search/route.ts:52-64`). The older public `/api/blog` endpoint filters only status/type/tag (`src/app/api/blog/route.ts:16-18`), comments can be read/created on any slug-resolved post without checking published/author/seller visibility (`src/app/api/blog/[slug]/comments/route.ts:29-33`, `src/app/api/blog/[slug]/comments/route.ts:97-99`), save/unsave resolves by slug only (`src/app/api/blog/[slug]/save/route.ts:9-10`), the sitemap includes published posts without seller-profile visibility (`src/app/sitemap.ts:67-72`), and popular blog tags/suggestions have the same drift (`src/app/api/search/popular-blog-tags/route.ts:8-18`, `src/app/api/blog/search/suggestions/route.ts:23-58`). Fix direction: extract a shared `publicBlogPostWhere()` helper matching the stricter search API and use it for page, detail, API list, comments, saves, sitemap, tag cloud, and suggestions.
3. **[MEDIUM OPEN] `SOLD_OUT` listing UX is internally unreachable.** Stock transitions deliberately set zero-stock listings to `SOLD_OUT` (`src/app/api/listings/[id]/stock/route.ts:113-125`, `src/app/api/stripe/webhook/route.ts:960-969`), and the listing page/purchase panel contain out-of-stock and notify-me UI (`src/app/listing/[id]/page.tsx:172-175`, `src/components/ListingPurchasePanel.tsx:170-177`). But `canViewListingDetail()` only permits `ACTIVE` public listings (`src/lib/listingVisibility.ts:36-44`, `src/lib/listingVisibility.ts:47-70`), `/api/listings/[id]/notify` also uses `publicListingWhere()` (`src/app/api/listings/[id]/notify/route.ts:31-38`), and saved listings explicitly link to `SOLD`/`SOLD_OUT` items (`src/app/account/saved/page.tsx:40-52`). Fix direction: decide whether sold-out listing pages should stay public; if yes, introduce a public-but-unbuyable listing visibility predicate and let the notify endpoint subscribe to public `SOLD_OUT` in-stock listings.
4. **[MEDIUM OPEN] Favorites can be created for non-public listings and then leak into personalized surfaces.** `POST /api/favorites` only verifies the listing owner for self-favorite blocking (`src/app/api/favorites/route.ts:46-60`) and then notifies the owner from an unrestricted lookup (`src/app/api/favorites/route.ts:70-87`). It never applies public listing visibility, so direct calls can favorite hidden/private/rejected/banned-seller listings, polluting `Favorite` counts used by quality/relevance (`src/lib/quality-score.ts:89-144`). The account dashboard's recent saved row also fetches favorites with only `userId` (`src/app/account/page.tsx:53-75`) and links whatever listing is returned (`src/app/account/page.tsx:226-248`), even though the full saved page has stronger filters (`src/app/account/saved/page.tsx:40-52`). Fix direction: validate favorites against the intended public/sold-out visibility predicate before upsert, and reuse the same filter in account dashboard recent saved items.
5. **[MEDIUM OPEN] Seller workshop gallery cannot be cleared.** `GalleryUploader` only submits hidden `galleryImageUrls` inputs for existing URLs (`src/components/GalleryUploader.tsx:19-22`), so removing every image submits no values. The seller settings action reads an empty list (`src/app/dashboard/seller/page.tsx:83-87`) but only writes `galleryImageUrls` when the list length is greater than zero (`src/app/dashboard/seller/page.tsx:132-136`). Result: a seller can remove all gallery images in the UI, save, and the old DB gallery remains public. Fix direction: always persist the submitted gallery array, or submit an explicit "gallery touched" sentinel so empty means clear rather than no-op.
6. **[LOW-MEDIUM OPEN] Public listing view/click tracking increments non-public listings.** The view route updates by raw listing id (`src/app/api/listings/[id]/view/route.ts:52-63`) and the click route does the same (`src/app/api/listings/[id]/click/route.ts:51-63`) without applying `publicListingWhere()` (`src/lib/listingVisibility.ts:3-17`). Known IDs for hidden/private/rejected listings can inflate listing counters and `ListingViewDaily` rows, which feed seller analytics and can carry into future quality/relevance if the listing later becomes public. Fix direction: silently skip tracking unless the listing currently satisfies the public visibility predicate.
7. **[LOW-MEDIUM OPEN] Follow state can include inactive or hidden sellers.** The follow API GET/POST/DELETE resolves `SellerProfile` by id only (`src/app/api/follow/[sellerId]/route.ts:22-30`, `src/app/api/follow/[sellerId]/route.ts:77-87`, `src/app/api/follow/[sellerId]/route.ts:154-158`), while public seller pages hide banned/deleted and non-charges-enabled sellers from non-owners (`src/app/seller/[id]/page.tsx:116-128`, `src/app/seller/[id]/shop/page.tsx:117-119`). The following page then renders followed sellers without filtering `sellerProfile.user.banned/deletedAt` or `chargesEnabled` (`src/app/account/following/page.tsx:22-58`, `src/app/account/following/page.tsx:80-149`). Fix direction: apply active seller visibility to follow creation and account following, while still allowing unfollow/delete cleanup for stale rows.
8. **[LOW-MEDIUM OPEN] Gift wrapping is still omitted on two staff reconciliation surfaces.** The earlier order-total fix covered buyer/seller/admin order pages, analytics, and email, but `admin/flagged` still computes total as items + shipping + tax (`src/app/admin/flagged/page.tsx:79-83`) and `admin/cases/[id]` selects/displays items, shipping, and tax without `giftWrappingPriceCents` (`src/app/admin/cases/[id]/page.tsx:84-94`, `src/app/admin/cases/[id]/page.tsx:174-185`). Fix direction: use `orderTotalCents()` and display a gift wrapping row when present on these staff surfaces.
9. **[LOW-MEDIUM OPEN] Seller profile view analytics are inflated by render-time increments.** Seller profile pages mutate `profileViews` during server render with no cookie/IP/user dedupe and no owner/bot exclusion (`src/app/seller/[id]/page.tsx:276-280`). That counter is displayed in seller analytics and sorts maker pages (`src/app/api/seller/analytics/route.ts:152`, `src/app/api/seller/analytics/route.ts:630`, `src/app/makers/[metroSlug]/page.tsx:98`). Fix direction: move profile tracking to a small POST endpoint mirroring listing view dedupe, skip owners, and rate-limit/dedupe by viewer/IP.
10. **[LOW IMPROVEMENT OPEN] Seller profile rating still does an on-demand review aggregate.** `src/app/seller/[id]/page.tsx:210-220` computes seller rating with a raw review aggregate on every seller profile render even though `SellerRatingSummary` already stores the same seller-wide calculation (`src/lib/sellerRatingSummary.ts:8-30`) and `getSellerRatingMap()` is used on homepage/browse/listing detail. Fix direction: read `SellerRatingSummary` on seller profile pages as well.
11. **[LOW COMPLIANCE/OBSERVABILITY OPEN] Account export has no durable fulfillment audit trail.** `/api/account/export` ensures the signed-in user and rate-limits, then immediately streams the JSON download (`src/app/api/account/export/route.ts:376-384`) without a `logUserAuditAction()`/security event or another durable support record. Fix direction: add a low-detail audit entry for export generation (user id, timestamp, route/method, no exported payload) and consider step-up auth as a separate product/security decision for full PII exports.
