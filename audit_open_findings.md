...
2. **[CANARY READY; PRODUCTION-SAFE TICKET RETRY PREPARED] Notification authenticated route smoke initially had no eligible Clerk-backed dedicated test identity.** Two attempts failed closed and fully cleaned before any ordinary account was impersonated. Drew then authorized the permanent operational canary, which now exists as exactly one no-password Clerk identity and signed-webhook production row with current legal state, one controlled Gmail alias/welcome reservation, no fallback-outbox row, and zero marketplace activity. The third attempt's child preparation, real-RPC local proof, and Preview attestation passed, but its first authenticated run stopped before any HTTP route request because live Clerk rejects backend `sessions.createSession` with `request_invalid_for_environment`. All fixtures/state/cache cleanup passed. The corrected flow uses Clerk's 60-second one-use sign-in token and Frontend API `ticket` strategy, then mints the bounded JWT and revokes every active session on the dedicated canary in `finally`. A response-shape probe briefly created one canary session and immediately inventoried/revoked it; the corrected probe passed with zero remaining sessions. One same-child retry is allowed only from the exact retained pre-request failure and complete cleanup evidence; teardown tracks both Preview deployments.
