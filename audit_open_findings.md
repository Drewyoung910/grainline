...
6. **[LOW UX FIXED 2026-05-08] Address search/autocomplete existed only in the pickup map flow.** `AddressAutocomplete.tsx` and `usStates.ts` now back checkout shipping, seller ship-from, and pickup-map search with a shared throttled dropdown. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
7. **[LOW UX FIXED 2026-05-08] Blog search could show a right-edge vertical artifact.** `BlogSearchBar` now mirrors the global search pill's zero-radius submit button treatment and explicit dropdown border styling. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
