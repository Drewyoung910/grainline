...
1. **[LOW-MEDIUM UX FIXED 2026-05-09] Checkout/seller address autocomplete could leave city blank for common US Nominatim results.** Address parsing now falls through additional city-like fields (`hamlet`, `suburb`, `neighbourhood`, `city_district`) and then parses the display name while still excluding county from city and suggestion labels. Selecting a suggestion now clears the search box so the populated destination fields are the source of truth. Regression coverage: `tests/address-autocomplete-state.test.mjs`, `tests/post-launch-ui-followups.test.mjs`.
...
11. **[DEFERRED REPRO NEEDED 2026-05-09] Checkout address autocomplete still feels unreliable for some addresses.** Current shared code no longer uses county as city and formats labels cleanly; reopen with a concrete failing address if Nominatim returns incomplete structured data, then consider an inline "verify fields" notice or a paid US address provider.
