...
- [PARTIAL 2026-04-27] Listing and seller public pages now accept slugged `id--readable-name` URLs, publish canonical metadata to slugged paths, emit slugged URLs from sitemap and listing cards, and keep legacy CUID-only URLs working. Homepage, blog, commission, metro maker, map, buyer account/cart/order, message, and transactional-email surfaces now use readable links where the label is already available. Deeper admin/dashboard/notification link cleanup can continue incrementally.
