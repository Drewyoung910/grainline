...
Latest mechanical open-heading count after the 2026-05-06 observability cleanup pass: **0** broad unclosed numbered findings in the prior mechanical fix queue and **0** verified code findings from the post-fix audit-only pass. The only remaining item from this live ledger is the deferred Stripe Connect v2 architecture modernization, which should be handled as a dedicated branch rather than mixed into bug-fix batches. Historical sections still contain raw open/design/proven-stale notes for traceability; each future pass should keep verifying reproducibility before code changes.
