...
Latest mechanical open-heading count after the 2026-05-06 extended audit-only pass: **0** broad unclosed numbered findings in the prior mechanical fix queue, **10** newly verified open follow-ups from the latest audit-only sweep, plus the deferred Stripe Connect v2 architecture modernization. Historical sections still contain raw open/design/proven-stale notes for traceability; each future pass should keep verifying reproducibility before code changes.

2026-05-06 extended audit-only sweep found these verified follow-ups and made no application-code changes:

1. **[MEDIUM OPEN] Seller profile view tracking endpoint is not public in middleware.** `src/components/SellerProfileViewTracker.tsx:7` posts to `/api/seller/${sellerId}/view`, and `src/app/api/seller/[id]/view/route.ts:15-66` is intentionally written as optional-auth public analytics. `src/middleware.ts:11-54` public-allows listing view/click tracking but not `/api/seller/([^/]+)/view`, so anonymous seller-profile views are blocked by `auth.protect()` at `src/middleware.ts:150-156` before the route can run. Fix direction: add the seller profile view endpoint to the public matcher while still letting signed-in banned/deleted users be blocked by the suspended-account guard.
2. **[MEDIUM OPEN] `activeSellerProfileWhere()` conflates visible sellers with currently sellable sellers.** `src/lib/sellerVisibility.ts:3-13` requires `vacationMode: false`, but public seller/profile shop pages intentionally render vacation sellers and their follow buttons (`src/app/seller/[id]/page.tsx:119-130,279-344`; `src/app/seller/[id]/shop/page.tsx:117-119,233-280`). Follow GET/POST, account following, and seller-profile view tracking use that vacation-excluding helper (`src/app/api/follow/[sellerId]/route.ts:24-28,78-82`; `src/app/account/following/page.tsx:24-28`; `src/app/api/seller/[id]/view/route.ts:30-34,57-60`), so visible vacation sellers can render as followable but return 404 and disappear from following/profile-view surfaces. Fix direction: split `visibleSellerProfileWhere()` from `sellableSellerProfileWhere()`.
3. **[MEDIUM OPEN] `/seller/payouts` is treated as a public seller page even though it is a seller operations surface.** `src/middleware.ts:17` public-allows `/seller(.*)`, which includes `src/app/seller/payouts/page.tsx:15-51`. The page has no server auth redirect and no error UI around its protected Stripe API calls; anonymous visitors can load broken payout-management UI. `src/app/api/stripe/connect/create/route.ts:76-77` also still falls back to `/seller/payouts`, so this route remains part of the Stripe onboarding flow. Fix direction: narrow the public seller matcher and make `/seller/payouts` authenticated or redirect it to `/dashboard/seller`.
4. **[MEDIUM OPEN] Global search blog suggestions can return non-public blog posts.** `src/app/api/search/suggestions/route.ts:115-125` joins only `BlogPost` to author `User`, filtering published + active author, but it does not apply the seller-profile visibility predicate from `publicBlogPostWhere()` (`src/lib/blogVisibility.ts:3-22`). Dedicated blog suggestions include the missing seller joins at `src/app/api/blog/search/suggestions/route.ts:25-42`. `SearchBar` pushes returned blog slugs to `/blog/${slug}` (`src/components/SearchBar.tsx:93-103,206-210`), while `src/app/blog/[slug]/page.tsx:68-105` 404s the same slug when the seller is vacationing, disconnected, banned, or deleted. Fix direction: mirror the blog-search raw SQL predicate or extract a shared raw-SQL visibility fragment.
5. **[MEDIUM OPEN] Account following empty-state CTA links to a missing `/sellers` page.** `src/app/account/following/page.tsx:77-82` links "Find Makers to Follow" to `/sellers`, but the route tree has only `src/app/sellers/map/page.tsx`; there is no `src/app/sellers/page.tsx`. Fix direction: link to an existing discovery page (`/map`, `/browse`, or a real seller-directory page).
6. **[LOW-MEDIUM OPEN] Dashboard analytics "View all" links to missing `/dashboard/listings`.** `src/app/dashboard/analytics/page.tsx:655-662` links top-listings "View all" to `/dashboard/listings`, but `src/app/dashboard/listings` currently contains only `new`, `custom`, and `[id]/edit` routes. Fix direction: point to the inventory/shop listing management surface or add a dashboard listing index.
7. **[LOW OPEN] `/seller/map` is a blank public 200.** `src/app/seller/map/page.tsx:1-3` returns `null`; the real map surfaces are `/map` and `src/app/sellers/map/page.tsx`. Fix direction: delete, 404, or redirect this stale singular route.
8. **[LOW COMPLIANCE/OBSERVABILITY OPEN] Account export audit logging is still best-effort, not durable.** `/api/account/export` writes `ACCOUNT_EXPORT` at `src/app/api/account/export/route.ts:385-393`, but `logUserAuditAction()` delegates to `logAdminAction()`, which catches audit insert failures and returns an empty string at `src/lib/audit.ts:14-41,44-67`. If compliance needs proof of export fulfillment, the export should fail closed or use a strict audit writer when no audit row is persisted.
9. **[LOW-MEDIUM OPEN] Case escalation route documents CRON_SECRET access that middleware blocks.** `src/app/api/cases/[id]/escalate/route.ts:1-31` accepts `CRON_SECRET` bearer requests and documents `id="all"` bulk escalation, and `CLAUDE.md:184-185` says CRON_SECRET is allowed. But `/api/cases/[id]/escalate` is not public in `src/middleware.ts:11-54`, so unauthenticated cron-secret requests are stopped by `auth.protect()` before route-level `verifyCronRequest()` can run. The real scheduled escalation currently lives in `/api/cron/case-auto-close`; fix direction: either remove the advertised cron-secret path or public-allow this route intentionally.
10. **[LOW DOC OPEN] `CLAUDE.md` still contains stale fixed-interval NotificationBell polling docs.** `CLAUDE.md:674` says `NotificationBell` polls every five minutes, while `src/components/NotificationBell.tsx:169-218` and `CLAUDE.md:3821-3829` document the current adaptive 60s/5min/15min/stop behavior. Fix direction: update the older component bullet or replace it with a pointer to the newer section.
