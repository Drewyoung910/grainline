...
2. **[CANARY CREATED AND VERIFIED; AUTHENTICATED ROUTE SMOKE NEXT] Notification authenticated route smoke had no eligible Clerk-backed dedicated test identity.** The first attempt found no marked terms-accepted identity and exposed the Preview/production Redis namespace collision. After that branch fix, the second attempt proved and attested the exact candidate but stopped before session/fixture/account-state work: all four email-marked child rows were synthetic provider actors with no Clerk identities, while all six active Clerk-backed rows were unmarked. No ordinary account was impersonated. Both Previews, 28-variable sets, bypasses, child branches, fixtures, and private states were removed with production unchanged. Drew then explicitly authorized Codex to create the identity. The initially reviewed identifier-less create failed closed because live Clerk requires email and produced no partial identity. The corrected permanent no-password Clerk operational canary uses the exact external id, current terms/age metadata, and an in-memory `+grainline-notification-canary` alias derived from the sole active production admin's Gmail inbox. It was created and independently rechecked on 2026-07-22: exactly one Clerk identity and signed-webhook production row, one welcome reservation, no fallback-outbox row, zero marketplace activity, and mode-0600 hashed evidence. It is not disposable because Clerk deletion would itself leave production anonymization/audit residue. The route selector binds only to that exact external id.
3. **[EXTRA HIGH AUTHORITY REVIEW PASSED; NOT YET PRODUCTION] Notification's consolidated SQL/application authority review completed at `ab2d08a6`.** All seventeen definer and eight invoker functions were reviewed for fixed SQL, pinned search paths, narrow grants, source-derived target/payload/link/replay identity, bounded cleanup, and server-resolved recipient context. The preparation/activation split preserves old-app compatibility and makes the guarded prelaunch purge the first locked activation step. The exact head passed 54/54 source coverage, candidate reproduction, 1,857 local tests with three expected skips, TypeScript, and lint; branch workflow `29894705025` passed database preparation through rollback at `a4ced63b`. Remaining gates are the authorized authenticated route smoke, legacy aggregate/backup inspection, clean release/full CI, application rollback proof, and production migration/deployment/postflight sequence.
