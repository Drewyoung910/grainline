...
| Test coverage gaps | Partially open. Pure tests now cover account-state errors, account-export download formatting/payload shape, shipping tokens, unsubscribe tokens, notification dedup, Sentry filters, cron auth/retry helpers, email outbox quota/retry/stale-state helpers, listing variants, slug helpers, media URL/R2 key validation, DB URL SSL normalization, upload verification tokens, order PII retention cutoff policy, marketplace refund splitting, checkout transfer math, refund-lock state, Stripe webhook event/idempotency state, webhook metadata/seller-state helpers, and refund-route guard state. | Continue route-level payment/webhook/refund coverage where pure helpers can be extracted without brittle mocks. |
