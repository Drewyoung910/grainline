...
4. **[PHASE 2 UX FIXED 2026-05-07] Non-sellers did not have a discoverable "Become a Maker" CTA.** `/become-a-maker` is now a public redirect entry that sends signed-out users to sign-up with `redirect_url=/dashboard` and signed-in users to `/dashboard`; `/dashboard` still creates/loads the seller profile and redirects incomplete sellers to onboarding. The footer links to `/become-a-maker`, and `/account` shows a non-seller "Become a Maker" CTA. Regression coverage: `tests/verified-audit-followups.test.mjs`.
