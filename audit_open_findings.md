...
2. **[AUTHENTICATED ROUTE SMOKE PASSED; ALL DISPOSABLES CLEANED] Notification authenticated route smoke initially had no eligible Clerk-backed dedicated test identity.** Two attempts failed closed and fully cleaned before any ordinary account was impersonated. Drew then authorized the permanent operational canary, which now exists as exactly one no-password Clerk identity and signed-webhook production row with current legal state, one controlled Gmail alias/welcome reservation, no fallback-outbox row, and zero marketplace activity. The third attempt's child preparation, real-RPC local proof, and Preview attestation passed, but its first authenticated run stopped before any HTTP route request because live Clerk rejects backend `sessions.createSession` with `request_invalid_for_environment`. The corrected 60-second one-use sign-in-token/Frontend-API-ticket flow passed on the single bounded retry: unauthenticated denial, canary-only bell/page projection, exact read mutations, foreign-row preservation, and cross-origin denial all matched. Session/token/fixture/state/cache cleanup passed. Full teardown removed both Previews, 28 variables, the automation bypass, synthetic fixtures/schema, child branch, and private state; independent inventory was zero while production remained unchanged. The retained permanent canary has zero marketplace activity and zero active sessions. Endpoint-specific Preview exceptions remain isolated from production packaging.
3. **[PREPARATION RELEASE PACKAGING; NOT YET PRODUCTION] Notification's consolidated SQL/application authority review completed at `ab2d08a6`.** All seventeen definer and eight invoker functions were reviewed for fixed SQL, pinned search paths, narrow grants, source-derived target/payload/link/replay identity, bounded cleanup, and server-resolved recipient context. The preparation/activation split preserves old-app compatibility and makes the guarded prelaunch purge the first locked activation step. The exact head passed 54/54 source coverage, candidate reproduction, 1,857 local tests with three expected skips, TypeScript, and lint; branch workflow `29894705025` passed database preparation through rollback at `a4ced63b`, and the authenticated route smoke now passes. The next owner-only legacy inspector has been hardened before use: it fixes invalid `pg_catalog.current_user` syntax, requires an exact clean manually dispatched `main` commit plus protected-credential digest, runs only in a repeatable-read read-only transaction, verifies the prepared/no-policy/legacy-CRUD posture, and writes aggregate-only mode-`0600` evidence. It is deliberately not runnable from the feature branch because the GitHub `Production` environment remains main-only. The compatible preparation migration is staged at SHA-256 `9f7eeaf23e0f334dbb52427d27343674a5d11095b0b7f433d3ca177e3914956e`; a release verifier proves its executable body matches the earlier disposable-proof candidate and that activation remains absent. A fresh disposable PostgreSQL run on this promoted artifact, temporary-artifact exclusion, clean release/full CI, production preparation, protected aggregate inspection plus backup boundary, compatible application deployment and rollback proof, activation, and production postflight remain required.

4. **[CLEAN PREPARATION RELEASE SHAPE; NOT YET PRODUCTION] The release derivative removes every disposable HTTP/deployment exception while retaining reusable proof machinery.** The `/api/internal/rls-context-gate` route, Clerk middleware exemption, runner-only test, completed proof-branch `vercel.json` entries, Vercel database-alias exception, and broad draft-presence build exception are absent. The provider measurement engine is retained under `scripts` outside application runtime; the generic context harness, PostgreSQL preparation/authority/rollback proofs, 54/54 coverage gate, authenticated-smoke record, canary operator, exact migration verifier, and durable security tests remain. The exact `notification-preparation-reviewed` artifact guard and preparation-body verifier pass, as do 1,890 local tests with three expected skips, TypeScript, and lint. The first Turbopack build attempt was an invalid-worktree failure because `node_modules` was a symlink outside the temporary worktree root; a webpack fallback then exhausted the local 2 GB heap, and its 4 GB retry exhausted disk cache space. These are retained as local-environment failures, not successful build evidence; a clean GitHub checkout must still prove the normal build path.

5. **[TEMP-WORKTREE RECOVERY VERIFIED] The local build briefly reduced free disk space to 32 MiB, after which the system removed six `/private/tmp` worktree directories.** The preparation checkpoint was already pushed at `5f4e7a2e`. The only known uncommitted work was the active release-cleanup diff; it was reconstructed from that checkpoint, rerun through the exact release guards, 1,890 tests, TypeScript, and lint, then committed and pushed at `7df9fe04`. A branch audit proved each deleted worktree's local head exactly matched its corresponding `origin` head for Notification, provider proof 5, runtime credential separation, SavedSearch Phase-B operator, and SavedSearch Phase-B release. Git cannot recover or prove an unknown never-staged scratch file after its temp directory is removed, but prior clean-status evidence and the exact local/remote head matches show no missing durable work. The main checkout and its unrelated user-owned changes were not cleaned or modified.

6. **[CI TEST EXPECTATION FIXED; RERUN REQUIRED] Main-targeted CI run `29949186993` passed the release artifact, migrations, final grant audit, TypeScript, and lint, then stopped in the synthetic live grant-audit regression.** The audit correctly reported `runtime role has unexpected column privileges: id:REFERENCES`, but the older test expected the less-specific pre-hardening phrase `has column privileges`. The runtime and PUBLIC cases now assert their distinct exact diagnostics. This changes no database authority or application behavior; the final SHA must repeat full CI and the disposable Notification PostgreSQL proof before integration.
...
3. **[DOCUMENTED 2026-05-13] Marketplace security benchmark added.** `docs/security-hardening-plan.md` now records source-backed comparisons against Etsy/Shopify/Stripe/Postgres/Cloudflare/GitHub/security.txt practices, including seller MFA, seller identity verification, purchase protection, PCI SAQ A, payment-page script controls, vulnerability disclosure, Cloudflare/GitHub hardening, and a deliberately staged RLS feasibility plan. `docs/launch-checklist.md`, `docs/runbook.md`, and `docs/legal-risk-register.md` now include corresponding ops/legal evidence items.
4. **[DOCUMENTED 2026-05-13] Maintainability and bug-resistance plan added.** `docs/maintainability-plan.md` records that the codebase needs stabilization, not a rewrite, and defines merge discipline, AI-assisted development rules, refactor priorities, and team handoff standards.
5. **[DOCUMENTED 2026-05-13] Legal risk register added.** `docs/legal-risk-register.md` tracks counsel-facing risk areas: Terms/Privacy/clickwrap, marketplace payments, sales tax, seller disclosures/INFORM, consumer protection, privacy/retention, UGC/IP/DMCA, accessibility, email/marketing, geo/sanctions, and insurance. It is explicitly not legal advice.
6. **[STARTED 2026-05-13] Authorization/IDOR audit log added.** `docs/security-audit-log.md` records the route/action inventory and first spot checks. Initial spot checks found no verified IDOR in review edit/delete, seller order fulfillment, commission detail/close, or case mark-resolved paths; remaining dynamic private routes still need the same treatment.
7. **[HARDENED 2026-05-13] Admin local gates now reject suspended/deleted staff.** Middleware already blocks suspended/deleted accounts and requires Admin PIN for admin API/server-action POSTs, but admin order/support/blog/broadcast actions, admin read pages, admin APIs, and the Admin PIN verification route now also select `banned`/`deletedAt` and fail closed locally before rendering or mutating high-privilege data. Regression coverage: `tests/admin-action-guardrails.test.mjs`.
8. **[DOCUMENTED 2026-05-13] RLS feasibility plan added.** `docs/rls-feasibility-plan.md` records that Grainline does not currently use PostgreSQL RLS, should not enable broad production policies before launch, and should first prototype on low-blast-radius tables (`Notification`, `SavedSearch`) with runtime/migration role separation, no `BYPASSRLS` runtime role, transaction-local `set_config('app.user_id', ..., true)`, explicit staff/cron/webhook bypasses, and rollback tests. Regression coverage: `tests/rls-feasibility-plan.test.mjs`.
9. **[HARDENED 2026-05-13] Open Stripe Checkout Sessions now expire when seller/listing availability changes.** Seller vacation mode and listing hide/mark-sold/archive/AI-hold transitions queue `expireOpenCheckoutSessionsForSeller()` or `expireOpenCheckoutSessionsForListing()` so buyers are not allowed to complete stale checkout sessions after a seller/listing becomes unavailable. Successful proactive expiration also calls the idempotent stock-restore helper; the webhook still revalidates buyer/seller/listing state at payment completion and refunds blocked checkouts as a backstop. Regression coverage: `tests/checkout-session-expiry.test.mjs`.
10. **[AUDITED CLEAN 2026-05-13] Account/cart/notification/favorite/listing-interaction route batch.** `docs/security-audit-log.md` now records the inspected current-user and current-seller ownership paths for account feed, shipping address, notification preferences/read state, cart read/add/update, favorites, stock notifications, listing stock patch, retired photo mutation route, public listing analytics/recent/similar routes, and `/api/me`. No verified IDOR found. Noted policy nuance: do not proactively expire sessions from the stock-to-zero patch path because Stripe expired-session restoration would fight seller intent; webhook payment-completion revalidation remains the backstop.
11. **[HARDENED 2026-05-13] Seller operational routes spot-checked.** Vacation mode now accepts native `YYYY-MM-DD` return dates, keeps invalid-date rejection, expires open checkout sessions only when enabling vacation mode, and lets sellers toggle back off from the warning popup as a cancel-equivalent. Seller broadcasts remain gated on `chargesEnabled && !vacationMode`, first-party media URLs, and seller rate limits; non-blocking notification fanout failures now emit bounded Sentry evidence instead of disappearing silently. Seller analytics/recent-sales scoping was re-checked against current-user seller ownership and whole-order seller ownership. Regression coverage: `tests/seller-ops-hardening.test.mjs`.
12. **[HARDENED 2026-05-13] Payment/order side-effect failures are observable and non-masking.** Fulfillment notifications/emails, seller-refund buyer notification/email, shipping-label lock/orphan-record cleanup, and checkout stock-restoration best-effort failures now emit Sentry evidence with bounded IDs/counts. Fulfillment side-effect failures no longer convert a successful order status mutation into a false 500 response. Label telemetry avoids full label URLs/tracking numbers, and checkout restoration telemetry avoids buyer/address payloads. Regression coverage: `tests/payment-side-effect-observability.test.mjs`.
13. **[AUDITED/HARDENED 2026-05-13] Cron, health, CSP-report, blog/search, and checkout rollback surfaces audited.** All cron routes use shared bearer auth, cron-run idempotency state, and Sentry cron monitors; CSP reports and health checks are public but rate-limited and sanitized/detail-gated; checkout rollback is current-buyer-scoped and restores stock idempotently. `/api/blog` now has the same shared IP rate-limit boundary as public search, and blog search endpoints use shared `getIP()` instead of local forwarded-header parsing. Regression coverage: `tests/public-cron-search-hardening.test.mjs`.
14. **[HARDENED 2026-05-13] Social interaction routes audited.** Blog comments, blog saves, reviews, review replies/votes, follow/unfollow, block/unblock, and user reports were re-checked for current-user scoping, public visibility predicates, account-state gates, media URL restrictions, and side-effect handling. Fixes: review and follow notification failures no longer mask successful primary mutations; duplicate review races return `409`; blog comment trees are bounded and replies cannot target hidden/inactive parent comments; commission-request reports are limited to open public commission targets. Regression coverage: `tests/social-interaction-hardening.test.mjs`.
15. **[AUDITED/HARDENED 2026-05-13] Server action spot check continued.** Account block unblock, onboarding actions, admin order/support actions, and seller listing shop actions were reviewed for current-user/current-seller/staff scoping. Fixes: listing activation and new-listing follower fanout failures now emit Sentry evidence instead of silent catches, and listing-create AI review/error-marking failures emit bounded Sentry evidence while still failing closed into review. Regression coverage: `tests/server-action-hardening.test.mjs`.
16. **[HARDENED 2026-05-13] Message/custom-order routes audited.** Message list/read/stream/unread and custom-order request routes were checked for conversation participant scoping, current-account gates, block checks, seller eligibility, listing context validation, and side-effect ordering. Fixes: custom-order budget validation now runs before conversation/message creation, and seller notification failures no longer mask successful custom-order message creation. Regression coverage: `tests/custom-order-admin-thread-followups.test.mjs`.
17. **[HARDENED 2026-05-13] Upload-backed media write paths now require current-uploader ownership.** The upload routes already created user-segmented R2 keys, but later hidden-field write paths mostly checked only that URLs were first-party Grainline media. New writes now use `isFirstPartyMediaUrlForUser()` / `filterFirstPartyMediaUrlsForUser()` so message attachments, listing photos, profile media, onboarding avatars, commission references, review photos, blog covers, broadcast images, and legacy listing actions cannot attach another user's public Grainline media URL. Listing/profile/review/blog edits still preserve already-owned existing DB values so legacy or unchanged media is not broken. Regression coverage: `tests/media-url.test.mjs`, `tests/pr-i-media-upload-unsubscribe-followups.test.mjs`, `tests/seller-ops-hardening.test.mjs`, and `tests/server-action-hardening.test.mjs`.
18. **[HARDENED 2026-05-14] Account deletion media cleanup is scoped to the deleted uploader.** Account deletion collected seller/listing/review/commission/message/blog media URLs and best-effort deleted R2 objects after the local anonymization transaction. Because blog markdown and message JSON can contain copied first-party public URLs, cleanup now filters through `accountDeletionMediaUrlsForCleanup()` and deletes only configured first-party media keys whose path-safe user segment matches the deleted Clerk user. This prevents one account deletion from deleting another user's uploaded image by referencing its public URL. Regression coverage: `tests/account-deletion-media.test.mjs`.
19. **[LOW-MEDIUM FIXED 2026-05-14] Commission reference uploads used a seller-only media endpoint.** `/commission/new` was using `listingImage`, which is intentionally seller-only in the upload route. Non-seller buyers posting a commission request could therefore fail to attach reference images. Commission references now upload and validate through `messageImage`, which is current-user scoped but not seller-gated; account-deletion media cleanup includes `messageImage` keys. Regression coverage: `tests/pr-i-media-upload-unsubscribe-followups.test.mjs`.
20. **[LOW-MEDIUM HARDENED 2026-05-14] Message attachment upload rules allowed videos despite image/PDF-only UX.** `messageAny` and `messageFile` accepted MP4/MOV through server upload rules even though the composer and buyer-facing copy only support images/PDFs. Message endpoints now allow images plus PDFs for `messageAny`, PDFs only for `messageFile`, and keep videos isolated to seller-only `listingVideo`; `CLAUDE.md` upload-limit docs were also corrected to match `uploadRules.ts`. Regression coverage: `tests/upload-ux-followups.test.mjs`.
21. **[LOW HARDENED 2026-05-14] Blog notification side effects still had silent catches.** Admin blog-comment approval and maker blog publish/update follower fanout were intentionally non-blocking, but their failure paths used silent `catch {}` blocks. They now emit bounded Sentry evidence with post/comment/seller IDs while preserving the successful primary action. Regression coverage: `tests/blog-action-guardrails.test.mjs`, `tests/admin-moderation-observability.test.mjs`.
22. **[LOW PRIVACY HARDENED 2026-05-14] Central email helper still sent raw recipient emails to Sentry extras.** Public support/data-request and suppression telemetry were already hashed, but `src/lib/email.ts` still reported raw `to` values on inactive-account lookup, invalid-recipient, retry, and send-failure paths. Central email telemetry now uses `hashEmailForTelemetry()` and `subjectLength` instead of raw recipient/subject values. Regression coverage: `tests/account-privacy-observability.test.mjs`.
23. **[LOW PRIVACY HARDENED 2026-05-14] Profanity/moderation checks logged raw matched words to server logs.** Message, blog, broadcast, commission, comment, review, and review-reply profanity checks were log-only but emitted `[PROFANITY] ... matches.join(...)` console lines. They now use `captureProfanityFlag()` with bounded IDs and `matchCount`, avoiding raw submitted text and raw matched terms in telemetry. Regression coverage: `tests/profanity-telemetry.test.mjs`.
...
Drew shipped commit f578cef (preview links + alt-text retry + lightbox + header), then reported regressions and polish issues.

1. **[CRITICAL FIXED 2026-05-10] Re-crop modal slider was dragging the underlying `<li draggable>` photo card.** `ImageCropModal` rendered inline as a child of the parent component tree. In `PhotoManager` and `EditPhotoGrid`, the modal lived inside a `<li draggable>` photo card, so pointer events on the zoom slider bubbled into the HTML5 drag handler and "moved" the underlying card around. Fix: render the modal through `createPortal(modal, document.body)` after a client-mount guard. Documented as "ImageCropModal portal behavior". `src/components/ImageCropModal.tsx`.
2. **[CRITICAL FIXED 2026-05-10] Adding a photo on the edit listing page didn't show the new card until the page was manually refreshed.** `AddPhotosButton` correctly calls `router.refresh()` after attaching the photo, but `EditPhotoGrid` initialized `photos`/`altTexts` from `useState(initialPhotos)` — only the initial render. New props from the refresh never propagated. Fix: `useEffect` keyed on `${id}:${url}` per photo syncs local state when the prop changes; alt-text merge keeps in-progress local edits per existing photo id. Documented as "EditPhotoGrid prop sync behavior". `src/components/EditPhotoGrid.tsx`.
3. **[FIXED 2026-05-10] "Live message updates are temporarily unavailable" warning was firing on every SSE blip even when polling fallback was working silently.** `es.onerror` set the warning before falling back to 3-second polling. Stream errors are noisy (visibility change, network blips, idle drops), so users saw the warning constantly even though messages kept flowing via polling. Fix: dropped the `setStreamError` from the `es.onerror` handler — polling silently takes over. Only TERMINAL polling failures (401/403/429) surface the warning at the polling site. Documented as "Message stream fallback behavior". `src/components/ThreadMessages.tsx`.
4. **[FIXED 2026-05-10] Message thread used bare `rounded-lg + border` and `md:border` styling instead of the design system.** Listing context card on the message thread page now uses `card-section` with `hover:shadow-md`. Thread container uses `md:card-section md:p-4` instead of `md:rounded-xl md:border md:bg-white`. Archive button border explicitly scoped to `border-neutral-200`. `src/app/messages/[id]/page.tsx`, `src/components/ThreadMessages.tsx`.
5. **[FIXED 2026-05-10] Message thread empty state was a blank white area.** Now centered `MessageCircle`-on-amber-50 visual with status-aware headline ("Start the conversation" / "Say hi to {name}") and supporting copy. `src/components/ThreadMessages.tsx`.
6. **[FIXED 2026-05-10] Header icon-only buttons had inconsistent hover treatment — message envelope had a hover circle, cart and notification bell did not.** Applied the same `relative inline-flex h-9 w-9 items-center justify-center rounded-full text-neutral-800 hover:bg-neutral-50` pattern to cart, signed-out message fallback, and `NotificationBell`. Documented as "Header icon button hover behavior". `src/components/Header.tsx`, `src/components/NotificationBell.tsx`.
7. **[FIXED 2026-05-10] Cart loading was a plain "Loading…" text and empty state was unstyled.** Added `CartLoadingSkeleton` (animate-pulse cart-shape skeleton) replacing plain text, and `CartEmptyState` (card-section with `ShoppingBag`-on-amber-50, primary CTA "Browse the workshop", secondary "Sign in" when not signed in). Documented as "Message thread + cart empty-state behavior". `src/app/cart/page.tsx`.

Verification: `npm test` 476/476 pass (added 6 new test cases — ImageCropModal portal, EditPhotoGrid prop sync, SSE silent fallback, message thread card-section + empty state, header hover circle, cart skeleton + empty state), `npx tsc --noEmit` clean, `npm run lint` clean (existing jsx-ast-utils warnings only), `npm run build` clean, `npm audit --audit-level=moderate` 0 vulnerabilities.

## 2026-05-10 (afternoon) — Listing card 404, AI alt-text retry, lightbox crop, header layout (Claude direct write — Codex unavailable)

Drew shipped commit 7996026 (edit-flow polish), then reported five new issues from production testing.

1. **[FIXED 2026-05-10] Dashboard My Listings card click hits 404 for HIDDEN/REJECTED/PENDING_REVIEW.** The card image and title were wrapped in `<Link href={publicListingPath(...)}>` for any status other than DRAFT, but the public listing page returns `notFound()` for HIDDEN/REJECTED/PENDING_REVIEW too. Fix: branch on `isPublicStatus = ACTIVE | SOLD | SOLD_OUT` and append `?preview=1` for the rest. Archived (HIDDEN + isPrivate) listings remain non-linkable. `src/app/dashboard/page.tsx`.
2. **[FIXED 2026-05-10] AI alt-text wasn't filling on edit-page paths.** Created `src/lib/photoAltTextBackfill.ts` exporting `backfillEmptyAltTexts(listingId, altTexts)` — writes to empty `Photo.altText` only, never overwrites seller-provided text, pairs by `sortOrder` ascending, fails non-fatally. Refactored `publishListingAction` to use it. Added the call to all three edit-page AI re-review paths (`updateListing`, `deletePhotoAction`, `replacePhotoAction`) and added `altTexts: [] as string[]` to each catch return so TypeScript can union the success type. Documented as "AI alt-text backfill helper". `src/lib/photoAltTextBackfill.ts`, `src/app/seller/[id]/shop/actions.ts`, `src/app/dashboard/listings/[id]/edit/page.tsx`.
3. **[FIXED 2026-05-10] Lightbox showed the 4:5 cropped image instead of the original.** The upload-time crop modal was forcing every listing photo to 4:5, so storage = 4:5 and lightbox `object-contain` displayed it as 4:5. Removed `cropAspect={4 / 5}` from `PhotoManager` (UploadButton) and `AddPhotosButton`. Listing photos now upload at original aspect; cards and listing-detail main photo CSS-crop via `aspect-[4/5] object-cover`; lightbox shows the full original. Re-crop (`ImageRecropButton` in `PhotoManager` and `EditPhotoGrid`) still defaults to 4:5 so sellers can opt into explicit thumbnail framing. Banner/avatar/workshop uploads keep their force-crop because the thumbnail is the only view there. Existing already-cropped listings retain their cropped storage — the fix applies to NEW uploads only. Documented as "Listing photo upload-time crop behavior". `src/components/PhotoManager.tsx`, `src/components/AddPhotosButton.tsx`.
4. **[FIXED 2026-05-10] After Publish, PENDING_REVIEW listings landed on the edit page's "cannot edit" block instead of showing the seller their listing.** Changed the `updateListing` redirect: PENDING_REVIEW now goes to `${publicListingPath}?preview=1`. The preview banner is status-aware — for PENDING_REVIEW it reads "Under review — your listing will go live once our team approves it." instead of the generic preview message. The unreachable `?saved=pending` banner code on the edit page was removed. Documented as "Edit listing redirect behavior". `src/app/dashboard/listings/[id]/edit/page.tsx`, `src/app/listing/[id]/page.tsx`.
5. **[FIXED 2026-05-10] Header had too much side whitespace and the search bar felt cramped.** Header `<nav>` widened from `max-w-6xl` (1152px) to `max-w-[1600px]` to match the body content. Search bar grew from `max-w-[400px]` to `flex-1 max-w-[640px]` so it has presence on desktop while still leaving room for the right-side nav. Padding stays at `px-4 sm:px-6 lg:px-8`. Documented as "Header layout behavior". `src/components/Header.tsx`.

Verification: `npm test` 470/470 pass (added 4 new test cases — dashboard card link branching, alt-text backfill wiring across all 4 paths, publish PENDING_REVIEW preview redirect + status-aware banner, header max-w/search-bar growth), `npx tsc --noEmit` clean, `npm run lint` clean (existing jsx-ast-utils warnings only), `npm run build` clean, `npm audit --audit-level=moderate` 0 vulnerabilities.
...
2026-05-09 order/detail/gallery launch polish follow-up:

1. **[LOW UI FIXED 2026-05-09] Header avatar rendered as a rounded square.** The global `button` radius rule could override Tailwind `rounded-full` on the avatar trigger; `UserAvatarMenu` now applies an explicit `borderRadius: "9999px"` inline style while keeping the neutral ring/shadow boundary. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
2. **[MEDIUM UX FIXED 2026-05-09] Buyer order detail "Message maker" ignored the computed conversation URL.** The page already computed `messageHref`, but the bottom CTA hardcoded `/messages`; it now opens the existing thread or `/messages/new?to=...` for the maker. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
3. **[LOW-MEDIUM UX FIXED 2026-05-09] Mobile listing gallery swipes competed with page/browser gestures.** The listing gallery main image and lightbox now use `touch-pan-y`, and visible previous/next buttons supplement swipe gestures for discoverability. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
4. **[MEDIUM UX FIXED 2026-05-09] Order confirmation emails could take up to five minutes.** Buyer order-confirmed, seller order-confirmed, and first-sale emails now send directly from the Stripe completed-checkout webhook instead of waiting for the five-minute outbox cron; seller order email still honors `EMAIL_NEW_ORDER`. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
5. **[LOW-MEDIUM A11Y/UX FIXED 2026-05-09] Workshop gallery lacked alt text and reordering.** `SellerProfile.galleryAltTexts` was added as a parallel array to `galleryImageUrls`; `GalleryUploader` now supports alt editing, drag reorder, and arrow reorder, and public `SellerGallery` uses the stored text as image alt attributes. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
6. **[LOW UI FIXED 2026-05-09] Page-wide amber gradients made the site feel less polished.** Header, homepage, browse, and listing-detail wrappers now use the solid warm cream `#F7F5F0` background while keeping photography and smaller amber accent zones for visual warmth. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
7. **[LOW UX FIXED 2026-05-09] Order timelines lacked expectation-setting context.** Timelines now show processing-window text and estimated-delivery text when available, while intentionally avoiding a redundant "Payment confirmed" step. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
8. **[LOW IA FIXED 2026-05-09] Admin "Flagged Orders" sounded like stale checkout-specific tooling.** The queue is still active for `reviewNeeded` orders from refunds, disputes, stock oversells, and label clawback failures, so labels now read "Orders Needing Review" while the `/admin/flagged` URL remains stable. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
9. **[VERIFIED NOT BUG 2026-05-09] Label purchase marking an order as shipped is intentional.** Purchasing a shipping label is the seller shipping commitment; carrier tracking updates follow separately. Leave this behavior unless the fulfillment model changes.
10. **[VERIFIED NOT BUG 2026-05-09] Public buyers should not see visible alt-text captions by default.** Alt text is stored for accessibility and image SEO through `alt` attributes; buyer-visible gallery captions are a separate richer-profile design feature already tracked as deferred.
11. **[DEFERRED REPRO NEEDED 2026-05-09] Checkout address autocomplete still feels unreliable for some addresses.** Current shared code no longer uses county/neighborhood/suburb as city, formats labels cleanly, and shows a no-results hint; reopen with a concrete failing address if Nominatim returns incomplete structured data, then consider a dedicated US address provider.

2026-05-09 listing form/product image polish follow-up:

1. **[LOW-MEDIUM UX FIXED 2026-05-09] Product image crop/display ratios were still inconsistent.** Listing photo uploads, new/edit previews, listing cards, listing detail gallery, related/recent/home/browse listing tiles, and skeletons now use a single 4:5 portrait product ratio. Lightbox stays `object-contain` for full-image inspection. Regression coverage: `tests/upload-ux-followups.test.mjs`, `tests/post-launch-ui-followups.test.mjs`.
2. **[LOW UX FIXED 2026-05-09] Address autocomplete could write county into the city field.** `AddressAutocomplete` no longer falls back to county for city values and strips county fragments from fallback labels. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
3. **[LOW-MEDIUM UX FIXED 2026-05-09] Made-to-order variants exposed an "In stock" checkbox.** Listing type and variants now share client state through `ListingTypeVariantSection`; made-to-order variants hide stock checkboxes and serialize `inStock: true`, while in-stock listings keep per-option stock toggles. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
4. **[LOW-MEDIUM UX FIXED 2026-05-09] Variant price inputs reformatted on every keystroke.** `VariantEditor` now keeps raw price drafts while focused and parses/formats on blur or submit so typing does not jump the cursor or eat digits. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
5. **[MEDIUM UX FIXED 2026-05-09] Enter could accidentally submit listing forms and server-side errors could clear visible input work.** Listing create/edit forms opt into `ActionForm`'s Enter-key guard and error restore behavior, preserving submitted field values after server validation errors. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
6. **[LOW UI FIXED 2026-05-09] Listing shop policies rendered duplicate right-side disclosure triangles.** The explicit right-side `▾` markers were removed; the native left disclosure marker remains. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
7. **[LOW UI FIXED 2026-05-09] Light/white avatars could disappear on white surfaces.** Header, seller profile, seller shop, message, listing seller-card, homepage maker, and commission avatar surfaces now use neutral rings/shadows instead of white rings/borders. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
8. **[VERIFIED NOT BUG 2026-05-09] Custom-order buttons are intentionally hidden from the seller's own view.** Seller profile, listing detail, and messages gate custom-order CTAs on the other party/seller accepting custom orders and not being the current seller. Test with a signed-out session or different buyer account before reopening.

2026-05-09 crop alignment/recrop follow-up:

1. **[MEDIUM UX FIXED 2026-05-09] Multi-photo listing crop batches could freeze on the second image.** `ImageCropModal` now resets processing, zoom, offset, natural size, drag state, and error state whenever the `file` prop changes, so each selected listing photo gets a fresh crop interaction. Regression coverage: `tests/upload-ux-followups.test.mjs`.
2. **[LOW-MEDIUM UX FIXED 2026-05-09] Crop ratios did not match public display ratios.** Public seller banners now display at 3:1 from the `sm` breakpoint up and the profile banner preview matches; workshop photo/gallery previews and public gallery cells now use 3:2; listing uploaders and management previews now use the final 4:5 portrait product-photo system. Regression coverage: `tests/upload-ux-followups.test.mjs`.
3. **[LOW-MEDIUM UX FIXED 2026-05-09] Existing images could not be adjusted without reuploading.** `ImageRecropButton` now supports recropping current banner/avatar/workshop/gallery/listing images by loading the current R2 image, reopening `ImageCropModal`, uploading the replacement crop, and updating the relevant hidden form state or listing photo URL. Active listing recrops go through the existing AI review/PENDING_REVIEW path. Regression coverage: `tests/upload-ux-followups.test.mjs`.
4. **[LOW UX FIXED 2026-05-09] Single-slot image fields could still open a multi-file picker when they reused the `galleryImage` endpoint.** `R2UploadButton` now accepts `allowMultiple`, and avatar, banner, workshop, blog-cover, and generic single-image fields pass `allowMultiple={false}`. Regression coverage: `tests/upload-ux-followups.test.mjs`.
5. **[PRODUCT/UX DEFERRED 2026-05-09] Seller profile could use a richer editorial shop-page treatment.** Potential follow-up: captioned workshop/gallery photos, richer maker-story modules, process timeline, materials/tooling sections, and featured collections. Not included in the crop bugfix batch because it needs a dedicated design pass.
...
3. **[LOW-MEDIUM UX FIXED 2026-05-09] Prominent profile/listing image uploads had no crop/position step.** `ImageCropModal` now supports pre-upload crop/zoom/pan for shop banners (3:1), avatars (1:1), and listing photo uploaders (4:5 portrait), exporting a max-2000px JPEG into the existing R2 upload flow. Chat/message uploads intentionally skip the crop step to avoid friction. Regression coverage: `tests/upload-ux-followups.test.mjs`.
...
1. **[MEDIUM UX FIXED 2026-05-08] Clerk OAuth callbacks could hang after external sign-in flows.** The sign-in and sign-up catch-all App Router pages now use Clerk path routing (`routing="path"` with explicit `path`) while preserving the durable `/accept-terms?redirect_url=...` post-auth gate. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
2. **[MEDIUM UX FIXED 2026-05-08] Stripe onboarding completion could remain visually stale after returning from Stripe.** The onboarding wizard and seller settings Connect buttons now return with `stripe_return=1`; the wizard calls the new authenticated `/api/stripe/connect/status` route to retrieve the Stripe account, persist `chargesEnabled`, and update the checkmark without requiring a second Connect click. Seller settings also refreshes account status on Stripe return. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
3. **[MEDIUM CONTENT INTEGRITY FIXED 2026-05-08] Non-staff blog creation did not require an actual seller profile.** Seller blog publishing remains intentionally allowed before Stripe connection, but non-staff authors must have a real `SellerProfile` and maker-authored posts must write `sellerProfileId` so public visibility/feed filters work. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
4. **[MEDIUM IA FIXED 2026-05-08] Public profile identity fields were split between Shop Settings and Shop Profile.** `/dashboard/profile` is now canonical for display name, bio/story, workshop photo, and workshop gallery; `/dashboard/seller` keeps operational settings only. Gallery clearing remains supported through `galleryImageUrlsTouched`. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
5. **[LOW-MEDIUM UI FIXED 2026-05-08] Old form styling remained on create-blog, account feed empty-state, shop settings, shop profile, FAQ, and guild application fields.** These surfaces now use `card-section`, `font-display` headings, rounded-md controls, explicit neutral borders, consistent focus rings, and consistent checkbox styling. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
6. **[LOW UX FIXED 2026-05-08] Address search/autocomplete existed only in the pickup map flow.** `AddressAutocomplete.tsx` and `usStates.ts` now back checkout shipping, seller ship-from, and pickup-map search with a shared dropdown. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
7. **[LOW UX FIXED 2026-05-08] Blog search could show a right-edge vertical artifact.** `BlogSearchBar` now keeps the clear affordance inside the input area and preserves the clipped submit cap so the search pill has no extra vertical strip. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
8. **[LOW UX FIXED 2026-05-08] Guild verification affordances used a generic sparkle icon and hid the Master path until Member approval.** Dashboard guild links now use the shield icon, the onboarding wizard uses the Store icon instead of the old hammer, and `/dashboard/verification` previews the locked Guild Master badge/requirements for non-members. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
9. **[PRODUCT/LEGAL DEFERRED 2026-05-08] Etsy-style business-registration and sales-tax certificate display is not a code bug.** Recommendation for launch is to skip required display until legal/tax requirements are confirmed; if needed later, implement optional seller-profile fields plus listing/shop/receipt display as a dedicated legal/product change.

2026-05-08 post-launch regression cleanup:

1. **[LOW-MEDIUM UI FIXED 2026-05-08] Shop Profile media uploaders still had old styling after the surrounding form was modernized.** `ProfileBannerUploader`, `ProfileAvatarUploader`, and `ProfileWorkshopUploader` now use explicit neutral borders, white/dashed placeholders, rounded-md neutral upload buttons, and no `bg-black` controls. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
2. **[MEDIUM UX FIXED 2026-05-08] Search dropdowns could disappear when the data-backed popular-tag APIs returned empty results or stale empty cache entries.** `SearchBar` and `BlogSearchBar` now keep small fallback popular-topic lists for pre-launch/empty-data states, and listing/blog publish-state paths call `revalidateListingSearchCaches()` / `revalidateBlogSearchCaches()` so popular-tag caches refresh after public inventory or blog visibility changes. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
3. **[LOW UX FIXED 2026-05-08] Shared address autocomplete felt slow and exposed noisy Nominatim neighborhood/county labels.** `AddressAutocomplete` now starts at two characters, debounces at 350ms, formats suggestions as street/city/state/zip, and no longer uses suburb/neighbourhood as city fallbacks. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
4. **[LOW UX FIXED 2026-05-08] Blog search still showed a right-edge vertical artifact when the clear button appeared.** The clear affordance now sits inside the input area instead of between the input and submit cap, preserving the pill shape. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.
5. **[LOW UI FIXED 2026-05-08] Onboarding wizard primary CTAs used bright amber instead of the site primary-button color.** Wizard CTAs now use neutral/espresso-style primary classes while the progress bar and warning callouts keep amber as an accent. Regression coverage: `tests/post-launch-ui-followups.test.mjs`.

2026-05-08 post-merge regression audit/hotfix pass:

1. **[CRITICAL FIXED 2026-05-08] PR #13 made pre-diagnostics legacy sellers invisible by requiring `stripeAccountVersion === "v2"` in public Prisma filters.** `publicListingWhere()`, `publicListingDetailWhere()`, `visibleSellerProfileWhere()`, and `activeSellerProfileWhere()` now accept `stripeAccountVersion IN (null, "v2")`; in-memory listing visibility and order blockers use the same rule so `null` means a legacy pre-diagnostics account, not an unsupported account. Explicit unsupported strings remain blocked. Regression coverage: `tests/listing-visibility.test.mjs`, `tests/observability-cleanup-followups.test.mjs`, `tests/order-state-followups.test.mjs`, and `tests/stripe-connect-v2.test.mjs`.
2. **[HIGH FIXED 2026-05-08] The merged main branch still contained the dead Connect v2 thin-event branch inside the legacy snapshot webhook route.** `/api/stripe/webhook` is snapshot-only again, with Connect v2 thin events isolated to `/api/stripe/webhook/v2` and `STRIPE_V2_WEBHOOK_SECRET`. This fixes the half-merged state that left `stripe.parseEventNotification()` in the legacy route despite the split-route tests. Regression coverage: `tests/stripe-webhook-v2-route.test.mjs` and `tests/stripe-connect-v2.test.mjs`.

2026-05-08 onboarding wizard step 4 UX cleanup:

1. **[LOW UX FIXED 2026-05-08] Onboarding wizard step 4 rendered a confusing disabled secondary button labelled "Create a listing first" alongside the orange "Create a Listing →" primary action.** The disabled button was actually a "Continue" button hidden behind imperative copy that read like a separate action. Step 4 now shows a clear primary "Create a Listing →" CTA plus a real `Skip for now` secondary button when no listing exists, matching the pattern used in steps 1, 2, and 3. The step 5 server invariant still gates `completeOnboarding()` on `chargesEnabled && listingCount >= 1`, so skipping step 4 cannot bypass the listing requirement. Regression coverage: `tests/pr-e-onboarding-summary-followups.test.mjs` (now also asserts the disabled label is gone and `Skip for now` is present).

2026-05-08 terms clickwrap bypass follow-up:

1. **[HIGH LEGAL FIXED 2026-05-08] The signup path could show a separate pre-Clerk Grainline terms gate and then still require the durable `/accept-terms` gate after account creation.** Clerk sign-in/sign-up completion now routes through `/accept-terms?redirect_url=...`, and `/sign-up` no longer renders or stores a separate pre-Clerk `unsafeMetadata` terms gate. The only Grainline consent surface is the full-page `/accept-terms` route backed by `User.termsAcceptedAt`, `User.termsVersion`, and `User.ageAttestedAt`; middleware continues redirecting signed-in users with missing/current-stale acceptance away from `/`, `/browse`, `/dashboard`, `/account`, `/messages`, and APIs. Regression coverage: `tests/terms-acceptance-enforcement.test.mjs`, `tests/internal-return-url.test.mjs`, and `tests/verified-audit-followups.test.mjs`.

2026-05-08 launch-readiness route/legal/SEO pass:

1. **[CRITICAL FIXED 2026-05-08] Signed-out protected pages could render Clerk's protect-rewrite 404 instead of a Grainline sign-in redirect, and `/cart` was blocked despite anonymous-cart support.** Middleware now returns app-owned responses for missing sessions: non-public pages redirect to `/sign-in?redirect_url=...`, non-public APIs return JSON 401, `/cart(.*)` is public so the anonymous cart/sign-in merge page can render, and `/accept-terms(.*)` is public so the full-page gate can redirect signed-out users to sign-in instead of being rewritten to 404. Signed-in users missing current Terms still cannot use `/cart` because `/cart` is not in `isTermsAcceptanceAllowed`. Regression coverage: `tests/terms-acceptance-enforcement.test.mjs`.
2. **[MEDIUM COMPLIANCE COPY FIXED 2026-05-08] Privacy export copy drifted from implemented export/request behavior.** The Privacy Policy no longer says all data exports are manual and fulfilled within 30 days; it now distinguishes the automated signed-in account export from verified privacy-rights requests handled through the legal request process and promised within the applicable legal timeframe. Regression coverage: `tests/launch-readiness-followups.test.mjs`.
3. **[LOW SEO FIXED 2026-05-08] Search Console setup was only a layout TODO and docs still referenced stale `/sitemap.xml`.** Root metadata now emits Google verification from `NEXT_PUBLIC_GOOGLE_SITE_VERIFICATION` when configured, `.env.example` documents the variable, and `CLAUDE.md` points launch ops to the live `/sitemap_index.xml` advertised by `robots.txt`. Regression coverage: `tests/launch-readiness-followups.test.mjs`.
4. **[MODERATE DEV-TOOLING CVE FIXED 2026-05-08] `npm audit` surfaced Hono advisories through Prisma's dev tooling dependency tree.** `package-lock.json` now resolves transitive `hono` to 4.12.18; app code has no direct Hono import, and `npm audit --audit-level=moderate` returns zero vulnerabilities.
5. **[LAUNCH BUSINESS BLOCKER OPEN 2026-05-08] Terms and Privacy still display `DRAFT — Under Attorney Review`.** This remains intentionally open until attorney sign-off removes the public draft banners; it was not "fixed" in code because removing the warning without sign-off would hide a real legal launch gate.
...
1. **[LOW-MEDIUM FIXED 2026-05-07; CORRECTED 2026-05-08] `stripeAccountVersion` was written for v2 accounts but not read by runtime guards.** The initial fix over-tightened public/sellable filters to `stripeAccountVersion === "v2"` and hid pre-diagnostics legacy sellers. The corrected behavior accepts `stripeAccountVersion IN (null, "v2")` for public visibility, seller visibility, orderability, and Connect dashboard-link routes while still rejecting explicit unsupported strings. Regression coverage: `tests/stripe-connect-v2.test.mjs`, `tests/listing-visibility.test.mjs`, `tests/order-state-followups.test.mjs`, and `tests/observability-cleanup-followups.test.mjs`.
...
2. **[CRITICAL LAUNCH BLOCKER FIXED 2026-05-07] Onboarding could deadlock after profile/shop/listing were complete but Stripe was still incomplete.** The final onboarding summary now renders a primary "Connect Stripe Payouts →" button whenever `chargesEnabled === false`; it reuses the existing `/api/stripe/connect/create` return-to-onboarding flow, while "Go to My Dashboard →" remains disabled until the server-side `completeOnboarding()` invariant can pass. Regression coverage: `tests/verified-audit-followups.test.mjs`.
3. **[TERMS REDIRECT HARDENED 2026-05-07] `/accept-terms` success now performs a full document navigation after the durable DB write.** Recent Vercel production log checks found no `/accept-terms` or recent 500s, so the reported black error page was treated as a client-transition robustness issue. `AcceptTermsForm` now uses `window.location.assign(redirectUrl)` after `POST /api/account/accept-terms` succeeds, and `currentTermsAcceptanceUpdate()` records a fresh timestamp when users accept a newer terms version instead of preserving stale legal evidence. Regression coverage: `tests/terms-acceptance-state.test.mjs` and `tests/verified-audit-followups.test.mjs`.
4. **[HIGH FIXED 2026-05-07] Publishing a new listing without Stripe connected could clear the form.** The server action now returns an inline `PUBLISH_REQUIRES_STRIPE_MESSAGE` instead of redirecting to `?error=stripe`, preserving the uncontrolled form state after rejected publish attempts. The Publish button is disabled when `chargesEnabled === false`, while Save as Draft stays enabled. Regression coverage: `tests/verified-audit-followups.test.mjs`.
5. **[PHASE 2 UX FIXED 2026-05-07] Non-sellers did not have a discoverable "Become a Maker" CTA everywhere they needed it.** `/become-a-maker` is now a public redirect entry that sends signed-out users to sign-up with `redirect_url=/dashboard` and signed-in users to `/dashboard`; `/dashboard` still creates/loads the seller profile and redirects incomplete sellers to onboarding. The footer links to `/become-a-maker`, `/account` shows a non-seller "Become a Maker" CTA, and signed-in non-sellers now see "Start Selling" in both the desktop avatar menu and mobile drawer. Regression coverage: `tests/verified-audit-followups.test.mjs`.
6. **[VISUAL POLISH FIXED 2026-05-07] Onboarding wizard pages used generic gray hard-corner panels.** `/dashboard/onboarding` now uses the Grainline warm page background, `card-section` panels, `font-display` headings, rounded-md action controls, rounded status banners, and a styled final-summary Stripe reconnect CTA. This is a focused fix for the walkthrough-blocking wizard surface, not a wholesale site-wide redesign. Regression coverage: `tests/verified-audit-followups.test.mjs`.

2026-05-06 order-state/case-resolution follow-up pass closed these verified findings:

1. **[MEDIUM FIXED 2026-05-06] `acceptingNewOrders` was UI-only and did not block purchases.** `src/lib/sellerOrderState.ts` now centralizes seller order-block reasons, and cart add, buy-now checkout, seller cart checkout, shipping quotes, and custom-order requests all reject sellers with `acceptingNewOrders === false` before cart mutation, Shippo quote, or Stripe session creation. Listing detail purchase controls also use the same server-backed availability state. Regression coverage: `tests/order-state-followups.test.mjs`.
2. **[MEDIUM FIXED 2026-05-06] Staff case resolution used a stale read followed by an unconditional final update.** `POST /api/cases/[id]/resolve` now writes the final case resolution with an atomic `updateMany` precondition requiring the case to remain unresolved, while the refund path still keeps the existing order refund lock before Stripe calls. Concurrent staff/user resolution attempts now get a 409 instead of overwriting a terminal case. Regression coverage: `tests/order-state-followups.test.mjs`.
3. **[LOW-MEDIUM FIXED 2026-05-06] Full case refunds persisted `null` as `Case.refundAmountCents`.** Case resolution now stores the computed `refundAmountForOrder` for full and partial refunds, and uses that amount in case email/audit copy so admin reconciliation displays the actual refund amount. Regression coverage: `tests/order-state-followups.test.mjs`.
4. **[LOW-MEDIUM FIXED 2026-05-06] Shipping quotes could call Shippo for listings checkout would later reject.** `/api/shipping/quote` now mirrors checkout availability checks for listing status, private reservations, self-purchase, inactive/vacation/not-accepting sellers, payment readiness, made-to-order quantity, and stock before returning signed rates. Regression coverage: `tests/order-state-followups.test.mjs`.
5. **[LOW-MEDIUM FIXED 2026-05-06] First cart add and quantity cap logic had read-then-create/update races.** `/api/cart/add` now creates the cart with `cart.upsert`, handles first-item unique races, and increments existing cart items through `updateMany` with a quantity guard so concurrent adds cannot exceed the 99-item cap. Regression coverage: `tests/order-state-followups.test.mjs`.
6. **[LOW FIXED 2026-05-06] `CaseResolutionPanel` could leave the admin UI stuck on network or non-JSON failures.** The component now wraps resolve calls in `try/catch/finally`, parses response text defensively, shows a network error, and clears loading state. Regression coverage: `tests/order-state-followups.test.mjs`.
7. **[LOW OBSERVABILITY FIXED 2026-05-06] Seller-cart checkout silently rejected tampered shipping-rate tokens.** `/api/cart/checkout-seller` now logs bounded `token_rejected` security events on 400 verification failures, matching the single-checkout route without storing raw token material. Regression coverage: `tests/order-state-followups.test.mjs`.

2026-05-06 extended audit-only sweep found these verified follow-ups; the 2026-05-06 route/docs follow-up pass closed them:

1. **[MEDIUM FIXED 2026-05-06] Seller profile view tracking endpoint is not public in middleware.** `/api/seller/([^/]+)/view` is now in the public matcher, while signed-in suspended/deleted users still hit the existing suspended-account API guard. Regression coverage: `tests/verified-audit-followups.test.mjs`.
2. **[MEDIUM FIXED 2026-05-06] `activeSellerProfileWhere()` conflates visible sellers with currently sellable sellers.** `src/lib/sellerVisibility.ts` now exposes `visibleSellerProfileWhere()` for public profile/follow/view surfaces while keeping `activeSellerProfileWhere()` as the stricter sellable predicate. Follow GET/POST, account following, and profile-view tracking use the visible predicate. Regression coverage: `tests/observability-cleanup-followups.test.mjs`.
3. **[MEDIUM FIXED 2026-05-06] `/seller/payouts` is treated as a public seller page even though it is a seller operations surface.** Middleware now narrows public seller profile/shop matchers instead of allowing all `/seller(.*)`, `/seller/payouts` redirects to `/dashboard/seller`, and Stripe Connect default refresh/return URLs now use `/dashboard/seller`. Regression coverage: `tests/verified-audit-followups.test.mjs`.
4. **[MEDIUM FIXED 2026-05-06] Global search blog suggestions can return non-public blog posts.** `/api/search/suggestions` now mirrors the public blog raw-SQL seller-profile predicate and excludes blocked seller profile IDs for blog suggestions. Regression coverage: `tests/verified-audit-followups.test.mjs`.
5. **[MEDIUM FIXED 2026-05-06] Account following empty-state CTA links to a missing `/sellers` page.** The CTA now links to `/map`, which is an existing maker discovery surface. Regression coverage: `tests/verified-audit-followups.test.mjs`.
6. **[LOW-MEDIUM FIXED 2026-05-06] Dashboard analytics "View all" links to missing `/dashboard/listings`.** The analytics "View all" link now points to `/dashboard/inventory`, the existing listing management surface. Regression coverage: `tests/verified-audit-followups.test.mjs`.
7. **[LOW FIXED 2026-05-06] `/seller/map` is a blank public 200.** The stale singular route now redirects to `/map`. Regression coverage: `tests/verified-audit-followups.test.mjs`.
8. **[LOW COMPLIANCE/OBSERVABILITY FIXED 2026-05-06] Account export audit logging is still best-effort, not durable.** Account export now checks the audit-row id returned by `logUserAuditAction()` and fails closed with 503 if the audit trail cannot be persisted. Regression coverage: `tests/observability-cleanup-followups.test.mjs`.
9. **[LOW-MEDIUM FIXED 2026-05-06] Case escalation route documents CRON_SECRET access that middleware blocks.** `/api/cases/([^/]+)/escalate` is now public at middleware and explicitly geo-allowed so the route-level session-or-CRON_SECRET checks can run as documented. Regression coverage: `tests/verified-audit-followups.test.mjs`.
10. **[LOW DOC FIXED 2026-05-06] `CLAUDE.md` still contains stale fixed-interval NotificationBell polling docs.** The stale component bullet, older polling note, and Stripe Connect fallback docs now match current adaptive polling and `/dashboard/seller` return behavior. Regression coverage: `tests/verified-audit-followups.test.mjs`.

2026-05-06 post-fix audit-only pass found these verified items; fixed entries remain here for traceability while the live count above tracks what remains open:

1. **[HIGH FIXED 2026-05-06] Full Stripe refunds with tax could under-reverse seller transfers.** `src/lib/marketplaceRefunds.ts` now uses one full-charge Stripe refund with `reverse_transfer: true` for connected-seller full refunds under the manual `transfer_data.amount` checkout model, and no longer sets `refund_application_fee` because checkout does not create an `application_fee_amount`. Stripe test-mode replay confirmed the original bug and fix: the old split path on `pi_3TUA3l7dFPHZMTng12DFGuZp` / transfer `tr_3TUA3l7dFPHZMTng1EkpK9Uy` refunded 11,325 cents but reversed only 9,272 of the 10,000-cent seller transfer, leaving 728 cents unreversed; the single full-refund path on `pi_3TUA3t7dFPHZMTng02fepZxK` / transfer `tr_3TUA3t7dFPHZMTng0rgep0Po` reversed 10,000 of 10,000 cents. `tests/marketplace-refunds.test.mjs` now asserts the single-refund model, gift-wrap inclusion, partial proportional reverse transfers, and the helper-level order-total cap.
2. **[MEDIUM FIXED 2026-05-06] Blog public visibility rules were inconsistent across page/API surfaces.** `src/lib/blogVisibility.ts` now owns `publicBlogPostWhere()`, and the public blog page/detail/API list/comments/save/sitemap paths use that helper or equivalent raw-SQL seller-profile predicates. Tag cloud, popular blog tags, blog suggestions, and author suggestions now exclude posts/sellers whose author or seller profile is banned, deleted, vacationing, or not charges-enabled. Regression coverage: `tests/blog-visibility.test.mjs`.
3. **[MEDIUM FIXED 2026-05-06] `SOLD_OUT` listing UX was internally unreachable.** `src/lib/listingVisibility.ts` now separates ACTIVE-only `publicListingWhere()` from `publicListingDetailWhere()` / `isPublicListingDetail()`, allowing public `SOLD_OUT` detail pages and notify-me subscriptions while keeping browse/sellable surfaces ACTIVE-only. Regression coverage: `tests/listing-visibility.test.mjs`.
4. **[MEDIUM FIXED 2026-05-06] Favorites could be created for non-public listings and then leak into personalized surfaces.** `POST /api/favorites` now validates the target listing with `publicListingDetailWhere()` before upsert/notification, and account dashboard/full saved listing queries share `savedListingFavoriteWhere()` so hidden/private/rejected/banned-seller listings do not render from saved state. Regression coverage: `tests/public-visibility-followups.test.mjs`.
5. **[MEDIUM FIXED 2026-05-06] Seller workshop gallery could not be cleared.** `GalleryUploader` now submits `galleryImageUrlsTouched=1`, and the seller settings action persists the filtered array whenever that sentinel is present, including an empty array. Regression coverage: `tests/public-visibility-followups.test.mjs`.
6. **[LOW-MEDIUM FIXED 2026-05-06] Public listing view/click tracking increments non-public listings.** Listing view and click endpoints now apply `publicListingWhere()` inside the counter `updateMany()` guard and skip aggregate cookies/daily rows when the listing is not currently public, so known IDs for hidden/private/rejected listings cannot inflate seller analytics or future relevance signals. Regression coverage: `tests/observability-cleanup-followups.test.mjs`.
7. **[LOW-MEDIUM FIXED 2026-05-06] Follow state can include inactive or hidden sellers.** `src/lib/sellerVisibility.ts` now owns `activeSellerProfileWhere()`. Follow GET/POST use that predicate for public seller state, DELETE still allows stale-row cleanup, and the account following page filters followed sellers plus latest listing/count data through active seller/public listing predicates. Regression coverage: `tests/observability-cleanup-followups.test.mjs`.
8. **[LOW-MEDIUM FIXED 2026-05-06] Gift wrapping is still omitted on two staff reconciliation surfaces.** `admin/flagged` and `admin/cases/[id]` now use `orderTotalCents()` for staff totals, and case detail selects/displays a gift-wrapping row when present. Regression coverage: `tests/observability-cleanup-followups.test.mjs` plus the static order-total guard in `tests/verified-audit-followups.test.mjs`.
9. **[LOW-MEDIUM FIXED 2026-05-06] Seller profile view analytics are inflated by render-time increments.** Seller profile pages no longer mutate `profileViews` during server render. `SellerProfileViewTracker` calls `/api/seller/[id]/view`, which rate-limits, skips likely bots, skips owners, requires active seller visibility, and dedupes by tracking cookies before incrementing profile views. Regression coverage: `tests/observability-cleanup-followups.test.mjs`.
10. **[LOW IMPROVEMENT FIXED 2026-05-06] Seller profile rating still does an on-demand review aggregate.** Seller profile pages now read `SellerRatingSummary` through `getSellerRatingMap()` instead of running a seller-wide review aggregate on each render. Regression coverage: `tests/observability-cleanup-followups.test.mjs`.
11. **[LOW COMPLIANCE/OBSERVABILITY FIXED 2026-05-06] Account export has no durable fulfillment audit trail.** `/api/account/export` now writes a low-detail `ACCOUNT_EXPORT` audit action after building the export, recording route and method without storing the exported payload. `src/lib/audit.ts` lazy-loads Stripe only for ban-undo rollback work so account-export audit logging does not import the Stripe client at module load. Regression coverage: `tests/observability-cleanup-followups.test.mjs`.
12. **[ARCHITECTURE FIXED ON FEATURE BRANCH 2026-05-06] Stripe Connect onboarding still uses legacy Express account creation.** `feature/stripe-connect-v2` replaces legacy `type: "express"` account creation with Accounts v2 `/v2/core/accounts` creation through `src/lib/stripeConnectV2.ts`, records nullable account-version diagnostics on `SellerProfile`, handles Accounts v2 `v2.core.account[...]` thin events for `chargesEnabled` mirroring, and preserves the existing destination-charge/platform-retains-tax checkout model. Merge remains gated on the Stripe test-mode onboarding, purchase, refund, dispute, disable, and label-clawback verification checklist.

2026-05-06 verified follow-up fix pass closed four audit-only code findings plus the Stripe docs drift: `/messages/new` now blocks inactive/self/blocked recipients and validates `contextListingId` through active listing/seller state before attaching it to a conversation; commission close/fulfill and seller-interest writes now use atomic OPEN/non-expired predicates to avoid stale terminal-state races; order total displays, seller refund caps, analytics snippets, and transactional order emails now route through `orderTotalCents()` so gift wrapping is included consistently; delivered/picked-up buyer order pages can open damaged/not-as-described/wrong-item cases while suppressing the no-longer-valid `NOT_RECEIVED` reason; and `CLAUDE.md` now documents the current Stripe API version pin. Regression coverage: `tests/conversation-start-state.test.mjs`, `tests/commission-state.test.mjs`, `tests/order-totals.test.mjs`, and `tests/verified-audit-followups.test.mjs`.

2026-05-06 label-clawback follow-up pass closed the final code finding from the audit-only sweep: label-cost clawback failures now enter the existing durable admin reconciliation path by setting `Order.reviewNeeded` with a bounded label-clawback note for both missing Stripe transfer IDs and failed Stripe transfer reversals. The route still captures Sentry evidence, but staff now get a persistent `/admin/flagged` order instead of only an ephemeral log. Regression coverage: `tests/label-clawback-state.test.mjs` plus the static route guard in `tests/verified-audit-followups.test.mjs`.

2026-05-06 audit-only sweep found seven verified follow-ups and made no application-code changes. Code findings: **[FIXED 2026-05-06]** `/messages/new` trusted arbitrary recipient/listing context and could attach private/inactive/unrelated listings to conversations; **[FIXED 2026-05-06]** commission request PATCH could rewrite terminal `CLOSED`/`FULFILLED` state and the interest route could race a close into a stale interest/conversation; **[FIXED 2026-05-06]** gift-wrapping charges were omitted from several buyer/seller/admin order totals and from the seller refund UI max even though checkout/refund math included them; **[FIXED 2026-05-06]** buyer order detail hid the case form for `DELIVERED`/`PICKED_UP` orders despite the API allowing damaged/wrong/not-as-described cases; **[FIXED 2026-05-06]** label-cost transfer reversal failures were Sentry-only and now create durable admin reconciliation notes. Improvement/documentation items: **[FIXED 2026-05-06]** Stripe client/docs drift (`src/lib/stripe.ts` pins `2025-10-29.clover` and `CLAUDE.md` now matches) and Connect onboarding still creates legacy Express accounts, which should be evaluated in a dedicated Stripe modernization pass rather than mixed into bug fixes.
...
2026-05-05 payment webhook/refund pass closed six broad payment-path findings after re-verification: seller/case refund lock acquisition now atomically excludes concurrently inserted open dispute ledger rows and reports those conflicts explicitly, seller refund attempts use same-request stale-lock release results instead of stale in-memory order state, closed/lost Stripe disputes clear stale refund-lock timestamps, checkout webhooks log Stripe/cart/live price drift without changing Stripe-authoritative order data, old multi-seller cart-cleanup and completed-vs-expired webhook race claims were verified stale under the current transaction/advisory-lock flow, and checkout lock expiry was verified handled by the shared stock-restore helper.

2026-05-05 checkout revalidation/refund-total pass closed five more broad payment findings: completed checkout webhooks now lock and re-read buyer and seller account/Stripe-connect state inside the session advisory-lock transaction before `Order.create`, blocked-checkout auto-refunds use that transaction-revalidated invalid state, Stripe refund ledger metadata now records order totals and flags over-refund totals for staff reconciliation before fulfillment, the old pending refund-lock orphan finding was verified fixed by timestamped locks plus route/cron cleanup, and the notification dedup critical was verified stale under the current per-UTC-day dedup key + unique constraint path.

2026-05-05 listing publish/re-review pass closed two listing-state findings: `publishListingAction()` now blocks in-stock listings with no stock before AI review, guards every AI result write including the fail-closed catch path by the originally read status and `updatedAt`, requires positive stock on in-stock ACTIVE/HOLD writes, and keeps the same post-activation ACTIVE-to-SOLD_OUT correction used by listing edit/photo flows as a defensive backstop.

2026-05-05 checkout stock-restore verification pass closed one stale finding without code changes: the cart expiration "over-restore" scenario was re-audited against the current reservation model and is correct. Checkout decrements stock at Stripe Session creation, so a failed/expired session must add back exactly its own reservation even if other buyers purchased separate stock meanwhile. Restoration is session-idempotent, advisory-locked, skipped once an order exists for that session, and prefers Stripe immutable line items / `reservedStock` metadata over mutable cart state.

2026-05-05 multi-seller receipt-email verification pass closed the final mechanical queue item as product/design rather than a live bug: multi-seller cart checkout intentionally creates one Stripe Checkout Session, payment, order, seller notification, and transactional buyer order receipt per seller. The success page already consolidates `session_ids` into one buyer-visible receipt. A single consolidated email would require a first-class checkout-group model so webhooks can know when all seller sessions in the batch are complete; do not fake this with page-render side effects or by suppressing legally useful per-order receipts.

2026-05-05 R49 buyer-flow follow-up pass closed three verified medium findings plus same-pattern adjacent endpoints: review creation now re-checks the seller account state before seller notifications/email so a concurrent ban/deletion cannot receive new review side effects, block follow-cleanup failures now emit console evidence instead of disappearing silently, and signed-in user-specific GET routes (`/api/messages/unread-count`, `/api/follow/[sellerId]`, `/api/search/suggestions`, `/api/account/feed`, `/api/listings/recently-viewed`) use `ensureUserByClerkId()` plus `accountAccessErrorResponse()` before returning personalized data. Regression coverage in `tests/r49-account-state-routes.test.mjs` keeps those guardrails from drifting.

2026-05-05 R56-R67 small follow-up pass closed the confirmed low-risk UI/state/config/dead-code items from the extended re-audit: `markSoldAction()` now uses an atomic `Listing.updateMany()` status precondition, review submit is single-flight and its remove-photo control meets larger touch-target sizing, case replies surface network failures without leaving a permanent sending state, broadcast/thread/photo reorder fetch-like lifecycles abort or ignore post-unmount updates, the Buy Now modal no longer uses the disallowed stone card surface, blog copy-link failures show toast feedback, `.env.example` documents optional `CRON_SECRET_PREVIOUS`, the unused `packing.ts` helper was deleted, and the unused direct `@react-email/render` dependency was removed. Regression coverage in `tests/r56-r67-small-fixes.test.mjs` keeps the small guardrails from drifting.

2026-05-05 R65 observability follow-up pass closed the confirmed incremental logging gaps: cart GET/add/update unexpected failures now capture tagged Sentry exceptions after console logging, checkout/single records invalid shipping-rate token rejections without storing raw tokens or normal expired-rate noise, unsubscribe invalid-token attempts emit bounded Sentry evidence, review attempts against banned/deleted seller accounts and failed admin PIN challenges flow through `logSecurityEvent()`, and Stripe signature failures no longer double-report both an exception and duplicate message while still updating webhook failure-spike accounting. Regression coverage in `tests/r65-observability-guardrails.test.mjs` keeps these signals from drifting.

2026-05-06 R62 performance follow-up pass closed the safe high-traffic page findings without broad cache changes: listing detail seller-rating display now reads `SellerRatingSummary` through `getSellerRatingMap()` instead of aggregating all seller reviews on every listing render, and remaining homepage/browse/listing raw image tags on hot surfaces carry intrinsic dimensions to reduce layout shift. Whole-page ISR/cache proposals for homepage/browse were intentionally not applied because those pages mix public data with auth, block filters, saved-state, and personalized sections; cache work there needs per-query/per-segment isolation rather than a blanket page cache. Regression coverage in `tests/r62-performance-guardrails.test.mjs` keeps the safe performance guardrails from drifting.
...
- **Full refunds now include gift wrapping.** Seller and admin refund paths include `Order.giftWrappingPriceCents` in the full-refund total, and marketplace refund accounting treats gift wrap as seller-reversible pre-tax value inside the single full-charge reverse-transfer refund.
...
- **Current state**: Fixed, then superseded on 2026-05-06. Full marketplace refunds now use one full-charge `reverse_transfer` refund when seller transfer reversal is available because Stripe test-mode replay proved the earlier split item/shipping + tax model under-reversed manual `transfer_data.amount` transfers. Disconnected sellers still use a platform-funded refund path with a manual reconciliation note.
...
- **Current state:** Fixed. `orderRefundTotalCents()` includes gift wrapping, both seller/admin refund routes pass it to `createMarketplaceRefund()`, and marketplace refunds include gift wrapping in the seller-reversible portion of the single full-charge reverse-transfer refund.
...
20. **[FIXED/VERIFIED 2026-05-05] Per-seller webhook race on multi-seller cart cleanup** — Current cart checkout order creation and seller-scoped `cartItem.deleteMany()` run in the same transaction after `lockCheckoutSessionMutation()`. If the transaction fails, neither the order nor cleanup commits and Stripe retries the webhook; if it commits, seller cart rows are removed before side effects.
...
23. **[DESIGN / NOT A BUG 2026-05-05] 3 separate confirmation emails** — Multi-seller checkout creates independent Stripe Checkout Sessions, payments, `Order` rows, seller fulfillment timelines, seller notifications, and buyer transactional receipts. That makes one buyer confirmation per seller order accurate and legally useful under the current model. The buyer-facing success page already consolidates `session_ids` into a multi-order receipt. A single consolidated cart email is a future product/design change that should start with a durable checkout-group model; it should not be bolted onto independent webhook completions or page-render side effects.
...
4. **[FIXED 2026-04-29; ACCOUNTING UPDATED 2026-05-06] Refund route does NOT refund `giftWrappingPriceCents`** — Seller/admin full refunds include `Order.giftWrappingPriceCents`; marketplace refund accounting includes gift wrapping in the seller-reversible pre-tax portion of the single full-charge reverse-transfer refund.
...
14. **[FIXED 2026-04-30; SUPERSEDED 2026-05-06] `marketplaceRefunds.ts:151-156` partial-failure orphan-refund DB write silently drops** — seller and case refund orphan-record writes capture Sentry failures when post-Stripe DB reconciliation or lock release fails. The helper-level partial-failure class was removed when marketplace refunds became single-step; route-level orphan handling still records Stripe refund IDs once a refund call has returned.
...
14. Refund lock release on post-Stripe rollback — `refund/route.ts:226-247`. Older code used helper-level partial-failure errors; current 2026-05-06 refund accounting is single-step, so only route-level post-refund DB failures can orphan a returned Stripe refund ID. **Fix**: rescue path falls back to durable manual reconciliation once a refund ID exists.
...
16. **[FIXED 2026-05-05] Stale `sellerRefundLockedAt` not cleared by dispute webhook** — `chargeDisputeLedgerState()` now includes `sellerRefundLockedAt: null` for `charge.dispute.closed` / terminal dispute statuses, and seller refund attempts apply same-request stale-lock release results before checking the in-memory order state. Closed/lost disputes no longer leave stale local refund-lock timestamps that require an extra buyer/seller retry.

17. **[FIXED 2026-05-05] Webhook trusts Stripe `line_items.price.unit_amount` blindly** — Checkout webhooks now call `checkoutPriceDriftState()` for cart and single-listing checkouts, logging Stripe-vs-checkout unit amount mismatches and checkout-vs-current `priceVersion` drift to Sentry while still preserving Stripe as the payment source of truth.
...
- `createMarketplaceRefund()` in `src/lib/marketplaceRefunds.ts` now uses one full-charge reverse-transfer refund for connected-seller full refunds; Stripe test-mode replay on 2026-05-06 superseded the older split-tax model.
...
🔴 **CRITICAL (2)**
1. **[STALE 2026-05-01] `application_fee_amount` NOT used — CLAUDE.md docs are wrong** — verified stale: CLAUDE.md line 3946 already explicitly documents the implicit-fee design ("`application_fee_amount` is intentionally not used with this manual-transfer model; adding it on top of `transfer_data.amount` would change Connect accounting and must be treated as a separate payment-design migration"), and both checkout routes carry inline `// Do not also set application_fee_amount with this manual transfer model.` comments. Code and docs are aligned; the original finding was based on incomplete reading.
...
🔴 **CRITICAL (4)**
1. **[FIXED/VERIFIED 2026-05-05] Refund "pending" lock orphans on Stripe hang** — Seller and case refunds now set `sellerRefundLockedAt` when claiming the `"pending"` sentinel, `releaseStaleRefundLocks()` clears pending locks with missing/old timestamps, seller refund attempts apply the cleanup result before checking the loaded order state, case resolution runs cleanup before loading the case, and `notification-prune` runs the same cleanup through cron.
2. **[FIXED/VERIFIED 2026-05-05] checkout.session.completed + .expired race** — Completed and expired/async-failed paths both go through `lockCheckoutSessionMutation()` for the Stripe session ID; expired stock restoration also re-checks `Order.stripeSessionId` after taking that lock and uses a durable `checkout-stock-restore:<session>` marker.
3. **[FIXED/VERIFIED 2026-05-05] Two concurrent .completed events double-create OrderItems via P2002 catch** — Current completed handling re-checks existing orders inside the session advisory lock and queues post-payment side effects only after the transaction returns an order. Duplicate deliveries enqueue idempotent side effects for the existing order and return success without creating duplicate items.
4. **[FIXED/VERIFIED 2026-05-05] Stock SOLD_OUT non-atomic check** — Current completed paths use atomic SQL `UPDATE "Listing" SET status = 'SOLD_OUT' WHERE ... "stockQuantity" <= 0 AND status = 'ACTIVE'`, so there is no separate read/update window for SOLD_OUT status.
...
6. **[FIXED 2026-05-05] chargesEnabled flips false during checkout** — Cart and single-listing `checkout.session.completed` handlers now lock and re-read buyer and seller state inside the session-locked transaction immediately before `Order.create`. If `chargesEnabled` or `stripeAccountId` changed after checkout-session creation, the order is marked for review and the blocked-checkout refund path runs from the transaction-revalidated state.
7. **[FIXED 2026-05-05] dispute.created races seller refund** — Seller refund and case-resolution refund lock acquisition now use `blockingRefundOrDisputeLedgerWhere()` in the atomic `Order.updateMany()` predicate, so a concurrently inserted open dispute ledger row prevents claiming `sellerRefundId="pending"` even if the earlier pre-check raced. The post-failure fresh-order read also includes dispute ledger rows, so the route returns the dispute-specific 409 instead of a generic state-change response.
8. **[FIXED/VERIFIED 2026-05-05] Notification dedup is non-atomic — actually MISSING** — The finding is stale/incorrect under current code: `createNotification()` writes a hashed `notificationDedupKey()`, `Notification` has `@@unique([userId, type, dedupKey])`, and P2002 returns the existing row. The actual behavior is per-UTC-day dedup, not rolling 24-hour dedup.
9. **[FIXED 2026-05-05] publishListingAction races admin reject** — `publishListingAction()` already guarded normal AI-result writes by `updatedAt`; this pass added the original `status` guard and applied the same `status` + `updatedAt` guard to the fail-closed catch path, so a concurrent admin review/reject cannot be overwritten by a late seller resubmit/AI result.
10. **[FIXED 2026-05-05] Stock zero by seller while AI re-review running** — Listing edit/photo/new/custom flows already ran ACTIVE-to-SOLD_OUT correction after AI approval. This pass added the missing `publishListingAction()` stock gate for in-stock listings with no quantity, requires positive stock on in-stock ACTIVE/HOLD writes, and keeps the same post-activation `UPDATE ... status='SOLD_OUT' ... stockQuantity <= 0` correction for seller-publish AI approvals.
11. **[FIXED/VERIFIED 2026-05-05] fulfillment shipped + parallel case opened** — Current fulfillment mutations use `Order.updateMany()` with a relation guard that only updates when no active case exists (`OPEN`, `IN_DISCUSSION`, `PENDING_CLOSE`, `UNDER_REVIEW`) and no blocking refund ledger exists. Side effects fire only after `count === 1`.
12. **[FIXED/VERIFIED 2026-05-05] PICKED_UP marked while case OPEN** — Same fulfillment relation guard now covers pickup transitions as well as shipping transitions; pickup/shipping side effects are gated after the guarded update succeeds.

🟡 **MEDIUM (8)**
13. **[NOT A BUG/VERIFIED 2026-05-05] Cart .expired over-restores stock** — Re-audited against current checkout semantics. Stock is decremented at Stripe Session creation time, not at webhook completion time, so an expired/failed session adding back its own reserved quantity is correct available-stock math even if another buyer bought separate stock while the reservation was open. `restoreUnorderedCheckoutStockOnce()` is guarded by `checkout-stock-restore:${sessionId}`, a checkout-session advisory lock, and an `Order.stripeSessionId` existence check; expired webhooks retrieve Stripe line items and fall back to immutable `reservedStock` metadata before mutable cart rows.
14. **[FIXED 2026-05-05] Guild revoke cron + admin reinstate race** — `revokeMember()` now builds its `SellerProfile.updateMany()` guard from the exact revocation reason. Listing-threshold revokes require `listingsBelowThresholdSince < thirtyDaysAgo` at write time, and unresolved-case revokes require an active old seller case through the seller relation, so an admin reset/reinstate or concurrent case resolution makes the cron update a no-op.
...
16. **[FIXED 2026-05-05] account.updated arrives during .completed** — Same fix as #6: completed checkout order creation now row-locks and revalidates buyer and seller state inside the session advisory-lock transaction, so an `account.updated` / `account.application.deauthorized` event that lands after the pre-transaction read is reflected in the created order's review/refund decision.
17. **[FIXED/VERIFIED 2026-05-05] checkoutLock TTL outlives Stripe session** — `restoreUnorderedCheckoutStockOnce()` now releases the matching `checkoutLockKey` after expired/async-failed stock restoration. The Redis lock release is session-bound, so abandoned sessions do not keep buyers blocked for the remaining TTL after Stripe expiry.
18. **[FIXED 2026-05-05] PARTIAL refund + dispute_resolved race overpays** — `charge.refunded` handling now selects the order amount fields, stores `orderTotalCents` and `refundExceedsOrderTotal` in refund ledger metadata, and writes an explicit staff reconciliation review note when Stripe's cumulative refunded total exceeds the order total. The durable refund ledger continues to block later fulfillment transitions.
19. **[FIXED 2026-05-05] case-auto-close cron fires during message** — Case-message creation now runs in an interactive transaction that first `updateMany()`-guards the case by the status read before the message. If cron or another actor changes the case status first, the message request returns 409 and does not create a message on a resolved/closed case.
20. **[FIXED/VERIFIED 2026-05-05] mark-resolved double-click duplicates notifications** — Current mark-resolved uses a single guarded SQL update with active-status predicates and returns state/copy only; it does not create notifications on this path, so the duplicate-notification finding is stale.

**✏️ CORRECTION (Round 19, reconciled 2026-05-05)**: R18 agent claim about missing notification dedup was **WRONG**. Dedup is implemented via `notificationDedupKey()` using a per-UTC-day stable action/link key, backed by the `Notification` unique constraint + P2002 catch returning the existing row. The accepted limitation is UTC-day bucket behavior, not a rolling 24-hour window.
...
33. **[FIXED 2026-04-30] 9 subject lines have emoji** during domain warmup — built-in transactional email subject lines now remove emoji/non-ASCII symbols while preserving the message meaning. Verified with a subject-line regex over `src/lib/email.ts`.
...
108. **[FIXED 2026-04-30; HARDENED 2026-05-14] Recently Viewed: no rate limit on public endpoint** — the public endpoint now uses IP-keyed fail-closed `safeRateLimit` before reading listing IDs.
