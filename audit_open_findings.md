...
12. **[ARCHITECTURE FIXED ON FEATURE BRANCH 2026-05-06] Stripe Connect onboarding still uses legacy Express account creation.** `feature/stripe-connect-v2` replaces legacy `type: "express"` account creation with Accounts v2 `/v2/core/accounts` creation through `src/lib/stripeConnectV2.ts`, records nullable account-version diagnostics on `SellerProfile`, and preserves the existing destination-charge/platform-retains-tax checkout model. Merge remains gated on the Stripe test-mode onboarding, purchase, refund, dispute, disable, and label-clawback verification checklist.
