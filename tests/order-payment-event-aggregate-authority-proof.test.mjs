...
} from "../scripts/order-payment-event-aggregate-authority-postgres-proof.mjs";

const source = readFileSync(
  "scripts/order-payment-event-aggregate-authority-postgres-proof.mjs",
  "utf8",
);

describe("OrderPaymentEvent aggregate-authority real PostgreSQL proof", () => {
  it("accepts only separate loopback owner and runtime identities", () => {
    const config = parseOrderPaymentEventAggregateProofConfig({
      ORDER_PAYMENT_EVENT_AGGREGATE_PROOF_DATABASE_URL:
    ...
    assert.throws(() => parseOrderPaymentEventAggregateProofConfig({
      ORDER_PAYMENT_EVENT_AGGREGATE_PROOF_DATABASE_URL:
