...
describe("Bucket B Notification RLS inventory", () => {
  ...
  it("keeps asymmetric writes, staged activation, and blockers explicit", () => {
    ...
    assert.match(strategy, /remaining private-core review must execute each meaningful\s+action variant/);
    assert.match(strategy, /do\s+not dynamically execute every meaningful action variant/);
    ...
    assert.match(plan, /Latest accepted run `29892353264`/);
    assert.match(plan, /exact source\s+`32d18261c0d5e6e818103bd0ba97648f36798861`/);
