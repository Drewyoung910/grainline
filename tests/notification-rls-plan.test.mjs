...
describe("Bucket B Notification RLS inventory", () => {
  ...
  it("keeps asymmetric writes, staged activation, and blockers explicit", () => {
    ...
    assert.match(strategy, /executes all ten granted creation\s+families/);
    assert.match(strategy, /all 26 source-validation branches and their\s+meaningful action variants/);
    assert.match(strategy, /do not dynamically execute every conditional\s+source branch/);
    ...
    assert.match(plan, /Latest accepted run `29890596734`/);
    assert.match(plan, /exact source\s+`d1467b2477e9a11802e12244464f444bc27ef39a`/);
