...
describe("Case, CaseMessage, and attachment authority catalog", () => {
  ...
  it("classifies all 34 remaining references and retains the 46 converted references", () => {
    assert.equal(caseAuthorityReferenceCount(), 34);
    assert.equal(caseAuthorityConvertedReferenceCount(), 46);
    assert.equal(
      caseAuthorityReferenceCount() + caseAuthorityConvertedReferenceCount(),
      80,
    );
    assert.equal(Object.keys(CASE_AUTHORITY_SOURCE_DESTINATIONS).length, 12);
    assert.deepEqual(
