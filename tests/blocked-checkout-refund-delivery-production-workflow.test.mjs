...
test("production staging makes each sealed predecessor the visible leaf", () => {
  ...
  try {
    ...
    remove(ORDER_PARTICIPANT_DETAIL_PROJECTION_MIGRATION);
    remove(ORDER_PARTICIPANT_CURSOR_AUTHORITY_MIGRATION);
    remove(ORDER_PARTICIPANT_SUMMARY_AUTHORITY_MIGRATION);
    remove(ORDER_SELLER_METRICS_AUTHORITY_MIGRATION);
    remove(ORDER_SELLER_ANALYTICS_AUTHORITY_MIGRATION);
    ...
    remove(ORDER_PARTICIPANT_DETAIL_AUTHORITY_MIGRATION);
    remove(ORDER_PARTICIPANT_LIST_AUTHORITY_MIGRATION);
    remove(ORDER_PAYMENT_EVENT_FORCE_MIGRATION);
    remove(ORDER_PAYMENT_EVENT_ACTIVATION_MIGRATION);
