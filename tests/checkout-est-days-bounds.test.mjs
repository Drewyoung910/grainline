...
const paidCheckoutAuthority = fs.readFileSync("docs/rls-drafts/order-paid-checkout-authority.sql", "utf8");
...
describe("checkout shipping estimated-days bounds", () => {
  ...
  it("webhook uses bounded shipping estimated days and exact paid quantities", () => {
    assert.match(stripeWebhookRoute, /parseBoundedPositiveInt\(rawEstDays,\s*7,\s*SHIPPING_ESTIMATED_DAYS_MAX\)/);
    assert.match(stripeWebhookRoute, /Number\.isSafeInteger\(quantity\)/);
    assert.match(paidCheckoutAuthority, /provider_item->>'quantity' !~ '\^\[1-9\]\[0-9\]\{0,2\}\$'/);
  });
});
