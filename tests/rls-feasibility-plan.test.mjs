...
describe("RLS feasibility plan guardrails", () => {
  ...
  it("centralizes Notification owner reads and updates for the second RLS prototype", () => {
    ...
    assert.match(ownerAccess, /export type NotificationOwnerAccessClient = Pick<DbUserContextTransactionClient, "notification">/);
    ...
    assert.match(ownerAccess, /countOwnerNotificationsInContext\(userId, db\)/);
    assert.match(ownerAccess, /skip: \(page - 1\) \* pageSize/);
