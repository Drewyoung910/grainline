...
describe("listing variant selection", () => {
  ...
  it("checks invalid recalculated variant prices before persisting cart snapshots", () => {
    ...
    assert.ok(
      checkoutSingle.indexOf("const unitPriceError = validateVariantUnitPriceCents(unitPriceCents)") < checkoutSingle.indexOf("await createSnapshotSingleCheckoutStockReservation"),
    );
    assert.ok(
