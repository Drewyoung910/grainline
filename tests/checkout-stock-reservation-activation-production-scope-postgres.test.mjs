...
async function seedReviewedSourceConsistency(database) {
...
test("disposable PostgreSQL accepts only the exact activation restart transition", async () => {
  const database = await createLedger();
  try {
    await seedReviewedSourceConsistency(database);
    assert.equal(
      assertReservationActivationProductionScope(
...
test("disposable PostgreSQL rejects a zero-step activation row on restart", async () => {
  const database = await createLedger();
  try {
    await seedReviewedSourceConsistency(database);
    const activation = CATALOG.at(-1);
