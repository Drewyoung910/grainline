...
describe("isolated production migration runner", () => {
  ...
  it("uses an environment-scoped secret and never runs migrations in Vercel builds", () => {
    ...
    assert.match(
      workflow,
      /Verify exact DirectUpload activation migration tree[\s\S]{0,280}SAVED_SEARCH_RLS_DEPLOY_PHASE: direct-upload-activation-reviewed[\s\S]{0,220}Verify exact Case read-mode release bytes[\s\S]{0,180}audit:rls-case-read-mode-candidate[\s\S]{0,220}Verify Conversation and Message authority proof equivalence[\s\S]{0,180}audit:rls-conversation-message-authority-release[\s\S]{0,220}Verify Conversation and Message activation proof equivalence[\s\S]{0,180}audit:rls-conversation-message-activation-release[\s\S]{0,220}Verify Conversation and Message FORCE release bytes[\s\S]{0,180}audit:rls-conversation-message-force-release[\s\S]{0,220}Verify Notification activation proof equivalence[\s\S]{0,180}audit:rls-notification-activation-release[\s\S]{0,220}Verify Notification FORCE release bytes[\s\S]{0,180}audit:rls-notification-force-release[\s\S]{0,240}Verify DirectUpload retirement proof equivalence[\s\S]{0,180}audit:rls-direct-upload-retirement-release[\s\S]{0,240}Verify DirectUpload activation proof equivalence[\s\S]{0,180}audit:rls-direct-upload-activation-release[\s\S]{0,220}Generate Prisma client/,
    );
