...
describe("isolated production migration runner", () => {
  ...
  it("uses an environment-scoped secret and never runs migrations in Vercel builds", () => {
    ...
    assert.match(
      workflow,
      /Verify exact Case activation migration tree[\s\S]{0,220}SAVED_SEARCH_RLS_DEPLOY_PHASE: case-activation-reviewed[\s\S]{0,220}Verify exact Case activation proof equivalence[\s\S]{0,180}audit:rls-case-activation-release[\s\S]*Verify exact Case read-mode release bytes[\s\S]*audit:rls-case-read-mode-candidate[\s\S]*Verify DirectUpload activation proof equivalence[\s\S]*audit:rls-direct-upload-activation-release[\s\S]{0,220}Generate Prisma client/,
    );
