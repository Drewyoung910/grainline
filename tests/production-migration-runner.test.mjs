...
describe("isolated production migration runner", () => {
  ...
  it("uses an environment-scoped secret and never runs migrations in Vercel builds", () => {
    ...
    assert.match(
      workflow,
      /Verify exact Conversation and Message compatibility artifact[\s\S]{0,260}SAVED_SEARCH_RLS_DEPLOY_PHASE: conversation-message-compatibility-reviewed[\s\S]{0,220}Verify Notification activation proof equivalence[\s\S]{0,180}audit:rls-notification-activation-release[\s\S]{0,220}Verify Notification FORCE release bytes[\s\S]{0,180}audit:rls-notification-force-release[\s\S]{0,300}Generate Prisma client/,
    );
