...
async function seedReviewedAuthority(database) {
...
test("disposable PostgreSQL accepts only the exact restart-safe ledger transition", async () => {
  const database = await createLedger();
  try {
    await seedReviewedAuthority(database);
    assert.equal(
      assertReservationSourceConsistencyProductionScope(
...
test("disposable PostgreSQL rejects a zero-step source-consistency row", async () => {
  const database = await createLedger();
  try {
    await seedReviewedAuthority(database);
    const source = CATALOG.at(-1);
