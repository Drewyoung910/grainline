...
test("scope rejects missing, failed, duplicate, unknown, activation, and FORCE rows", () => {
  for (const rows of [
    ...
    accepted.map((row, index) => index === 3 ? { ...row, applied_steps_count: 1 } : row),
    accepted.map((row, index) => index === 3 ? { ...row, finished_at: new Date() } : row),
    accepted.map((row, index) => index === 2 ? { ...row, checksum: "8".repeat(64) } : row),
