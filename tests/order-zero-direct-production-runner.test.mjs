...
test("production workflow remains disabled and does not call the source-only runner", () => {
  const workflow = fs.readFileSync(".github/workflows/order-zero-direct-production.yml", "utf8");
  assert.match(workflow, /^    if: false$/mu);
  assert.doesNotMatch(workflow, /order-zero-direct-production-runner|PRODUCTION_MIGRATION_DIRECT_URL/u);
});
