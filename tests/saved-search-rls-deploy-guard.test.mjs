...
describe("SavedSearch RLS production deploy guard", () => {
  ...
  it("keeps the historical guarded migration command out of Vercel application builds", () => {
    ...
    const {
      ...
      "codex/order-staged-redirect-body-20260924": orderStagedRedirectBodyReviewDeployment,
    ...
    assert.equal(
      orderHostedFenceReviewDeployment,
      false,
      "the exact Order hosted source-fence review branch must remain deployment-disabled",
    );
    assert.equal(
    ...
    assert.equal(
      orderStagedPickupReviewDeployment,
      false,
      "the exact staged Order pickup smoke review branch must remain deployment-disabled",
    );
    assert.equal(
    ...
    assert.equal(
      orderStagedRedirectBodyReviewDeployment,
      false,
      "the exact staged Order redirect-body review branch must remain deployment-disabled",
    );
    assert.equal(
    ...
    assert.equal(
      r2PrivateHealthDeployment,
      false,
      "the exact private R2 health review branch must remain deployment-disabled",
    );
    assert.equal(
