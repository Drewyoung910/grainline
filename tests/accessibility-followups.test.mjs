...
describe("accessibility follow-ups", () => {
  ...
  it("keeps homepage heading order and static hero collage auditable", () => {
    ...
    assert.match(heroMosaic, /const HERO_DESKTOP_TILE_CLASSES =/);
    assert.match(heroMosaic, /const HERO_MOBILE_TILE_CLASSES =/);
    assert.match(heroMosaic, /lg:left-\[10%\] lg:top-\[0%\] lg:h-\[39%\] lg:w-\[30%\]/);
    assert.match(heroMosaic, /lg:left-\[82\.5%\] lg:top-\[13%\] lg:h-\[28%\] lg:w-\[17\.5%\]/);
    assert.match(heroMosaic, /lg:left-\[28%\] lg:top-\[54%\] lg:h-\[46%\] lg:w-\[72%\]/);
    assert.match(heroMosaic, /left-\[33%\] top-\[0%\] h-\[25%\] w-\[67%\]/);
    assert.match(heroMosaic, /left-\[55%\] top-\[71%\] h-\[29%\] w-\[45%\]/);
    assert.doesNotMatch(heroMosaic, /const HERO_TILE_SURFACE_CLASSES =/);
    assert.doesNotMatch(heroMosaic, /absolute -left|absolute -right|grid-cols-|grid-rows-/);
    assert.match(heroMosaic, /inset-x-7 top-3 bottom-0 sm:inset-x-10/);
    assert.match(heroMosaic, /bottom-0/);
    assert.match(heroMosaic, /absolute rounded-\[14px\] bg-\[#F7F5F0\] p-\[3px\]/);
    assert.match(heroMosaic, /className="h-full w-full rounded-lg object-cover"/);
    assert.doesNotMatch(heroMosaic, /ring-\[|ring-stone|ring-neutral|shadow-\[0_8px_22px/);
    assert.match(heroMosaic, /loading=\{index < 5 \? "eager" : "lazy"\}/);
    assert.match(heroMosaic, /fetchPriority=\{index < 3 \? "high" : "auto"\}/);
    assert.match(heroMosaic, /rgba\(247,245,240,0\.88\)_24%/);
    ...
    assert.match(home, /<SearchBar variant="frosted" \/>/);
    ...
    assert.match(home, /bg-white\/24 px-3 py-1 text-xs font-medium text-neutral-700 shadow-sm ring-1 ring-white\/40 backdrop-blur-md/);
    assert.match(searchBar, /className="relative ml-auto mr-auto w-full min-w-0 max-w-lg"/);
    assert.match(searchBar, /variant\?: "default" \| "glass" \| "frosted"/);
    assert.match(searchBar, /isFrosted/);
    assert.match(searchBar, /bg-white\/28 backdrop-blur-lg border-white\/50/);
    assert.match(searchBar, /bg-neutral-950\/90 text-white/);
