...
import {
  ...
  assertOrderPaymentEventTransitionAuthorityPostflightGitState,
  assertOrderPaymentEventTransitionAuthorityRuntimeSnapshot,
  parseOrderPaymentEventTransitionAuthorityPostflightConfig,
  proveOrderPaymentEventTransitionPrivateExecutionDenied,
  writeOrderPaymentEventTransitionAuthorityPostflightEvidence,
} from "../scripts/order-payment-event-transition-authority-production-postflight.mjs";
...
function environment(evidencePath) {
  return {
    NODE_ENV: "production",
    ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_POSTFLIGHT_CONFIRM:
      ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_POSTFLIGHT_CONFIRMATION,
    ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_POSTFLIGHT_RELEASE_COMMIT: COMMIT,
    ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_POSTFLIGHT_MAIN_CI_RUN_ID: "101",
    ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_POSTFLIGHT_INSPECTION_RUN_ID: "102",
    ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_POSTFLIGHT_MIGRATION_RUN_ID: "103",
...
function transitionSnapshot() {
  ...
  return {
    ...
    functions: ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_FUNCTION_IDENTITIES.map(
      (identity) => ({
        identity,
    ...
    projectionAggregate: {
      total_order_count: "2",
      open_dispute_blocked_count: "1",
    },
  };
}

test("transition-authority postflight accepts exact runtime-only bindings", () => {
  const directory = mkdtempSync(path.join(tmpdir(), "ope-transition-postflight-"));
  const evidencePath = path.join(
    directory,
    `order-payment-event-transition-authority-production-postflight-${COMMIT}.json`,
  );
...
test("transition-authority postflight rejects privileged aliases and stale evidence", () => {
  const directory = mkdtempSync(path.join(tmpdir(), "ope-transition-postflight-"));
  const evidencePath = path.join(
    directory,
    `order-payment-event-transition-authority-production-postflight-${COMMIT}.json`,
  ...
  assert.throws(() => parseOrderPaymentEventTransitionAuthorityPostflightConfig(
    {
      ...environment(evidencePath),
  ...
  writeOrderPaymentEventTransitionAuthorityPostflightEvidence(evidencePath, {
    status: "occupied",
  });
  assert.throws(() => parseOrderPaymentEventTransitionAuthorityPostflightConfig(
    environment(evidencePath),
...
test("transition-authority postflight requires exact clean Git state", () => {
  assert.deepEqual(
    assertOrderPaymentEventTransitionAuthorityPostflightGitState(
      { head: COMMIT, status: "" },
      COMMIT,
  ...
  assert.throws(() => assertOrderPaymentEventTransitionAuthorityPostflightGitState(
...
test("transition-authority postflight accepts only exact catalog and bounded aggregates", () => {
  assert.deepEqual(
    assertOrderPaymentEventTransitionAuthorityRuntimeSnapshot(
      transitionSnapshot(),
    ),
  ...
  const forged = transitionSnapshot();
  forged.functions[0].runtime_can_execute = true;
  assert.throws(() => assertOrderPaymentEventTransitionAuthorityRuntimeSnapshot(
    forged,
  ));

  const ciOwned = transitionSnapshot();
  for (const row of ciOwned.functions) row.owner_name = "ci";
  for (const row of ciOwned.triggers) row.function_owner = "ci";
  assert.doesNotThrow(() =>
    assertOrderPaymentEventTransitionAuthorityRuntimeSnapshot(ciOwned, {
      functionOwner: "ci",
    }),
  );
  assert.throws(() => assertOrderPaymentEventTransitionAuthorityRuntimeSnapshot(
  ...
  const driftedTrigger = transitionSnapshot();
  driftedTrigger.triggers[0].update_columns = [];
  assert.throws(() => assertOrderPaymentEventTransitionAuthorityRuntimeSnapshot(
    driftedTrigger,
  ));

  const impossibleAggregate = transitionSnapshot();
  impossibleAggregate.projectionAggregate.open_dispute_blocked_count = "3";
  assert.throws(() => assertOrderPaymentEventTransitionAuthorityRuntimeSnapshot(
    impossibleAggregate,
  ));
});

test("transition-authority postflight proves all private functions are denied", async () => {
  const attempted = [];
  const client = {
    async query(sql) {
      if (
...
test("transition-authority evidence is fresh, sanitized and mode 0600", () => {
  const directory = mkdtempSync(path.join(tmpdir(), "ope-transition-postflight-"));
  ...
  assert.throws(() => writeOrderPaymentEventTransitionAuthorityPostflightEvidence(
    evidencePath,
    { status: "overwritten" },
  ));
  assert.throws(() => writeOrderPaymentEventTransitionAuthorityPostflightEvidence(
...
test("transition-authority postflight source pins read-only and output safety", () => {
  const source = readFileSync(
    "scripts/order-payment-event-transition-authority-production-postflight.mjs",
    "utf8",
  );
