...
type ChartGrouping = "hour" | "day" | "month";

function getRangeDates(range: RangeKey): { startDate: Date; endDate: Date; chartGrouping: ChartGrouping } {
  const now = new Date();
  ...
  switch (range) {
    ...
    case "alltime":
    default:
      return { startDate: new Date("2020-01-01T00:00:00Z"), endDate: now, chartGrouping: "month" };
  }
}

function generateHourBuckets(startDate: Date, endDate: Date): { label: string; timestamp: string }[] {
  // For today: generate 0-23 hours. For yesterday: also 0-23 hours for that day.
  const buckets: { label: string; timestamp: string }[] = [];
  const start = new Date(startDate);
  for (let h = 0; h < 24; h++) {
    const d = new Date(Date.UTC(start.getUTCFullYear(), start.getUTCMonth(), start.getUTCDate(), h));
    if (d > endDate) break;
    const hour = h % 12 || 12;
...
export async function GET(req: Request) {
  try {
    ...
    const validRanges: RangeKey[] = ["today", "yesterday", "week", "month", "last30", "year", "last365", "alltime"];
    const range = validRanges.includes(rangeParam as RangeKey) ? (rangeParam as RangeKey) : "last30";

    const { startDate, endDate, chartGrouping } = getRangeDates(range);
    ...
    // Engagement: listing views/clicks aggregated
    const engagementListings = await prisma.listing.findMany({
      where: { sellerId },
      select: { viewCount: true, clickCount: true },
    });
    const totalViews = engagementListings.reduce((s, l) => s + l.viewCount, 0);
    const totalClicks = engagementListings.reduce((s, l) => s + l.clickCount, 0);
    const viewToClickRatio = totalViews > 0 ? (totalClicks / totalViews) * 100 : 0;
    const conversionRate = totalViews > 0 ? (totalOrders / totalViews) * 100 : 0;

    // Favorites + stock notification counts
    const listingIds = engagementListings.length > 0
      ? (await prisma.listing.findMany({ where: { sellerId }, select: { id: true } })).map((l) => l.id)
      : [];
    ...
    const [favoritesCount, stockNotificationSubs] = await Promise.all([
      listingIds.length > 0
        ...
        : 0,
      listingIds.length > 0
        ? prisma.stockNotification.count({ where: { listingId: { in: listingIds } } })
        : 0,
    ]);

    // Cart abandonment: CartItems added in range for this seller's listings with no matching order
    let cartAbandonment = 0;
    if (listingIds.length > 0) {
      type CartAbandonRow = { count: bigint };
      const cartRows = await prisma.$queryRaw<CartAbandonRow[]>`
        SELECT COUNT(*) AS count
        FROM "CartItem" ci
        WHERE ci."listingId" IN (
          SELECT id FROM "Listing" WHERE "sellerId" = ${sellerId}
        )
      ...
      `;
      cartAbandonment = Number(cartRows[0]?.count ?? 0);
    }
    ...
    // Avg processing time in hours (shippedAt - createdAt for shipped orders)
    ...
    let chartData: Array<{ label: string; timestamp: string; revenue: number; orders: number; views: number; clicks: number }> = [];
    ...
    // Get favorites + stock notification counts per listing
    const topListingIds = topListingRows.map((r) => r.id);
    const [topFavsRows, topStockRows] = await Promise.all([
      topListingIds.length > 0
        ? prisma.favorite.groupBy({ by: ["listingId"], where: { listingId: { in: topListingIds } }, _count: { _all: true } })
        : [],
      topListingIds.length > 0
        ? prisma.stockNotification.groupBy({ by: ["listingId"], where: { listingId: { in: topListingIds } }, _count: { _all: true } })
        : [],
    ]);
    ...
    const topListings = topListingRows.map((r) => {
      const daysSinceCreated = Math.max(1, Math.ceil((Date.now() - new Date(r.created_at).getTime()) / (1000 * 60 * 60 * 24)));
      return {
        id: r.id,
        title: r.title,
