 ...
 *
 * 1. POST /api/stripe/webhook, POST /api/stripe/webhook/connect, and
 *    POST /api/stripe/webhook/v2
 *    — Webhooks called by Stripe servers using HTTPS POST with a raw body.
 *      Protected by Stripe-Signature HMAC verification. Platform snapshot and
 *      classic Connect payout events use stripe.webhooks.constructEvent with
 *      distinct signing secrets; Connect v2 thin events use
 *      stripe.parseEventNotification with a third signing secret.
