...
import {
  ...
  readOrderPaymentEventTransitionAuthorityRuntimeSnapshot,
} from "./order-payment-event-transition-authority-production-postflight.mjs";
import {
...
const { Client } = pg;
const PROOF_URL_ENV =
  "ORDER_PAYMENT_EVENT_TRANSITION_POSTFLIGHT_PROOF_DATABASE_URL";

export function parseOrderPaymentEventTransitionPostflightProofConfig(
  env = process.env,
) {
...
export async function runOrderPaymentEventTransitionPostflightPostgresProof(
  config,
) {
  const client = new Client({
    connectionString: config.databaseUrl,
    application_name: "grainline-ope-transition-postflight-proof",
  ...
  try {
    ...
    return Object.freeze({
      ...
      deniedTransitionPrivateFunctionCount: transitionDenial.deniedFunctionCount,
...
async function main() {
  try {
    const result = await runOrderPaymentEventTransitionPostflightPostgresProof(
      parseOrderPaymentEventTransitionPostflightProofConfig(),
    );
    process.stdout.write(`${JSON.stringify(result)}\n`);
  } catch (error) {
    process.stderr.write(
      `OrderPaymentEvent transition-authority postflight PostgreSQL proof failed: ${
