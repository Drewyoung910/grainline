...
export function parseCheckoutStockReservationForceRollbackProofConfig(
  env = process.env,
) {
...
export async function runCheckoutStockReservationForceRollbackProof(
  env = process.env,
) {
  const { databaseUrl } =
    parseCheckoutStockReservationForceRollbackProofConfig(env);
  verifyCheckoutStockReservationForceRelease();
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    process.stdout.write(`${JSON.stringify(
      await runCheckoutStockReservationForceRollbackProof(),
      null,
      2,
    )}\n`);
  } catch (error) {
    process.stderr.write(
      `CheckoutStockReservation FORCE rollback proof failed: ${
