...
import pg from "pg";
import { runCaseCompatibleDatabasePostflight } from
  "./case-compatible-database-production-postflight.mjs";

const { Client } = pg;
const PROOF_URL_ENV = "CASE_COMPATIBLE_DB_POSTFLIGHT_PROOF_DATABASE_URL";
const PROOF_DATABASE = "grainline_ci";
const PROOF_OWNER = "ci";
const PROOF_RUNTIME_ROLE = "grainline_app_runtime";
const PROOF_PASSWORD = "case-compatible-ci-proof";
...
export function parseCaseCompatibleDatabasePostflightProofConfig(
  env = process.env,
) {
  ...
  assert.ok(
    ...
    "Case compatible database postflight proof refuses a non-loopback database",
  );
  assert.equal(
    parsed.pathname,
    `/${PROOF_DATABASE}`,
    `Case compatible database postflight proof requires ${PROOF_DATABASE}`,
  );
  assert.equal(
    decodeURIComponent(parsed.username),
    PROOF_OWNER,
    `Case compatible database postflight proof requires owner ${PROOF_OWNER}`,
  );
...
export async function runCaseCompatibleDatabasePostflightPostgresProof(
  config,
) {
  const owner = client(
    config.databaseUrl,
    "grainline-case-compatible-postflight-proof-owner",
  );
  await owner.connect();
  ...
  try {
    ...
    await owner.query(`
      ...
      PASSWORD 'case-compatible-ci-proof'
    `);
    ...
    const result = await runCaseCompatibleDatabasePostflight({
...
async function main() {
  try {
    const config = parseCaseCompatibleDatabasePostflightProofConfig(
      process.env,
    );
    const result =
      await runCaseCompatibleDatabasePostflightPostgresProof(config);
    process.stdout.write(`${JSON.stringify(result)}\n`);
  } catch (error) {
    process.stderr.write(
      `Case compatible database postflight PostgreSQL proof failed: ${
        safeError(error)
      }\n`,
    );
    process.exitCode = 1;
