...
import {
  ...
  parseOrderPaymentEventAggregateAuthorityScopeEnvironment,
...
export const ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_SCOPE_STAGES =
...
function isAppliedRow(row) {
  return row?.migration_name === ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_MIGRATION
    && row.checksum === ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_MIGRATION_SHA256
    && row.finished_at != null
    && row.rolled_back_at == null
    && Number(row.applied_steps_count) === 1;
}

export function parseOrderPaymentEventTransitionAuthorityScopeEnvironment(
  env = process.env,
) {
  const stage = required(
    env,
    "ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_SCOPE_STAGE",
  );
  if (!ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_SCOPE_STAGES.includes(stage)) {
    throw new Error("OrderPaymentEvent transition-authority scope stage is invalid");
  }
  const predecessor = parseOrderPaymentEventAggregateAuthorityScopeEnvironment({
    ...env,
    ORDER_PAYMENT_EVENT_AGGREGATE_AUTHORITY_SCOPE_STAGE: "after",
  });
...
export function assertOrderPaymentEventTransitionAuthorityLedger(rows, stage) {
  if (
    !Array.isArray(rows)
    || !ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_SCOPE_STAGES.includes(stage)
    || rows.some(
      (row) => row?.migration_name
        !== ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_MIGRATION,
    )
  ) {
    throw new Error("OrderPaymentEvent transition-authority ledger is invalid");
  }
  ...
  if (
    ...
    throw new Error(
      "OrderPaymentEvent transition-authority ledger is not at the reviewed stage",
    );
  }
  return applied;
}
...
function assertColumns(rows, applied) {
  if (!applied) {
    if (!Array.isArray(rows) || rows.length !== 0) {
      throw new Error("Order open-dispute projection exists before migration");
    }
    return;
  }
  if (
    !Array.isArray(rows)
    || rows.length !== 1
    || rows[0]?.column_name !== ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_COLUMNS[0]
    || rows[0]?.type_name !== "boolean"
    || rows[0]?.not_null !== true
    || rows[0]?.default_expression !== "false"
  ) {
    throw new Error("Order open-dispute projection column catalog drifted");
  }
}
...
function assertFunctions(rows, applied, migrationRole, root) {
  if (!applied) {
    if (!Array.isArray(rows) || rows.length !== 0) {
      throw new Error("Order open-dispute functions exist before migration");
    }
    return;
  }
  const sources = orderPaymentEventTransitionAuthorityFunctionSources(root);
  const byIdentity = new Map(
    Array.isArray(rows) ? rows.map((row) => [row?.identity, row]) : [],
  );
  if (
    !Array.isArray(rows)
    || rows.length !== ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_FUNCTION_IDENTITIES.length
    || byIdentity.size !== ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_FUNCTION_IDENTITIES.length
    || !ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_FUNCTION_IDENTITIES.every(
      (identity) => {
        ...
        return row?.identity === identity
          ...
          && row.function_kind === "f"
          && row.language_name === (identity.endsWith("(text)") ? "sql" : "plpgsql")
          && row.volatility === "v"
    ...
    throw new Error("Order open-dispute function catalog drifted");
  }
}
...
function assertTriggers(rows, applied, migrationRole) {
  if (!applied) {
    if (!Array.isArray(rows) || rows.length !== 0) {
      throw new Error("Order open-dispute triggers exist before migration");
    }
    return;
  }
  ...
  const guard = byName.get("grainline_order_payment_open_dispute_guard");
  const refresh = byName.get("grainline_order_payment_open_dispute_refresh");
  if (
    !Array.isArray(rows)
    || rows.length !== ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_TRIGGERS.length
    || byName.size !== ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_TRIGGERS.length
    || !guard
    || !refresh
  ) {
    throw new Error("Order open-dispute trigger inventory drifted");
  }
  if (
    ...
    || guard.update_columns.length !== 1
    || guard.update_columns[0] !== "paymentOpenDisputeBlocked"
    || guard.function_schema !== "public"
    || guard.function_owner !== migrationRole
    || guard.function_identity !== "grainline_order_payment_open_dispute_guard()"
    || guard.function_kind !== "f"
  ) {
    throw new Error("Order open-dispute guard trigger drifted");
  }
  if (
    ...
    || refresh.function_identity !== "grainline_order_payment_open_dispute_refresh()"
    || refresh.function_kind !== "f"
  ) {
    throw new Error("Order open-dispute refresh trigger drifted");
  }
}

export function assertOrderPaymentEventTransitionAuthorityProductionScope(
  snapshot,
  stage,
  {
    assertPredecessor = assertOrderPaymentEventAggregateAuthorityProductionScope,
    migrationRole = MIGRATION_ROLE,
    runtimeRole = RUNTIME_ROLE,
    root = process.cwd(),
  } = {},
) {
  const applied = assertOrderPaymentEventTransitionAuthorityLedger(
    snapshot?.candidateLedgerRows,
    stage,
  );
  const predecessor = assertPredecessor(
    snapshot?.orderPaymentEventAggregateAuthority,
    "after",
  ...
  if (
    ...
    throw new Error("Order open-dispute projection does not match immutable evidence");
  }
  return Object.freeze({
    orderPaymentEventAggregateAuthorityApplied:
      predecessor.orderPaymentEventAggregateAuthorityApplied,
    orderPaymentEventTransitionAuthorityApplied: applied,
    privateFunctionCount: applied
      ? ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_FUNCTIONS.length
      : 0,
    projectionColumnCount: applied ? 1 : 0,
    ...
    state: applied
      ...
      : "transition-authority-predecessor",
...
async function readFunctions(client, runtimeRole) {
  return (await client.query(
    `SELECT
     ...
     ORDER BY identity`,
    [runtimeRole, ORDER_PAYMENT_EVENT_TRANSITION_AUTHORITY_FUNCTIONS],
  )).rows;
}
...
async function readProjectionMismatchCount(client) {
  return Number((await client.query(
    `SELECT pg_catalog.count(*)::integer AS mismatch_count
      ...
      WHERE orders."paymentOpenDisputeBlocked" IS DISTINCT FROM
            public.grainline_order_payment_open_dispute_state(orders.id)`,
  )).rows[0]?.mismatch_count);
}

export async function readOrderPaymentEventTransitionAuthorityProductionSnapshotFromClient(
  client,
  { runtimeRole = RUNTIME_ROLE, root = process.cwd() } = {},
) {
  verifyOrderPaymentEventTransitionAuthorityMigrationBytes(root);
  const orderPaymentEventAggregateAuthority =
  ...
  const projectionMismatchCount = columns.length === 1
...
export async function readOrderPaymentEventTransitionAuthorityProductionSnapshot(
  connectionString,
  { runtimeRole = RUNTIME_ROLE, root = process.cwd() } = {},
) {
  verifyOrderPaymentEventTransitionAuthorityMigrationBytes(root);
  const client = new Client({
    ...
    application_name: "grainline-order-payment-transition-authority-scope-proof",
...
export async function verifyOrderPaymentEventTransitionAuthorityProductionScope(
  config,
  {
    readSnapshot = readOrderPaymentEventTransitionAuthorityProductionSnapshot,
    assertPredecessor = assertOrderPaymentEventAggregateAuthorityProductionScope,
    migrationRole = MIGRATION_ROLE,
    runtimeRole = RUNTIME_ROLE,
    root = process.cwd(),
  ...
  return assertOrderPaymentEventTransitionAuthorityProductionScope(
...
async function main() {
  try {
    const config = parseOrderPaymentEventTransitionAuthorityScopeEnvironment();
    const result =
      await verifyOrderPaymentEventTransitionAuthorityProductionScope(config);
    process.stdout.write(`${JSON.stringify(result)}\n`);
  } catch {
    process.stderr.write(
      "OrderPaymentEvent transition-authority production scope proof failed closed.\n",
    );
    process.exitCode = 1;
  }
}
