...
import {
  runOrderCompatibleRuntimePostflight,
} from "./order-compatible-runtime-postflight.mjs";

const { Client } = pg;
const PROOF_ENV = "ORDER_COMPATIBLE_RUNTIME_POSTFLIGHT_PROOF_DATABASE_URL";
const PROOF_DATABASE = "grainline_ci";
const PROOF_OWNER = "ci";
const PROOF_RUNTIME_ROLE = "grainline_app_runtime";
const PROOF_PASSWORD = "order-compatible-runtime-postflight-proof";
...
export function parseOrderCompatibleRuntimePostflightProofConfig(
  env = process.env,
) {
  ...
  assert.ok(
    ...
    "Order compatible runtime postflight proof refuses a non-loopback database",
  );
...
export async function runOrderCompatibleRuntimePostflightProof(config) {
  const owner = client(
    config.databaseUrl,
    "grainline-order-compatible-runtime-postflight-proof-owner",
  );
  await owner.connect();
  ...
  try {
    ...
    await owner.query(`
      ...
      PASSWORD 'order-compatible-runtime-postflight-proof'
    `);
    ...
    const result = await runOrderCompatibleRuntimePostflight({
    ...
    assert.equal(result.runtimeFunctionCount, 40);
    assert.equal(result.privateFunctionCount, 8);
    return result;
  } finally {
...
async function main() {
  try {
    const config = parseOrderCompatibleRuntimePostflightProofConfig(
      process.env,
    );
    const result = await runOrderCompatibleRuntimePostflightProof(config);
    process.stdout.write(`${JSON.stringify(result)}\n`);
  } catch (error) {
    process.stderr.write(
      `Order compatible runtime postflight PostgreSQL proof failed: ${safeError(error)}\n`,
    );
    process.exitCode = 1;
  }
}
