...
export const SELLER_PAYOUT_EVENT_FORCE_DRAFT =
  "docs/rls-drafts/seller-payout-event-force.sql";
export const SELLER_PAYOUT_EVENT_FORCE_DRAFT_SHA256 =
  "12a0bc72942fe322e1b061667e687f46240e18bcdd0ddb459d5b28f57b8134a9";
export const SELLER_PAYOUT_EVENT_FORCE_MIGRATION_SHA256 =
  "d15faee92ed7dcee599cd66a306928d52ce84fab73589db177c39baca871b12f";
export const SELLER_PAYOUT_EVENT_FORCE_ROLLBACK_DRAFT =
  "docs/rls-drafts/seller-payout-event-force-rollback.sql";
export const SELLER_PAYOUT_EVENT_FORCE_ROLLBACK_SHA256 =
  "aaa4724038c54cc58dfd28022277a72b18f875c788da3725f2b6c46a869c70c5";
export const SELLER_PAYOUT_EVENT_FORCE_STAGING_ACK =
  "I_ACKNOWLEDGE_LOOPBACK_SELLER_PAYOUT_EVENT_FORCE_STAGING";
...
export function buildSellerPayoutEventForceCandidate(
  rootDirectory = process.cwd(),
) {
  const draft = fs.readFileSync(
    path.join(rootDirectory, SELLER_PAYOUT_EVENT_FORCE_DRAFT),
    "utf8",
  );
  const draftSha256 = sha256(draft);
  if (draftSha256 !== SELLER_PAYOUT_EVENT_FORCE_DRAFT_SHA256) {
    throw new Error(
      `${SELLER_PAYOUT_EVENT_FORCE_DRAFT} byte pin drifted: expected `
      + `${SELLER_PAYOUT_EVENT_FORCE_DRAFT_SHA256}, got ${draftSha256}`,
    );
  }
  if (!draft.startsWith(`${draftHeader}\n`)) {
    throw new Error("SellerPayoutEvent FORCE draft header is missing");
  }
  ...
  for (const pattern of forbidden) {
    if (pattern.test(migration)) {
      throw new Error(
        `SellerPayoutEvent FORCE candidate crossed its reviewed boundary: ${pattern}`,
      );
    }
  }
  ...
  if (
    ...
    || count(migration, /IF accepted_function_count <> 3/gu) !== 1
    || count(migration, /IF named_runtime_function_count <> 3/gu) !== 1
    || count(migration, /IF table_function_count <> 3/gu) !== 1
  ) {
    throw new Error("SellerPayoutEvent FORCE candidate catalog count drifted");
  }

  const migrationSha256 = sha256(migration);
  if (migrationSha256 !== SELLER_PAYOUT_EVENT_FORCE_MIGRATION_SHA256) {
    throw new Error(
      "SellerPayoutEvent FORCE promoted migration byte pin drifted",
    );
  }
  const rollbackDraft = fs.readFileSync(
    path.join(rootDirectory, SELLER_PAYOUT_EVENT_FORCE_ROLLBACK_DRAFT),
    "utf8",
  );
  const rollbackDraftSha256 = sha256(rollbackDraft);
  if (rollbackDraftSha256 !== SELLER_PAYOUT_EVENT_FORCE_ROLLBACK_SHA256) {
    throw new Error(
      "SellerPayoutEvent FORCE rollback draft byte pin drifted",
    );
  }
...
function assertDisposableTarget() {
  if (
    process.env.SELLER_PAYOUT_EVENT_FORCE_STAGING_ACK
      !== SELLER_PAYOUT_EVENT_FORCE_STAGING_ACK
  ) {
    throw new Error("disposable SellerPayoutEvent FORCE acknowledgement is missing");
  }
  ...
  if (
    ...
    throw new Error(
      "SellerPayoutEvent FORCE migration may be staged only for loopback grainline_ci",
    );
  }
}
...
function stageCandidate(migration, rootDirectory = process.cwd()) {
  ...
  if (fs.existsSync(directory)) {
    throw new Error(`SellerPayoutEvent FORCE destination exists: ${directory}`);
  }
...
function unstageCandidate(migration, rootDirectory = process.cwd()) {
  const { directory, migrationPath } = candidatePaths(rootDirectory);
  if (!fs.existsSync(directory)) {
    throw new Error("SellerPayoutEvent FORCE destination does not exist");
  }
  ...
  if (
    ...
    throw new Error("refusing to remove drifted SellerPayoutEvent FORCE migration");
  }
...
function main() {
  ...
  if (!["--verify", "--stage", "--unstage"].includes(mode)) {
    throw new Error(
      "usage: stage-seller-payout-event-force-migration.mjs "
  ...
  process.stdout.write(`${JSON.stringify({
    mode,
    migrationName: SELLER_PAYOUT_EVENT_FORCE_MIGRATION,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    ...
    process.stderr.write(
      `SellerPayoutEvent FORCE staging failed closed: ${
