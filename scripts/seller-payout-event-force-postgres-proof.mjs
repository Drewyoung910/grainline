...
const { Client } = pg;
const DATABASE_ENV = "SELLER_PAYOUT_EVENT_FORCE_PROOF_DATABASE_URL";
const DATABASE_NAME = "grainline_ci";
const OWNER_ROLE = "ci";
const RUNTIME_ROLE = "grainline_app_runtime";

export function parseSellerPayoutEventForceProofConfig(env = process.env) {
  ...
  assert.ok(
    ...
    "SellerPayoutEvent FORCE proof refuses a non-loopback database",
  );
  assert.equal(parsed.pathname, `/${DATABASE_NAME}`);
  assert.equal(decodeURIComponent(parsed.username), RUNTIME_ROLE);
  assert.ok(parsed.password, "SellerPayoutEvent FORCE proof requires direct login");
  return Object.freeze({ databaseUrl });
}

async function expectedFailure(client, operation, code, label) {
  await client.query("SAVEPOINT seller_payout_force_expected_failure");
  let caught;
  try {
    await operation();
  } catch (error) {
  ...
  await client.query("ROLLBACK TO SAVEPOINT seller_payout_force_expected_failure");
  await client.query("RELEASE SAVEPOINT seller_payout_force_expected_failure");
  assert.equal(caught?.code, code, `${label} returned the wrong SQLSTATE`);
}

export async function runSellerPayoutEventForceProof(env = process.env) {
  const { databaseUrl } = parseSellerPayoutEventForceProofConfig(env);
  verifySellerPayoutEventForceRelease();
  ...
  try {
    ...
    await proveSellerPayoutEventActivatedCatalog(client, OWNER_ROLE, true);
    await expectedFailure(
      client,
      () => client.query('SELECT id FROM public."SellerPayoutEvent" LIMIT 1'),
      "42501",
      "direct runtime read",
    );

    const absentActor = "seller-payout-force-proof-absent-user";
    ...
    await expectedFailure(
      ...
      () => client.query(`
        SELECT * FROM public.grainline_seller_payout_event_apply(
          'seller-payout-force-proof-absent-event',
          1,
          $1,
          'acct_seller_payout_force_proof_absent',
          'po_seller_payout_force_proof_absent',
          0,
          'usd',
          'force-proof',
          'Read-only fence'
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    process.stdout.write(`${JSON.stringify(
      await runSellerPayoutEventForceProof(),
      null,
      2,
    )}\n`);
  } catch (error) {
    process.stderr.write(
      `SellerPayoutEvent FORCE PostgreSQL proof failed: ${
