...
async function applyThroughWorker(sourceRoot, parent, env, crashBeforeDeploy = false) {
  ...
  if (crashBeforeDeploy) {
    ...
    fs.writeFileSync(file, bytes.replace(boundary, `process.kill(process.pid, "SIGKILL");\n      ${boundary}`));
  ...
  try {
    process.stdout.write(`Disposable ${crashBeforeDeploy ? "crash" : "application"} fixture: preparing clean worker.\n`);
    ...
    if (crashBeforeDeploy) {
      await assert.rejects(worker.executeDisposable(payload)); await worker.close();
      ...
      return { crashBeforeDeployPreserved: true };
    }
...
export async function proveOrderExecution(env = process.env) {
  ...
  const crashed = await applyThroughWorker(sourceRoot, crashParent, env, true);
