...
export const CHECKOUT_STOCK_RESERVATION_ACTIVATION_POSTFLIGHT_CONFIRMATION =
...
export function parseCheckoutStockReservationActivationPostflightConfig(
  env = process.env,
) {
  assertDeterministicPostgresEnvironment(
    env,
    "CheckoutStockReservation activation production postflight",
  );
  if (
    env.CHECKOUT_STOCK_RESERVATION_ACTIVATION_POSTFLIGHT_CONFIRM
      !== CHECKOUT_STOCK_RESERVATION_ACTIVATION_POSTFLIGHT_CONFIRMATION
  ) {
    throw new Error(
      "CheckoutStockReservation activation postflight confirmation is invalid",
    );
  }
  const privilegedKeys = privilegedDatabaseEnvironmentKeys(env);
  if (privilegedKeys.length > 0) {
    throw new Error(
      `CheckoutStockReservation activation postflight rejects privileged database keys: ${privilegedKeys.join(", ")}`,
    );
  }
  const unreviewedUrlKeys = unreviewedPostgresUrlEnvironmentKeys(env);
  if (unreviewedUrlKeys.length > 0) {
    throw new Error(
      `CheckoutStockReservation activation postflight rejects aliased PostgreSQL URLs: ${unreviewedUrlKeys.join(", ")}`,
    );
  }
  ...
  if (!COMMIT_PATTERN.test(releaseCommit)) {
    throw new Error(
      "CheckoutStockReservation activation release commit is invalid",
    );
  }
  ...
  const evidencePath = path.resolve(required(
    env,
    "CHECKOUT_STOCK_RESERVATION_ACTIVATION_POSTFLIGHT_EVIDENCE_PATH",
  ));
  if (
    ...
    throw new Error(
      "CheckoutStockReservation activation evidence path is not fresh and exact",
    );
  }
...
export function readCheckoutStockReservationActivationPostflightGitState(
  cwd = process.cwd(),
) {
...
export function assertCheckoutStockReservationActivationPostflightGitState(
  state,
  releaseCommit,
) {
  if (state?.head !== releaseCommit || state.status !== "") {
    throw new Error(
      "CheckoutStockReservation activation postflight requires the exact clean release commit",
    );
  }
...
async function expectSqlState(client, operation, code, label) {
  await client.query("SAVEPOINT checkout_reservation_activation_expected_failure");
  let caught;
  try {
    await operation();
  } catch (error) {
  ...
  await client.query(
    "ROLLBACK TO SAVEPOINT checkout_reservation_activation_expected_failure",
  );
  await client.query(
    "RELEASE SAVEPOINT checkout_reservation_activation_expected_failure",
  );
  assert.equal(caught?.code, code, `${label} returned the wrong SQLSTATE`);
}

export async function verifyCheckoutStockReservationActivationRuntimeIdentity(
  client,
  expected,
...
export async function verifyCheckoutStockReservationActivatedCatalog(
  client,
  migrationRole = MIGRATION_ROLE,
      ...
      (SELECT pg_catalog.count(*)::integer
        ...
        WHERE actual_namespace.nspname = 'public'
          AND (
            ...
            OR (
              actual.proname = 'grainline_stripe_webhook_bind_source'
              AND pg_catalog.oidvectortypes(actual.proargtypes) =
                  'text, text, bigint, text'
            )
            OR (
              actual.proname = 'grainline_stripe_webhook_begin'
              AND pg_catalog.oidvectortypes(actual.proargtypes) =
                  'text, text, text'
            )
  ...
  for (const row of functions.rows) {
    ...
    assert.equal(row.parallel_mode, expected.parallelSafety, signature);
...
export async function runCheckoutStockReservationActivationPostflight(
  ...
  const client = new Client({
    ...
    application_name: "grainline-checkout-stock-reservation-activation-postflight",
    ...
    const fixedRead = await client.query(`
      SELECT pg_catalog.count(*)::integer AS count
        FROM public.grainline_checkout_reservation_export(
          'grainline-activation-postflight-absent-user'
        )
    `);
...
export function writeCheckoutStockReservationActivationPostflightEvidence(
  pathname,
  evidence,
) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink() || (stat.mode & 0o777) !== 0o600) {
    throw new Error(
      "CheckoutStockReservation activation evidence is not mode 0600",
    );
  }
}
...
async function main() {
  try {
    const config = parseCheckoutStockReservationActivationPostflightConfig(
      process.env,
    );
    const evidence = await runCheckoutStockReservationActivationPostflight(
      config,
    );
    ...
    process.stderr.write(
      `CheckoutStockReservation activation production postflight failed: ${
