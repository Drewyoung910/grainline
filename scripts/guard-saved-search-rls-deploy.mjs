...
export const SELLER_PAYOUT_EVENT_FORCE_MIGRATION =
...
export const CHECKOUT_STOCK_RESERVATION_SOURCE_CONSISTENCY_MIGRATION_TREE_SHA256 =
  "527b93f81e4b74a2cf04218d2d4b53cd8524bbb4fc9b93db6072c387bbb71e54";
export const CHECKOUT_STOCK_RESERVATION_ACTIVATION_MIGRATION_TREE_SHA256 =
  "b014ea6ccc6ec6107e06897269ed607e6a8930c770fea3914e4b6b8b42b502f3";
export const CHECKOUT_STOCK_RESERVATION_FORCE_MIGRATION_TREE_SHA256 =
  "75971d49d54b46759851be1f39353fee5132465ce8da59a8b3251a267216aa86";
export const SELLER_PAYOUT_EVENT_ACTIVATION_MIGRATION_TREE_SHA256 =
  "f680540b155b116e8fcba1cb3a33e84b87b59f07b53466956554a5313485b006";
export const SELLER_PAYOUT_EVENT_FORCE_MIGRATION_TREE_SHA256 =
  "e83f2289eca349834970b72bdf41ae181a2c6e50394cc3908b944af75a118d38";
export const ORDER_PAYMENT_EVENT_ACTIVATION_MIGRATION_TREE_SHA256 =
  "389cfab874e29921027e6661f7abd8e8286a46db8505cac589d15823c57b3adf";
export const ORDER_PAYMENT_EVENT_FORCE_MIGRATION_TREE_SHA256 =
...
const REVIEWED_ORDER_PAYMENT_EVENT_ACTIVATION =
  "order-payment-event-activation-reviewed";
const REVIEWED_ORDER_PAYMENT_EVENT_FORCE =
...
function assertReviewedMigrationTree(phase, migrationTreeSha256) {
  const expected = {
    ...
    [REVIEWED_ORDER_PAYMENT_EVENT_ACTIVATION]:
      ORDER_PAYMENT_EVENT_ACTIVATION_MIGRATION_TREE_SHA256,
    [REVIEWED_ORDER_PAYMENT_EVENT_FORCE]:
...
export function validateSavedSearchRlsDeployShape({
  ...
  const hasCheckoutStockReservationSourceConsistencyMigration = migrations.has(
    CHECKOUT_STOCK_RESERVATION_SOURCE_CONSISTENCY_MIGRATION,
  );
  const hasCheckoutStockReservationActivationMigration = migrations.has(
    CHECKOUT_STOCK_RESERVATION_ACTIVATION_MIGRATION,
  );
  const hasCheckoutStockReservationForceMigration = migrations.has(
    CHECKOUT_STOCK_RESERVATION_FORCE_MIGRATION,
  );
  const hasSellerPayoutEventAuthorityMigration = migrations.has(
    SELLER_PAYOUT_EVENT_AUTHORITY_MIGRATION,
  );
  const hasSellerPayoutEventActivationMigration = migrations.has(
    SELLER_PAYOUT_EVENT_ACTIVATION_MIGRATION,
  );
  const hasSellerPayoutEventForceMigration = migrations.has(
    SELLER_PAYOUT_EVENT_FORCE_MIGRATION,
  );
  const hasOrderPaymentEventActivationMigration = migrations.has(
    ORDER_PAYMENT_EVENT_ACTIVATION_MIGRATION,
  );
  const hasOrderPaymentEventForceMigration = migrations.has(
    ORDER_PAYMENT_EVENT_FORCE_MIGRATION,
  );
  ...
  if (phase === REVIEWED_CHECKOUT_STOCK_RESERVATION_SOURCE_CONSISTENCY) {
    if (
      !hasStripeWebhookEventForceMigration
      || !hasCheckoutStockReservationAuthorityMigration
      || !hasCheckoutStockReservationSourceConsistencyMigration
    ) {
      throw new Error(
        `${REVIEWED_CHECKOUT_STOCK_RESERVATION_SOURCE_CONSISTENCY} requires the `
        + "completed StripeWebhookEvent FORCE and CheckoutStockReservation "
        + "compatible-authority boundaries plus the reviewed source-consistency migration",
      );
    }
  ...
  if (phase === REVIEWED_CHECKOUT_STOCK_RESERVATION_ACTIVATION) {
    if (
      !hasStripeWebhookEventForceMigration
      || !hasCheckoutStockReservationAuthorityMigration
      || !hasCheckoutStockReservationSourceConsistencyMigration
      || !hasCheckoutStockReservationActivationMigration
    ) {
      throw new Error(
        `${REVIEWED_CHECKOUT_STOCK_RESERVATION_ACTIVATION} requires the `
        + "completed StripeWebhookEvent FORCE and CheckoutStockReservation "
        + "authority/source-consistency boundaries plus the reviewed "
        + "policyless CheckoutStockReservation activation migration",
      );
    }
  ...
  if (phase === REVIEWED_CHECKOUT_STOCK_RESERVATION_FORCE) {
    if (
      !hasStripeWebhookEventForceMigration
      || !hasCheckoutStockReservationAuthorityMigration
      || !hasCheckoutStockReservationSourceConsistencyMigration
      || !hasCheckoutStockReservationActivationMigration
      || !hasCheckoutStockReservationForceMigration
    ) {
      throw new Error(
        `${REVIEWED_CHECKOUT_STOCK_RESERVATION_FORCE} requires the completed `
        + "StripeWebhookEvent FORCE and CheckoutStockReservation authority, "
        + "source-consistency and policyless activation boundaries plus the "
        + "reviewed posture-only CheckoutStockReservation FORCE migration",
      );
    }
  ...
  if (phase === REVIEWED_SELLER_PAYOUT_EVENT_ACTIVATION) {
    if (
      !hasCheckoutStockReservationForceMigration
      || !hasSellerPayoutEventAuthorityMigration
      || !hasSellerPayoutEventActivationMigration
    ) {
      throw new Error(
        `${REVIEWED_SELLER_PAYOUT_EVENT_ACTIVATION} requires the completed `
        + "CheckoutStockReservation FORCE boundary, the compatible "
        + "SellerPayoutEvent authority migration and the reviewed policyless "
        + "SellerPayoutEvent activation migration",
      );
    }
  ...
  if (phase === REVIEWED_SELLER_PAYOUT_EVENT_FORCE) {
    if (
      !hasCheckoutStockReservationForceMigration
      || !hasSellerPayoutEventAuthorityMigration
      || !hasSellerPayoutEventActivationMigration
      || !hasSellerPayoutEventForceMigration
    ) {
      throw new Error(
        `${REVIEWED_SELLER_PAYOUT_EVENT_FORCE} requires the completed `
        + "CheckoutStockReservation FORCE boundary, the compatible "
        + "SellerPayoutEvent authority migration, policyless activation, "
        + "and reviewed posture-only FORCE migration",
      );
    }
  ...
  if (phase === REVIEWED_ORDER_PAYMENT_EVENT_ACTIVATION) {
    if (
      !hasSellerPayoutEventForceMigration
      || !hasOrderPaymentEventActivationMigration
    ) {
      throw new Error(
        `${REVIEWED_ORDER_PAYMENT_EVENT_ACTIVATION} requires the completed `
        + "SellerPayoutEvent FORCE boundary and the reviewed policyless "
        + "OrderPaymentEvent activation migration",
      );
    }
  ...
  if (phase === REVIEWED_ORDER_PAYMENT_EVENT_FORCE) {
    if (
      !hasOrderPaymentEventActivationMigration
      || !hasOrderPaymentEventForceMigration
    ) {
      throw new Error(
        `${REVIEWED_ORDER_PAYMENT_EVENT_FORCE} requires the accepted `
        + "OrderPaymentEvent Phase-A activation and reviewed posture-only "
        + "FORCE migration",
      );
    }
